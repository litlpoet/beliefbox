...
class KDNode {
 public:
  ...
  /// Make a node
  KDNode(const Vector& c_, int a_, Vector& inf, Vector& sup,
         const void* object_)
      : c(c_), a(a_), lower(NULL), upper(NULL), object(object_) {
    assert(a >= 0 && a < c.Size());
    box_inf = inf;
    box_sup = sup;
  ...
  KDNode* AddVector(const Vector& x, Vector& inf, Vector& sup,
                    const void* object);
  void NearestNeighbour(const Vector& x, KDNode*& nearest, real& dist);
  void KNearestNeighbours(const Vector& x, OrderedFixedList<KDNode>& knn_list,
                          real& dist);
};
...
class void_KDTree {
 ...
 public:
  ...
  OrderedFixedList<KDNode> FindKNearestNeighboursLinear(const Vector& x,
                                                        const int K);
...
class KDTree : public void_KDTree {
 public:
  ...
  OrderedFixedList<KDNode> FindKNearestNeighbours(const Vector& x,
                                                  const int K) {
    return void_KDTree::FindKNearestNeighbours(x, K);
  }

  T* getObject(KDNode* node) { return (T*)node->object; }
