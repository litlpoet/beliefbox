// -*- Mode: c++ -*-
/* VER: $Id: MathFunctions.c,v 1.2 2006/11/06 15:48:53 cdimitrakakis Exp
 * cdimitrakakis $ */
...
real logAdd(real x, real y) {
  ...
  if (std::isnan(minusdif))
    fprintf(stderr, "LogAdd: minusdif (%f) y (%f) or x (%f) is nan", minusdif,
            y, x);
#endif
...
#ifdef DEBUG
  if (std::isnan(minusdif))
    fprintf(stderr, "LogSub: minusdif (%f) y (%f) or x (%f) is nan", minusdif,
            y, x);
#endif
  if (x == y)
    return LOG_ZERO;
