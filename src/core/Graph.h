...
struct Edge {
  ...
  int dst;
  real w;
  Edge(int src_, int dst_, real w_ = 1.0) : src(src_), dst(dst_), w(w_) {}
};
...
class Graph {
 ...
 public:
  ...
  virtual ~Graph();
  virtual bool edge(int src, int dst) = 0;  ///< Returns true if there is an
  /// edge from \c src to \c dst.
  virtual real distance(int src, int dst) = 0;  ///< Distance between two nodes.
  /// Note that in some
  /// applications this would have
  /// the meaning of an edge weight
