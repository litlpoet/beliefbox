    ...
    int num_ints)     // number of integer variables
{
  ...
  int coordinates[MAX_NUM_VARS * 2 +
                  1]; /* one interval number per relevant dimension */
...
collision_table::~collision_table() { delete[] data; }
...
void GetTiles(int tiles[], int nt, collision_table *ct, float floats[],
              int nf) {
...
void GetTilesWrap(int tiles[], int num_tilings, int memory_size, float floats[],
  ...
  GetTilesWrap(tiles, num_tilings, memory_size, floats, num_floats, wrap_widths,
               i_tmp_arr, 0);
}
