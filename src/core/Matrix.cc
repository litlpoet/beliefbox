...
Matrix::Matrix(int rows_, int columns_, real* y,
               enum BoundsCheckingStatus check_)
    : rows(rows_),
      columns(columns_),
      x(y),
      checking_bounds(check_),
...
Matrix::Matrix(const Vector& v, enum BoundsCheckingStatus check_)
    : checking_bounds(check_), transposed(false), clear_data(true) {
  rows = v.Size();
  columns = 1;
...
Matrix Matrix::operator+(const Matrix& rhs) {
  ...
  Matrix lhs(M, N);
#if 1
  gsl_matrix_const_view A_view = gsl_matrix_const_view_array(x, rows, columns);
  gsl_matrix_const_view B_view =
      gsl_matrix_const_view_array(rhs.x, rhs.rows, rhs.columns);
  gsl_matrix_view C_view = gsl_matrix_view_array(lhs.x, lhs.rows, lhs.columns);
...
Matrix Matrix::operator*(const Matrix& rhs) {
  ...
  gsl_blas_dgemm(Trans_A, Trans_B, 1.0, &A_view.matrix, &B_view.matrix, 0.0,
                 &C_view.matrix);
  return C;
    ...
    while (fabs(d) < epsilon) {
      ...
      if (m >= n) {
        ...
        throw std::runtime_error(
            "Could not do the elimination, matrix singular");
      }
      d = A(m, k);
    }
    if (m != k) {
...
std::vector<Matrix> Matrix::LUDecomposition(real& determinant, real epsilon) {
  if (rows != columns) {
    throw std::domain_error(
        "LU Decomposition cannot be performed for non-square matrices");
  }
  const int n = rows;
...
std::vector<Matrix> Matrix::QRDecomposition() const {
  ...
  for (int k = 0; k < n; ++k) {
    ...
    if (s == 0) {
      ...
      throw std::runtime_error(
          "Could not do QR Decomposition, matrix not positive definite");
    }
    d(k) = (A(k, k) > 0) ? (-s) : s;
...
void Matrix::Cholesky(Matrix& chol, real epsilon) const {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (chol(i, i) <= 0) {
      ...
      fprintf(stderr, "\nproblem from %d %f\n", i, chol(i, i));
      throw std::runtime_error(
          "Could not do Cholesky, matrix not positive definite");
    }
    chol(i, i) = sqrt(chol(i, i));

    for (int j = i + 1; j < n; j++) {
      chol(i, j) = (*this)(i, j);
      for (int k = 0; k < i; k++) chol(i, j) -= chol(k, i) * chol(k, j);
      chol(i, j) /= chol(i, i);
    }
  }
}
