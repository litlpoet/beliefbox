...
bool SparseGraph::rCalculateDistance_iter(real* dist, int j, int* C) {
  ...
  for (HalfEdgeListIterator i = parent_list.begin(); i != parent_list.end();
       ++i) {
    int n = i->node;
    if (C == NULL || C[n] == 0) {
      if (n != j) {
...
bool SparseGraph::edge(int src, int dst) {
  ...
  for (HalfEdgeListIterator i = child_list.begin(); i != child_list.end();
       ++i) {
    if (dst == i->node) {
      return true;
    }
...
int SparseGraph::n_parents(int node) { return parents[node].size(); }

int SparseGraph::n_children(int node) { return children[node].size(); }
