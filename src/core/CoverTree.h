...
class CoverTree {
 public:
  ...
  struct Node {
    ...
    Node* child;   ///< Pointer to child in the path (helps as to go down in the
                   ///tree).
    ...
    bool active_flag;  ///< Indicates if the particular node is active or not
                       ///(sampling procedure)
    int active_index;  ///< Indicates the index on the active nodes set.
    bool basis_flag;   ///< Indicates if the particular node is an active basis
                       ///or not
    int basis_index;
    ...
    Node(const CoverTree& tree_, const Vector& point_, const int level_,
         CoverTree::Node* const father_, void* object_);
    ...
    void UpdateStatistics(const Vector& state, const Vector& next_state,
                          const real& reward, const bool& absorb,
                          CoverTree::Node* const child_) {
      Insert(state, next_state, reward, absorb);
      child = child_;
      samples++;
    }
    void Insert(const Vector& state, const Vector& next_state,
                const real& reward, const bool& absorb = false) {
    ...
    real Update(const Vector& state, const Vector& next_state,
      ...
      if (samples > 1) {
        ...
        //{
        //					children[i]->point.printf(stdout);printf("
        //Samples %d",children[i]->samples);
        //					printf("\n");
        //				}
        real temp = weight * p;
        ...
        //					printf("probability = %f Samples = %d,
        //weight = %f, level = %d, depth = %d\n",p,samples,weight,level,depth);
      }
    ...
    void Insert(const Vector& new_point, const Vector& phi,
                const Vector& next_state, const real& reward,
                const bool& absorb, const int level, void* obj = NULL);

    /// Find nearest neighbour of the node
    std::pair<const CoverTree::Node*, real> NearestNeighbour(
        const Vector& query, const real distance) const;
    std::pair<CoverTree::Node*, real> FindNearestNeighbour(const Vector& query,
                                                           const real distance);

    /// The number of children
    const int Size() const { return children.size(); }
    /// A lower bound on the level of children
    const int ChildrenLevel() const { return children_level; }
    /// Find the number of descendants
    const int Descendants() const { return descendants; }
    /// Returns the nodes' depth in the tree
    const int Depth() const { return depth; }
    /// Returns the number of observed points
    const int NumObs() const { return samples; }
    ...
    const real GetWeight() const { return weight; }
    /// Returns the node's index
    const int GetIndex() const { return index; }
    const int GetSamplingIndex() const { return active_index; }
    const int GetBasisIndex() const { return basis_index; }
    const bool GetActive() const { return active_flag; }
    const bool GetActiveBasis() const { return basis_flag; }
    const real GetValueFunction() const { return Q; }
    const void SetValueFunction(const real& q_) { Q = q_; }
  ...
  struct CoverSet {
    ...
    const int NChildren(const int i) const { return nodes[i]->Size(); }
    void Show() const {
  ...
  const std::vector<std::pair<int, real> > ExternalBasisCreation(
      const Vector& state) const;

  Node* Insert(const Vector& new_point, const CoverSet& Q_i, const int level,
               void* obj);
  Node* Insert(const Vector& new_point, const Vector& phi,
               const Vector& next_state, const real& reward, const bool& absorb,
               const CoverSet& Q_i, const int level, void* obj);
  Node* Insert(const Vector& new_point, void* obj = NULL);
  Node* Insert(const Vector& new_point, const Vector& next_state,
               const real& reward, const bool& absorb = false,
               void* obj = NULL);
  ...
  void ShowSampling() const;
  void ShowBasis() const;
  CoverTree(real c, real a_ = 0.1, real N0_ = 0.1, RBFBasisSet* RBFs_ = NULL,
            bool Sampling = true, bool f = false);
  ~CoverTree();
