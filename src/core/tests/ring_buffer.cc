...
int main(void) {
  ...
  for (int test = 0; test < n_tests; ++test) {
    bool res =
        TestRing(1 + (int)ceil(urandom(0, 20)), (int)ceil(urandom(0, 10)));
...
bool TestRing(int number_of_entries, int ring_size) {
...

  {
    ...
    for (i = 0; rit != ring.end(); ++i, ++rit, ++lit) {
      cout << *rit << " " << *lit << " index: " << rit.GetIndex()
           << " end: " << ring.end().GetIndex() << endl;
      if (*rit != *lit) {
        flag = false;
        // break;
      }
    ...
    if (i != std::min<int>(ring.size(), X.size())) {
      flag = false;
      std::cerr << "Counted " << i << " but ring has length " << ring.size()
