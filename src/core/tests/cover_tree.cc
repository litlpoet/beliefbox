...
int check_cover_tree_query(CoverTree& tree, std::vector<Vector>& X,
                           std::vector<Vector>& Q) {
  int n_errors = 0;
  ...
  for (int i = 0; i < n_test_points; ++i) {
    ...
    if (node) {
      ...
      if (error > 0.000001) {
        ...
        printf("Query: ");
        Q[i].print(stdout);
        printf("Found: (%f) ", tree.metric(Q[i], node->point));
        best_point.print(stdout);
        printf("Best: (%f) ", dist);
        X[arg_min].print(stdout);
        Vector Q_alt = X[arg_min];
        const CoverTree::Node* node_alt = tree.NearestNeighbour(Q_alt);
        printf("Alternative result: ");
        node_alt->point.print(stdout);
        n_errors++;
      }
    } else {
      printf("NULL\n");
...
int main(int argc, char** argv) {
  ...
  if (1) {
    ...
    printf("%f %f %f # COVER TREE\n", timer_mid - timer_start,
           timer_end - timer_mid, timer_end - timer_start);
