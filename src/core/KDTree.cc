...
/// Create a tree
void_KDTree::void_KDTree(int n)
    : n_dimensions(n), box_sup(n), box_inf(n), root(NULL) {
  for (int i = 0; i < n; ++i) {
    box_sup[i] = RAND_MAX;
    box_inf[i] = -RAND_MAX;
  }
...
OrderedFixedList<KDNode> void_KDTree::FindKNearestNeighboursLinear(
    const Vector& x, const int K) {
  int N = node_list.size();
...
OrderedFixedList<KDNode> void_KDTree::FindKNearestNeighbours(const Vector& x,
                                                             const int K) {
  OrderedFixedList<KDNode> knn_list(K);
  if (!root) {
    return knn_list;
...
/// Add a point to the corresponding (upper or lower) half, creating it if
/// necessary.
KDNode* KDNode::AddVector(const Vector& x, Vector& inf, Vector& sup,
                          const void* object) {
  box_inf = inf;
  box_sup = sup;
