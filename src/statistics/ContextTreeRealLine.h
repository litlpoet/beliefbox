...
class ContextTreeRealLine {
 public:
  ...
  struct Node {
    ...
    Node(real lower_bound_, real upper_bound_, int n_branches_, int max_depth_);
    Node(Node* prev_, real lower_bound_, real upper_bound_);
    ~Node();
    real Observe(real x, real probability);
    real pdf(real x, real probability);
    void Show();
    int NChildren();
    int S;
  };
  ...
  ContextTreeRealLine(int n_branches_ = 2, int max_depth_ = 0,
                      real lower_bound = 0, real upper_bound = 1);
  ~ContextTreeRealLine();
  real Observe(real x);
  real pdf(real x);
  void Show();
