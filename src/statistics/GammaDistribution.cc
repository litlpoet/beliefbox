...
GammaDistribution::GammaDistribution(real alpha_, real beta_)
...
real GammaDistribution::log_pdf(real x) const {
  ...
  real log_pdf =
      log(x) * (alpha - 1.0) + log(beta) * alpha - beta * x - logGamma(alpha);
  return log_pdf;
}
...
real GammaDistribution::generate() { return gengam(alpha, beta); }

/// Generate using ranlib
real GammaDistribution::generate() const { return gengam(alpha, beta); }
...
real GammaDistribution::setMaximumLikelihoodParameters(
    const std::vector<real>& x, int n_iterations) {
...
GammaDistributionUnknownShapeScale::GammaDistributionUnknownShapeScale(
    real lambda_, real mu_, int nu_)
    : lambda(lambda_), mu(mu_), nu(nu_), S(0), T(0) {}
...
real GammaDistributionUnknownShapeScale::pdf(real x) const { return 0.0; }

/// Get the posterior pdf at y, given data x!
Vector GammaDistributionUnknownShapeScale::posterior_pdf(std::vector<real>& y,
                                                         std::vector<real>& x,
                                                         int K) const {
  // real t = (real) T;
...
real GammaDistributionUnknownShapeScale::LogLikelihood(std::vector<real>& x,
                                                       int K) const {
  // real t = (real) T;
  ExponentialDistribution Exp(lambda);
  real log_likelihood = LOG_ZERO;
