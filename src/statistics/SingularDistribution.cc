...
SingularDistribution::SingularDistribution(real m) { this->m = m; }

void SingularDistribution::setMean(real mean) { m = mean; }

real SingularDistribution::getMean() const { return m; }

real SingularDistribution::getVariance() const { return 0.0; }

real SingularDistribution::generate() const { return m; }
...
UnknownSingularDistribution::UnknownSingularDistribution()
    : prior(new NormalDistribution()), observed(false) {}

UnknownSingularDistribution::UnknownSingularDistribution(
    const Distribution* prior_)
    : prior(prior_), observed(false) {}

UnknownSingularDistribution::~UnknownSingularDistribution() { delete prior; }
...
real UnknownSingularDistribution::marginal_pdf(real x) const { return pdf(x); }
