...
void TestBelief(DiscreteHiddenMarkovModel* hmm, int T, real threshold,
                real stationarity, int n_em_iter,
  ...
  DiscreteHiddenMarkovModel* estimated_hmm_ptr = MakeRandomDiscreteHMM(
      hmm->getNStates(), hmm->getNObservations(), stationarity, &rng);
  DiscreteHiddenMarkovModel& estimated_hmm = *estimated_hmm_ptr;

  ExpectationMaximisation<DiscreteHiddenMarkovModel, int> EM_algo(
      estimated_hmm);
  // DiscreteHiddenMarkovModelEM EM_hmm(hmm->getNStates(),
  // hmm->getNObservations(), stationarity, &rng, n_em_iter);
  std::vector<int> s(T);
  for (int t = 0; t < T; ++t) {
...
int main(int argc, char** argv) {
  if (argc != 7) {
    fprintf(stderr,
            "Usage: test_hmm_em n_states n_observations stationarity  EM_iter "
            "T n_iter\n");
    return -1;
  }
  ...
  for (int i = 0; i < n_iter; ++i) {
    ...
    DiscreteHiddenMarkovModel* hmm = MakeRandomDiscreteHMM(
        n_states, n_observations, true_stationarity, &random_device);
    TestBelief(hmm, T, threshold, stationarity, n_em_iter, oracle_state_stats,
               em_state_stats);
    delete hmm;
  }
  ...
  for (int t = 0; t < T; ++t) {
    printf("%f %f %f %f %f %f\n", em_state_bottom[t], em_state_mean[t],
           em_state_top[t], oracle_state_bottom[t], oracle_state_mean[t],
           oracle_state_top[t]);
  }
  return 0;
}
