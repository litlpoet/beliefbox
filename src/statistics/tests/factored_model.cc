...
enum EnvironmentType { ONEDMAZE, GRIDWORLD };
...
int main(int argc, char** argv) {
  ...
  for (int iter = 0; iter < n_iter; ++iter) {
    ...
    if (!model_name.compare("FMC")) {
      ...
      factored_predictor =
          new BayesianFMC(n_obs, n_actions, max_depth + 1, prior);
    } else if (!model_name.compare("BVMM_old")) {
      factored_predictor = new BayesianPredictiveStateRepresentation(
          n_obs, n_actions, max_depth + 1, prior);
    } else if (!model_name.compare("BFMC")) {
      factored_predictor = new TFactoredPredictor<ContextTreeBMC>(
          n_actions, n_obs, max_depth + 1);
    } else if (!model_name.compare("BVMM")) {
      factored_predictor =
          new TFactoredPredictor<ContextTree>(n_actions, n_obs, max_depth + 1);
    } else if (!model_name.compare("CTW")) {
      factored_predictor = new TFactoredPredictor<ContextTreeCTW>(
          n_actions, n_obs, max_depth + 1);
    } else if (!model_name.compare("PPM")) {
      factored_predictor = new TFactoredPredictor<ContextTreePPM>(
          n_actions, n_obs, max_depth + 1);
    } else if (!model_name.compare("CTW_old")) {
      factored_predictor = new BayesianPredictiveStateRepresentationCTW(
          n_obs, n_actions, max_depth + 1, prior);
