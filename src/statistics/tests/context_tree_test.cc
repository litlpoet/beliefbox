...
class ContextTreeInterface : public AbstractContextTreeInterface {
 ...
 public:
  ...
  virtual ~ContextTreeInterface() { delete tree; }
  virtual real Observe(int x, int y) { return tree->Observe(x, y); }
};

int main(int argc, char** argv) {
  if (argc < 4) {
    std::cerr
        << "Usage: context_tree_test method depth output [input_file [T]]\n";
    exit(-1);
  }
  ...
  if (!method_name.compare("BVMM")) {
    ...
    tree =
        new ContextTreeInterface<ContextTreeCTW>(n_symbols, n_symbols, depth);
  } else if (!method_name.compare("PPM")) {
    tree =
        new ContextTreeInterface<ContextTreePPM>(n_symbols, n_symbols, depth);
  } else if (!method_name.compare("BMC")) {
    tree =
        new ContextTreeInterface<ContextTreeBMC>(n_symbols, n_symbols, depth);
  } else {
  ...
  std::cout << "Depth: " << depth << ", log loss: " << logsum / (real)T
            << ", acc: " << acc / (real)T;
