...
void tree_train_and_test(Matrix& data, Matrix& test_data,
                         LocalOptions& options);
void kernel_train_and_test(Matrix& data, Matrix& test_data,
                           LocalOptions& options);
        ...
        case 0:
          ...
          switch (option_index) {
            ...
            case 1:
              ...
              break;
            case 2:
              options.max_depth_cond = atoi(optarg);
              break;
            case 3:
              options.joint = true;
              break;
            case 4:
              ...
              break;
            case 5:
              options.n_inputs = atoi(optarg);
              break;
            case 6:
              options.grid_size = atoi(optarg);
              assert(options.grid_size >= 0);
              break;
            case 7:
              options.test_filename = optarg;
              break;
            case 8:
              options.pdf_test_filename = optarg;
              break;
            case 9:
              options.kfold = atoi(optarg);
              break;
            case 10:
              options.kernel = true;
              break;
            case 11:
              ...
              break;
            case 12:
              options.tune_bandwidth = true;
              break;
            case 13:
              options.kernel = true;
              options.double_kernel = true;
              printf("double\n");
              break;
            case 14:
              options.noise_level = atof(optarg);
              break;
            default:
...
void tree_train_and_test(Matrix& data, Matrix& test_data,
  ...
  if (options.joint) {
    ...
    cpdf = new ConditionalKDContextTree(
        2, options.max_depth, options.max_depth_cond, lower_bound_x,
        upper_bound_x, lower_bound_y, upper_bound_y);
  }
...
void kernel_train_and_test(Matrix& data, Matrix& test_data,
  ...
  if (options.joint) {
    ...
    if (options.double_kernel) {
      ...
      dcpdf = new DoubleKernelCDE(lower_bound_x.Size(), lower_bound_y.Size(),
                                  bandwidth);
    } else {
      printf("# SINGLE_KERNEL\n");
      cpdf = new KernelConditionalDensityEstimator(
          lower_bound_x.Size(), lower_bound_y.Size(), bandwidth, knn);
    }
  }
