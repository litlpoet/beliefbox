...
struct ErrorStatistics {
  ...
  real total_loss() { return Sum(loss); }
  real total_accuracy() { return Sum(accuracy); }
...
int main(int argc, char** argv) {
  ...
  if (argc < 2) {
    fprintf(stderr,
            "Usage: bayesian_markov_chain_text filename [max states [max "
            "horizon [prior]]]\n");
    exit(-1);
  } else {
  ...
  BayesianHierarchicalMarkovChain bhmc(n_observations, 1 + max_states, prior,
                                       dense);
  ...
  for (int t = 0; t < T; ++t) {
    ...
    //        polya_error.accuracy[t] +=
    //        polya.NextStateProbability(observation);
  ...
  printf("%.1f %.1f %.1f %.1f %.1f # Err\n", bmc_error.total_loss(),
         bhmc_error.total_loss(), bpsr_error.total_loss(),
         polya_error.total_loss(), ctw_error.total_loss());
  printf("%.1f %.1f %.1f %.1f %.1f # Acc\n", bmc_error.total_accuracy(),
         bhmc_error.total_accuracy(), bpsr_error.total_accuracy(),
         polya_error.total_accuracy(), ctw_error.total_accuracy());
