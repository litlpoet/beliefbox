...
SparseGaussianProcess::SparseGaussianProcess(Matrix& X_, Vector& Y_,
...
real SparseGaussianProcess::pdf(Vector& x, real y) { return 0.0; }
...
void SparseGaussianProcess::Observe(const std::vector<Vector>& x,
...
void SparseGaussianProcess::AddObservation(const std::vector<Vector>& x,
                                           const std::vector<real>& y) {
  for (int i = 0; i < (int)x.size(); ++i) {
    AddObservation(x[i], y[i]);
  }
...
void SparseGaussianProcess::Clear() { N = 0; }
