...
///
/// Returns the log-likelihood of the parameters
real NormalDistribution::setMaximumLikelihoodParametersLogNormal(
    std::vector<real>& x) {
  int T = x.size();
  real t = (real)T;
  m = Sum(x) / t;
...
//------------------ Unknown mean -----------------------//
real NormalDistributionUnknownMean::generate() { return prior.generate(); }
...
real NormalDistributionUnknownMean::pdf(real x) const { return prior.pdf(x); }
...
real NormalDistributionUnknownMean::getMean() const { return mu_n / tau_n; }
...
real NormalUnknownMeanPrecision::getMean() const { return mu_n; }
...
real NormalUnknownMeanPrecision::LogLikelihood(const std::vector<real>& x,
                                               int K) const {
...
real NormalUnknownMeanPrecision::LogLikelihoodLogNormal(
    const std::vector<real>& x, int K) const {
  real log_likelihood = LOG_ZERO;
  ExponentialDistribution accuracy_prior(1.0);
  for (int k = 0; k < K; ++k) {
    real accuracy = accuracy_prior.generate();
...
void NormalUnknownMeanPrecision::Show() const { printf("Normal-Gamma - "); }
