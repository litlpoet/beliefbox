...
DoubleKernelCDE::DoubleKernelCDE(int n_x_dimensions, int n_y_dimensions,
...
real DoubleKernelCDE::log_pdf(const Vector& x, const Vector& y) {
  ...
  real log_Z = LOG_ZERO;
  real log_P = LOG_ZERO;
  for (std::vector<PointPair>::iterator it = D.begin(); it != D.end(); ++it) {
  ...
  while (1) {
    ...
    if (current_log_p > log_p) {
      fprintf(stderr, "b: %f %f -> %f %f (%f %f) # bandwidth changed\n", b_x,
              b_y, current_b_x, current_b_y, log_p, current_log_p);
      b_x = current_b_x;
      b_y = current_b_y;
      best_x = b_x;
      best_y = b_y;
      ...
      if (!stochastic) {
        ...
        fprintf(stderr, "b: %f %f ~> %f %f (%f %f) # bandwidth sampled\n", b_x,
                b_y, current_b_x, current_b_y, log_p, current_log_p);
