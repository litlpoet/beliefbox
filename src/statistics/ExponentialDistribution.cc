...
real ExponentialDistribution::setMaximumLikelihoodParameters(
...
real GammaExponentialPrior::log_pdf(real x, int n_iterations) const {
  ...
  real l2 = logGamma(alpha + 1) - logGamma(alpha) + alpha * log(beta) -
            (alpha + 1) * log(beta + x);
  printf("%f %f\n", log_likelihood, l2);
...
real GammaExponentialPrior::LogLikelihood(const std::vector<real>& x,
  ...
  for (int i = 0; i < (int)x.size(); ++i) {
    l2 += logGamma(alpha + 1) - logGamma(alpha) + alpha * log(beta) -
          (alpha + 1) * log(beta + x[i]);
    alpha += 1.0;
    beta += x[i];
  }
