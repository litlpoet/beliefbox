...
MultivariateNormalUnknownMeanPrecision::MultivariateNormalUnknownMeanPrecision(
    const Vector& mu, const real tau, const real alpha, const Matrix& T)
    : n_dim(mu.Size()),
      marginal_mean(n_dim),
      marginal(n_dim),
      mu_0(mu),
...
void MultivariateNormalUnknownMeanPrecision::Reset() {
  ...
  marginal_mean.setPrecision(tau_0 * (alpha_0 - (real)n_dim + 1.0) *
                             T_0.Inverse());

  marginal.setDegrees(alpha_0 - (real)n_dim + 1.0);
  marginal.setLocation(mu_0);
  marginal.setPrecision((tau_0 / (tau_0 + 1.0)) *
                        (alpha_0 - (real)n_dim + 1.0) * T_0.Inverse());
  n = 0;
  sum = 0.0;
  tau_n = tau_0;
  mu_n = mu_0;
...
*/
real MultivariateNormalUnknownMeanPrecision::marginal_pdf(
 ...
 */
void MultivariateNormalUnknownMeanPrecision::calculatePosterior(
    const Vector& x) {
  n++;
  ...
  // printf ("# M: %f %f %f %f %f\n", tau_n, alpha_n, InvT(0,0), M_2n(0,0),
  // mu_n(0));
  ...
  marginal.setPrecision((tau_n / (tau_n + 1.0)) *
                        (alpha_n - (real)n_dim + 1.0) * InvT);
...
void MultivariateNormalUnknownMeanPrecision::Show() {
  ...
  printf("# Conditional normal mean: ");
  mu_n.print(stdout);
