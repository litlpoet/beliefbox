...
class SparseGaussianProcess {
 ...
 public:
  SparseGaussianProcess(real noise_variance_, Vector scale_length_,
                        real sig_var_, real threshold_,
                        Vector scale_length_dic_);
  SparseGaussianProcess(Matrix& X_, Vector& Y_, real noise_variance_,
                        real scale_length_, real sig_var_, real threshold_,
                        Vector scale_length_dic_);
  ...
  virtual void Observe(const std::vector<Vector>& x,
                       const std::vector<real>& y);
  virtual void AddObservation(const Vector& x, const real& y);
  virtual void AddObservation(const std::vector<Vector>& x,
                              const std::vector<real>& y);
