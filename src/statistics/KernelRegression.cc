...
/// Constructor
KernelRegression::KernelRegression(int n_dimensions_x, int n_dimensions_y,
...
Vector KernelRegression::expected_value(const Vector& x) {
  ...
  if (nearest_neighbour_size == 0) {
    ...
    for (std::list<std::pair<real, KDNode*> >::iterator it =
             node_list.S.begin();
         it != node_list.S.end(); ++it) {
      KDNode* node = it->second;
      PointPair* p = kd_tree.getObject(node);
      real d = SquareNorm(&x, &(p->x));
...
void KernelRegression::BootstrapBandwidth() {
  ...
  for (std::list<PointPair>::iterator it = points.begin(); it != points.end();
       ++it) {
    if (urandom() < 0.3) {
