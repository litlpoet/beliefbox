...
ContextTreeKDTree::Node::Node(ContextTreeKDTree& tree_,
    ...
    : tree(tree_),
      ...
      gaussian((upper_bound_ + lower_bound_) * 0.5, 1.0, 1.0,
...
#else
  mid_point =
      (upper_bound[splitting_dimension] + lower_bound[splitting_dimension]) /
      2.0;
#endif
}
...
*/
real ContextTreeKDTree::Node::Observe(const Vector& x, real probability) {
...
*/
real ContextTreeKDTree::Node::pdf(const Vector& x, real probability) {
  int k;
  if (x[splitting_dimension] < mid_point) {
    k = 0;
  } else {
...
ContextTreeKDTree::ContextTreeKDTree(int n_branches_, int max_depth_,
...
ContextTreeKDTree::~ContextTreeKDTree() { delete root; }

real ContextTreeKDTree::Observe(const Vector& x) { return root->Observe(x, 1); }

real ContextTreeKDTree::pdf(const Vector& x) { return root->pdf(x, 1); }
...
int ContextTreeKDTree::NChildren() { return root->NChildren(); }
