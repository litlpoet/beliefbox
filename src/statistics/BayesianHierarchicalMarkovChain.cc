...
BayesianHierarchicalMarkovChain::BayesianHierarchicalMarkovChain(int n_states,
                                                                 int n_models,
                                                                 float prior,
                                                                 bool dense)
...
void BayesianHierarchicalMarkovChain::ObserveNextState(int state) {
  ...
  for (int model = 0; model <= top_model; ++model) {
    real log_posterior =
        log_weight[model] + log(P_obs(model, state)) - log(Lkoi(model, state));
...
int BayesianHierarchicalMarkovChain::predict() { return prediction; }
