...
#ifndef SRC_STATISTICS_DISTRIBUTION_H_
#define SRC_STATISTICS_DISTRIBUTION_H_
...
class AbstractDistribution : public Object {
 public:
  ...
  virtual real log_pdf(const T& x) const { return log(pdf(x)); }
};
...
class Distribution : public Object {
 public:
  ...
  virtual real log_pdf(real x) const { return log(pdf(x)); }
  ...
  ///< set the mean
  virtual void setMean(real mean) { Serror("Cannot set mean\n"); }
};
...
class ParametricDistribution : public Distribution {
 public:
  ...
  ///< set the variance
  virtual void setVariance(real var) { Serror("Cannot set variance\n"); }
};

/// A Conjugate prior is special in the sense that a posterior can be calculated
/// analytically.
...
class BernoulliDistribution : public ParametricDistribution {
 public:
  ...
  explicit BernoulliDistribution(real p);
...
class UniformDistribution : public ParametricDistribution {
 public:
  ...
  /// Create a uniform distribution with mean \c mean and standard deviation \c
  /// std
  ...
  virtual void setVariance(real var) { range = sqrt(12.0f * var); }

  virtual void setMean(real mean) { min = mean - 0.5 * range; }

  virtual real getMean() const { return (min + 0.5 * range); }
};
...
class LaplacianDistribution : public ParametricDistribution {
 public:
  ...
  explicit LaplacianDistribution(real lambda) {
    m = 0.0;
    l = lambda;
  }
  ...
  virtual void setVariance(real var) { l = sqrt(0.5f / var); }
...
#endif  // SRC_STATISTICS_DISTRIBUTION_H_
