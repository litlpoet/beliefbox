...
class GammaDistributionUnknownShapeScale : public ConjugatePrior {
 public:
  ...
  GammaDistributionUnknownShapeScale(real lambda_ = 1, real mu_ = 1,
                                     int nu_ = 1);  ///< constructor
  virtual ~GammaDistributionUnknownShapeScale();    ///< destructor
  virtual void calculatePosterior(
      real x);                   ///< calculate posterior from an observation
  virtual real Observe(real x);  ///< observe, return posterior..
  virtual Vector posterior_pdf(std::vector<real>& y, std::vector<real>& x,
                               int K) const;
