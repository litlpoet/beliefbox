...
DiscreteHiddenMarkovModelPF::DiscreteHiddenMarkovModelPF(real threshold,
                                                         real stationarity,
                                                         int n_states_,
                                                         int n_observations_,
                                                         int n_particles_)
  ...
  for (int i = 0; i < n_states; ++i) {
    ...
    state_prior[i]->Alpha(i) =
        threshold * stationarity * (real)(n_states - 1) / (1.0 - stationarity);
...
real DiscreteHiddenMarkovModelPF_ReplaceLowest::Observe(int x) {
  ...
  if (log_w[min_k] < replacement_threshold) {
    ...
    std::vector<MultinomialDistribution>& PX_k =
        hmm[k]->getObservationProbablities();
    std::vector<MultinomialDistribution>& PS_min =
        hmm[min_k]->getStateProbablities();
    std::vector<MultinomialDistribution>& PX_min =
        hmm[min_k]->getObservationProbablities();
    for (int i = 0; i < n_states; ++i) {
      for (int j = 0; j < n_states; ++j) {
...
void DiscreteHiddenMarkovModelRBPF::Reset() { t = 0; }
