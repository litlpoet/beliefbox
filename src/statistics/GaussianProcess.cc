...
GaussianProcess::GaussianProcess(Matrix& Sigma_p_, real noise_variance_)
    : Sigma_p(Sigma_p_),
...
GaussianProcess::GaussianProcess(real noise_variance_, Vector scale_length_,
                                 real sig_var_)
    : noise_variance(noise_variance_),
...
GaussianProcess::GaussianProcess(Matrix& X_, Vector& Y_, real noise_variance_,
                                 real scale_length_, real sig_var_)
    : X(X_),
      Y(Y_),
      noise_variance(noise_variance_),
      scale_length(scale_length_),
...
real GaussianProcess::pdf(Vector& x, real y) { return 0.0; }
...
/// The following function calculates the partial derivatives of the Covariance
/// function w.r.t hyperparameters
/// If input argument equal to 1, the derivative of the scale_lenght
/// 'hyperparameter' is calculated.
/// Otherwise, the derivative of the signal variance 'hyperparameter' is
/// calculated.
