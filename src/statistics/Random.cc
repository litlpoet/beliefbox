...
unsigned long lrandom() { return MersenneTwister::random(); }

real urandom2() { return MersenneTwister::uniform(); }
...
real urandom(real min, real max) { return min + ((max - min) * urandom()); }

int urandom(int min, int max) { return floor(min + ((max - min) * urandom())); }
...
real true_random(bool blocking) {
  ...
  if (rand_device) {
    ...
    fprintf(
        stderr,
        "Warning: true random device not available.  Using random() instead\n");
    warned = true;
  }
