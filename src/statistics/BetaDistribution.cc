...
real BetaDistribution::pdf(real x) const {
  ...
  if (x == 0 && alpha == 1 && beta > 0) {
    ...
    log_pdf = log(x) * (alpha - 1.0) + log(1 - x) * (beta - 1.0) -
              logBeta(alpha, beta);
  }
  return exp(log_pdf);
}
...
real BetaDistribution::log_pdf(real x) const {
  ...
  return log(x) * (alpha - 1.0) + log(1 - x) * (beta - 1.0) -
         logBeta(alpha, beta);
}
...
real BetaDistribution::getMean() const { return alpha / (alpha + beta); }
...
real BetaDistribution::setMaximumLikelihoodParameters(
  ...
  // max_log_likelihood, n_iterations);
