...
ContextTreeRealLine::Node::Node(real lower_bound_, real upper_bound_,
                                int n_branches_, int max_depth_)
...
ContextTreeRealLine::Node::Node(ContextTreeRealLine::Node* prev_,
                                real lower_bound_, real upper_bound_)
    : lower_bound(lower_bound_),
      upper_bound(upper_bound_),
...
// log_w_prior( - log(10))
{
  // DISABLED_ASSERT(lower_bound <
  // upper_bound)(lower_bound)(upper_bound)(depth);
...
/** Observe new data, adapt parameters.
    ...
    \f[
    P(X | C_k) = U(C_k) w_k + (1 - w_k) [a_{k,0} / n_k P(X | C_{k,0}) + a_{k,1}
   / n_k P(X | C_{k,1})]
    \f]
    ...
    Or in long hand
    \f[
    P(X | C) = P(U | C) P(X | U, C) + [1 - P(U | C)] \sum_{C'} P(X | C') P(C' |
   C, \not U)
    \f]
    That means that
    \f[
...
*/
real ContextTreeRealLine::Node::Observe(real x, real probability) {
...
*/
real ContextTreeRealLine::Node::pdf(real x, real probability) {
  int k;
  if (x < new_bound) {
    k = 0;
  } else {
...
void ContextTreeRealLine::Node::Show() {
  std::cout << S << " " << w << " " << depth << "[ " << lower_bound << ", "
            << upper_bound << " ] # obs weight depth\n";
...
ContextTreeRealLine::ContextTreeRealLine(int n_branches_, int max_depth_,
    ...
    : n_branches(n_branches_), max_depth(max_depth_) {
  root = new Node(lower_bound, upper_bound, n_branches, max_depth);
}

ContextTreeRealLine::~ContextTreeRealLine() { delete root; }

real ContextTreeRealLine::Observe(real x) { return root->Observe(x, 1); }

real ContextTreeRealLine::pdf(real x) { return root->pdf(x, 1); }
...
int ContextTreeRealLine::NChildren() { return root->NChildren(); }
