...
DiscretePolyaTree::DiscretePolyaTree(DiscreteVector values_) : values(values_) {
  // set up the parameters recursively
  root = new PolyaNode(0, values);
}

DiscretePolyaTree::~DiscretePolyaTree() { delete root; }

void DiscretePolyaTree::Observe(std::vector<int>& x) { root->Observe(x); }
