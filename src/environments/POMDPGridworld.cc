...
POMDPGridworld::POMDPGridworld(RandomNumberGenerator* rng_, const char* fname,
                               uint n_obs_, real random_, real pit_, real goal_,
                               real step_)
    : rng(rng_),
      observation(0),
      total_time(0),
  ...
  printf(
      "# Making POMDPGridworld %d x %d from %s with %d states, %d obs, %f "
      "randomness, %f pit, %f goal, %f step rewards\n",
      height, width, fname, n_states, n_obs, random, pit_value, goal_value,
      step_value);
  ...
  while (getline(ifs, line)) {  // && y<height) {
    if (line.length() != width) {
      Serror("Line length (%ld) does not match width (%d)", line.length(),
             width);
      exit(-1);
    }
    ...
    for (uint x = 0; x < width; ++x) {
      switch (line[x]) {
        ...
        case 'O':
          ...
          break;
        default:
          std::cerr << "Unknown maze element: '" << line[x] << "'" << std::endl;
          exit(-1);
      }
    }
    y++;
  ...
  if (y < height) {
    std::cerr << "Only " << y << " lines read while accessing file " << fname
              << std::endl;
    exit(-1);
  } else if (y > height) {
...
void POMDPGridworld::Reset() {
  ...
  do {
    ...
    x = state % height;
    y = (state - x) / width;
  } while (whatIs(x, y) != GRID || whatIs(x, y) == INVALID ||
           whatIs(x, y) == WALL);
