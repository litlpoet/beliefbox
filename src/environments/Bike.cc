...
#define sqr(x) ((x) * (x))
Bike::Parameters
    Bike::default_parameters =
        {
            -1.0,      0.0,  +0.01, 0.0001, 20,   0.01, (10.0 / 3.6), 9.82, 0.3,
            0.66,      0.94, 15.0,  1.7,    60.0, 75.0, 0.34, /* tyre radius */
            1.11, /* distance between the point where the front and back tyre
                     touch the ground */
            3.1415927, 1000, 0,     10};
...
Bike::Bike(bool random_parameters)
  ...
  I_bike = ((13.0 / 3) * parameters.Mc * parameters.h * parameters.h +
            parameters.Mp * (parameters.h + parameters.dCM) *
                (parameters.h + parameters.dCM));
...
void Bike::Simulate(const int action) {
  ...
  if (state[0] == 0.0) {
    ...
    rCM = sqrt(pow(parameters.l - parameters.c, 2.0) +
               parameters.l * parameters.l / (pow(tan(state[0]), 2.0)));
  ...
  state[4] =
      (parameters.h * parameters.M * parameters.g * sin(phi) -
       cos(phi) * (I_dc * sigma_dot * state[1] +
                   sign(state[0]) * parameters.v * parameters.v *
                       (parameters.Md * parameters.R * (1.0 / rf + 1.0 / rb) +
                        parameters.M * parameters.h / rCM))) /
      I_bike;
  ...
  if (temp > 1)
    temp = sign(psi + state[0]) * parameters.pi / 2.0;
  else
    temp = sign(psi + state[0]) * asin(temp);
  ...
  if (temp > 1)
    temp = sign(psi) * parameters.pi / 2.0;
  else
  ...
  if ((xf == xb) && (temp < 0.0))
    ...
    if (temp > 0.0)
      ...
      psi = sign(xb - xf) * (parameters.pi / 2.0) - atan(temp / (xb - xf));
  }
  ...
  if (fabs(state[2]) > (parameters.pi / 15.0)) { /* the bike has fallen over */
    ...
    if (temp < 1e-3)
      reward = parameters.R3;
    else
      reward = (0.95 - sqr(state[5])) * parameters.R_FACTOR;
    endsim = false;
  }
...
real Bike::calc_dist_to_goal(real xf, real xb, real yf, real yb) {
  ...
  temp = sqrt(
      std::max(0.0, (parameters.x_goal - xf) * (parameters.x_goal - xf) +
                        (parameters.y_goal - yf) * (parameters.y_goal - yf) -
...
real Bike::calc_angle_to_goal(real xf, real xb, real yf, real yb) {
  ...
  temp = (xf - xb) * (parameters.x_goal - xf) +
         (yf - yb) * (parameters.y_goal - yf);
  //	printf("temp = %f\n",temp);
  scalar = temp / (parameters.l * sqrt(sqr(parameters.x_goal - xf) +
                                       sqr(parameters.y_goal - yf)));
  //	printf("scalar = %f\n",scalar);
  tvaer = (-yf + yb) * (parameters.x_goal - xf) +
          (xf - xb) * (parameters.y_goal - yf);
  //	printf("tvaer = %f\n",tvaer);
  if (tvaer <= 0.0)
    temp = scalar - 1.0;
  else
    temp = fabs(scalar - 1.0);
...
real Bike::calc_angle_to_goal_1(real xf, real xb, real yf, real yb) {
  real temp = (xf - xb) * (parameters.x_goal - xf) +
              (yf - yb) * (parameters.y_goal - yf);
  real scalar = temp / (parameters.l * sqrt(sqr(parameters.x_goal - xf) +
                                            sqr(parameters.y_goal - yf)));
  return (scalar);
}
