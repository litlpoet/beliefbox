...
Gridworld::Gridworld(const char* fname, real random_, real pit_, real goal_,
                     real step_)
    : total_time(0),
      random(random_),
      pit_value(pit_),
  ...
  logmsg("Gridworld, states: %d, random: %f, pit: %f, goal: %f, step: %f\n",
         n_states, random, pit_value, goal_value, step_value);
...
bool Gridworld::Act(const int& action) {
  ...
  assert((state == (int)terminal_state) ||
         (whatIs(x, y) != INVALID && whatIs(x, y) != WALL));
