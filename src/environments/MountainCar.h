...
class MountainCar : public Environment<Vector, int> {
 ...
 public:
  ...
  virtual const char* Name() const { return "Mountain Car"; }

  void Show() {
    printf("%f %f %f %f %f %f %f # params (MountainCar)\n", parameters.U_POS,
           parameters.L_POS, parameters.U_VEL, parameters.L_VEL,
           parameters.INPUT, parameters.GRAVITY, parameters.MCNOISE);
  }
...
class MountainCarGenerator {
 public:
  MountainCar Generate(bool random = true) { return MountainCar(random); }
};
