...
/** Random MDP
    ...
    You can set the reward for each type of state, but they are always
   deterministic.
...
class RandomMDP : public DiscreteEnvironment {
 ...
 public:
  RandomMDP(uint n_actions, uint n_states, real randomness, real step_value,
            real pit_value, real goal_value, RandomNumberGenerator* rng,
  ...
  virtual DiscreteMDP* getMDP() const { return new DiscreteMDP(*mdp); }
  ...
  void AperiodicityTransform(real tau) { mdp->AperiodicityTransform(tau); }
