...
Pendulum::Pendulum(bool random_parameters)
  ...
  if (random_parameters) {
    ...
    parameters.pendulum_mass =
        (0.5 + urandom()) * default_parameters.pendulum_mass;
    parameters.cart_mass = (0.5 + urandom()) * default_parameters.cart_mass;
    parameters.pendulum_length =
        (0.5 + urandom()) * default_parameters.pendulum_length;
...
void Pendulum::penddot(Vector& xdot, real u, Vector& x) {
  ...
  xdot[0] = x[1];
  xdot[1] =
      (parameters.gravity * sin(x[0]) -
       0.5 * CCa * parameters.pendulum_mass * parameters.pendulum_length *
           dtheta2 * sin(2.0 * x[0]) -
       CCa * cos(x[0]) * u) /
      (4.0 / 3.0 * parameters.pendulum_length -
       CCa * parameters.pendulum_mass * parameters.pendulum_length * cx * cx);
}
