...
class POMDPGridworld : public Environment<int, int> {
 public:
  ...
  int total_time;
  int n_obs;
  enum MapDirection { NORTH = 0, SOUTH, EAST, WEST };
  enum MapElement { INVALID = -1, GRID, WALL, GOAL, PIT };
  int terminal_state;
  POMDPGridworld(RandomNumberGenerator* rng_, const char* fname,
                 uint n_obs_ = 32, real random_ = 0.0, real pit_ = -1.0,
                 real goal_ = 0.0, real step_ = -0.1);
  ...
  virtual DiscreteMDP* getMDP() const { return NULL; }
  ...
  int getObservation() { return observation; }
  ...
  int getNObs() const { return n_obs; }

  uint getWidth() const { return width; }

  uint getHeight() const { return height; }
  virtual const char* Name() { return "POMDPGridworld"; }
