...
class Corridor {
 ...
 public:
  Corridor(int n_states_, real randomness_, RandomNumberGenerator* rng_)
      : n_states(n_states_), randomness(randomness_), rng(rng_) {
  ...
  ~Corridor() { delete pomdp; }
  void Reset() {
    state = 0;
    observation = 0;
  }
  DiscretePOMDP* getPOMDP() { return pomdp; }
  int getObservation() { return observation; }
  real getReward() { return reward; }
  int getNStates() { return n_states; }
  int getNActions() { return 2; }
  int getNObservations() { return 2; }
