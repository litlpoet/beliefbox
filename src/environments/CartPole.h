...
class CartPole : public Environment<Vector, int> {
 ...
 public:
  ...
  virtual const char* Name() const { return "Cart Pole RL"; }
  ...
  void Show() {
    printf("%f %f %f %f %f %f # params (CartPole)\n", parameters.GRAVITY,
           parameters.MASSCART, parameters.MASSPOLE, parameters.LENGTH,
           parameters.FORCE_MAG, parameters.TAU);
  }
};

class CartPoleGenerator {
 public:
  CartPole Generate(bool random = true) { return CartPole(random); }
};
...
class HeuristicCartPolePolicy : public AbstractPolicy<Vector, int> {
 ...
 public:
  HeuristicCartPolePolicy() : n_actions(2), state(2) {}
