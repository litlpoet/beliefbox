...
class PuddleWorld : public Environment<Vector, int> {
 ...
 public:
  ...
  Vector& StateActionUpperBound() { return state_action_upper_bound; }
  Vector& StateActionLowerBound() { return state_action_lower_bound; }
  Vector& ActionUpperBound() { return action_upper_bound; }
  Vector& ActionLowerBound() { return action_lower_bound; }
  ...
  virtual real getTransitionProbability(const Vector& state, const int& action,
    ...
    switch (action) {
      case 0:
        input[0] = parameters.AGENTSPEED;
        break;
      case 1:
        input[0] = -parameters.AGENTSPEED;
        break;
      case 2:
        input[1] = parameters.AGENTSPEED;
        break;
      case 3:
        input[1] = -parameters.AGENTSPEED;
        break;
      default:
        Serror("Undefined action %d\n", action);
    }
  ...
  virtual const char* Name() const { return "Puddle World"; }

  void Show() const {
    printf(
        "%f %f %f %f | %d | %f %f %f %f | %f %f %f %f | %f %f | %f %f # params "
        "Puddle\n",
        parameters.U_POS_X, parameters.L_POS_X, parameters.U_POS_Y,
        parameters.L_POS_Y, parameters.NUMPUDDLES, parameters.U_POS_P(0, 0),
        parameters.U_POS_P(0, 1), parameters.U_POS_P(1, 0),
        parameters.U_POS_P(1, 1), parameters.L_POS_P(0, 0),
        parameters.L_POS_P(0, 1), parameters.L_POS_P(1, 0),
        parameters.L_POS_P(1, 1), parameters.RADIUSPUDDLES[0],
        parameters.RADIUSPUDDLES[1], parameters.AGENTSPEED, parameters.MCNOISE);
  }
};

class PuddleWorldGenerator {
 public:
  PuddleWorld Generate(bool random = true) { return PuddleWorld(random); }
};
