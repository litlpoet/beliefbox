...
class Pendulum : public Environment<Vector, int> {
 ...
 public:
  ...
  virtual void setRandomness(real w) { parameters.max_noise = w; }
  const Vector& StateUpperBound() const { return state_upper_bound; }
  const Vector& StateLowerBound() const { return state_lower_bound; }
  ...
  virtual const char* Name() const { return "Pendulum"; }
  void Show() {
    printf("%f %f %f %f %f %f # params (Pendulum)\n", parameters.pendulum_mass,
           parameters.cart_mass, parameters.pendulum_length, parameters.gravity,
           parameters.max_noise, parameters.Dt);
  }
};

class PendulumGenerator {
 public:
  Pendulum Generate(bool random = true) { return Pendulum(random); }
};
...
class HeuristicPendulumPolicy : public AbstractPolicy<Vector, int> {
 ...
 public:
  HeuristicPendulumPolicy() : n_actions(2), state(2) {}
  ...
  virtual void Observe(real r, const Vector& next_state) { state = next_state; }
  virtual void Reset() {
    state(0) = 0;
    state(1) = 0;
  }
