...
Blackjack::Blackjack(RandomNumberGenerator* rng_, real win_value_,
                     real draw_value_, real loss_value_)
...
real Blackjack::calculReward() const {
  ...
  */
  ...
  while (dhv < 17) {
    ...
    dhv += j;
    if (j == 1 && dhv < 12) dhv += 10;
  }

  /// Dealer bust
  if (dhv > 21) return win_value;

  /// Tie

  if (dhv == pc) return draw_value;

  /// Player win
  if (dhv < pc) return win_value;

  /// Player loose
  if (dhv > pc) return loss_value;
...
DiscreteMDP* Blackjack::getMDP() const {
  ...
  for (uint i = 0; i < n_states; i++) {
    for (int k = 0; k < 2; k++) mdp->setFixedReward(i, k, 0);
  }
...
bool Blackjack::Act(const int& action) {
  ...
  /// Generating the resulting state corresponding to the effect of action on
  /// the current state
  ...
  /// Standing
  if (action == 1) state = terminal_state;

  /// If action is to stand or state is terminal_state computing the reward for
  /// the player
...
void Blackjack::Reset() {
  ...
  if (isU == 1 && state_ini == 21 && num == 2) isNatural = true;

  /// If the dealer first two cards contains an Ace then Add 10 to his
  /// second_value

  if ((first_value == 1) || (second_value == 1)) second_value += 10;
