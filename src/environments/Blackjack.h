 ...
 * isUsable: 1 if a value of 11 has been used in the play for the ace, 0
 *otherwise
...
class Blackjack : public Environment<int, int> {
 public:
  ...
  Blackjack(RandomNumberGenerator* rng_, real win_value_ = 1.0,
            real draw_value_ = 0.0, real loss_value_ = -1.0);
  ...
  int getState1() const { return state; }
  ...
  virtual const char* Name() { return "Blackjack"; }
  virtual ~Blackjack();
