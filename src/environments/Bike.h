...
class Bike : public Environment<Vector, int> {
 protected:
  struct Parameters {
    ...
    real M;
    real R;  /// tyre radious
    real l;  /// distance between the point where the front and back tyre touch
             /// the ground
    real pi;
    /// position of goal
    real x_goal;
 ...
 public:
  ...
  virtual const char* Name() const { return "Bike"; }
  void Show() {
    printf("%f %f %f %f %d %f # params (Bike)\n", parameters.R1, parameters.R2,
           parameters.R3, parameters.R_FACTOR, parameters.N0_STATES2,
           parameters.dt);
  }
...
class BikeGenerator {
 public:
  Bike Generate(bool random = true) { return Bike(random); }
};
