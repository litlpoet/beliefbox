...
class InventoryManagement : public DiscreteEnvironment {
 protected:
  ...
  real demand;  ///< This is the probability that an item will be bought at each
                ///step
  real margin;  ///< this is the ratio (greater than 1) of the retail value of
                ///the item versus the order cost
  // std::vector<SingularDistribution*> dist; ///< container for the reward
  // distributions
 ...
 public:
  ...
  virtual const char* Name() const { return "Inventory management"; }
};
...
class InventoryManagementGenerator : public EnvironmentGenerator<int, int> {
 ...
 public:
  InventoryManagementGenerator(int max_stock_, int period_)
      : max_stock(max_stock_), period(period_) {}
  InventoryManagement* Generate(bool random = true) {
    real demand = urandom();
    real margin = 1 + urandom();
    InventoryManagement* inventory_management =
        new InventoryManagement(period, max_stock, demand, margin);
