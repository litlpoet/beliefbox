...
class RepresentativeStateModel {
 protected:
  ...
  std::vector<Vector> f;  ///< number of times each representative state transit
  /// to a terminal state;
 ...
 public:
  ...
  RepresentativeStateModel(
      real gamma_, real threshold_, Model model_, const std::vector<S>& states_,
      const std::vector<Vector>& v_, const std::vector<Vector>& f_,
      uint n_actions_,
      const FittedValueIteration<S, A>* fitted_value_iteration_)
      : gamma(gamma_),
  ...
  RepresentativeStateModel(
      real gamma_, real threshold_, Model& model_, uint n_states,
      uint n_actions_,
  ...
  RepresentativeStateModel(real gamma_, real threshold_, Model model_,
                           Sampler& sampler, uint n_states, uint n_actions_,
  ...
  void BuildMDP() {
    ...
    for (int i = 0; i < n_states; ++i) {
      for (uint a = 0; a < n_actions; ++a) {
        Vector p(n_states);
        //        Vector p(n_states + 1);
        //        real p0 = (f[i][a] + alpha) / (v[i][a] + alpha
        //+ beta);
        // p0 = 0.0;
        // Set rewards
        ...
        //        logmsg ("s:%d a:%d r:(%f %f) ", i, a, r_ia,
        // model.getExpectedReward(states[i], a)); p.print(stdout);
  ...
  void UpdateStatistics(const int& state, const int& action,
                        const bool& endsim) {
    v[state][action]++;
    if (!endsim) f[state][action]++;
  }
  ...
  std::vector<S> getRepresentativesSamples() { return states; }

  int getNSamples() { return states.size(); }
  S getSample(int i) {
