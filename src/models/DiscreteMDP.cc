// -*- Mode: c++ -*-
// copyright (c) 2005-2013 by Christos Dimitrakakis
...
void DiscreteMDP::AperiodicityTransform(real tau) {
  ...
  for (int s = 0; s < n_states; s++) {
    for (int a = 0; a < n_actions; a++) {
      reward_distribution.setFixedReward(
          s, a, tau * reward_distribution.expected(s, a));
    }
  }
  for (int s = 0; s < n_states; s++) {
    for (int a = 0; a < n_actions; a++) {
...
void DiscreteMDP::dotModel(FILE* fout) const {
  const char* colour[] = {"red",     "green", "blue", "yellow",
                          "magenta", "cyan",  "black"};
  ...
  for (int s = 0; s < n_states; s++) {
    for (int a = 0; a < n_actions; a++) {
      ...
      for (int j = 0; j < n_states; j++) {
        real p = getTransitionProbability(s, a, j);
        if (p > 0.000001) {
          fprintf(fout, "s%d -> s%d [label = \" p=%.2f, r=%.1f\", color=%s];\n",
                  s, j, p, reward_distribution.expected(s, a),
                  colour[colour_id]);
        }
      }
    }
...
#ifdef NDEBUG
bool DiscreteMDP::Check() const { return true; }
#else
