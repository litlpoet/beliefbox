 ...
 */
KNNModel::KNNModel(int n_actions_, int n_dim_, real gamma_, bool optimistic,
                   real optimism_, real r_max_)
...
void KNNModel::AddSample(TrajectorySample sample, int K, real beta) {
  if (max_samples < 0 || samples.size() < (uint)max_samples) {
    OrderedFixedList<KDNode> node_list =
        kd_tree[sample.a]->FindKNearestNeighbours(sample.s, K);
...
/** Predict the next reward and a state
    ...
    \param alpha absolute-to-relative parameter. Set to 0 for absolute
   predictions, 1 for predictions relative to the current state.
...
*/
void KNNModel::GetExpectedTransition(real alpha, Vector& x, int action,
                                     real& reward, Vector& y, int K, real b) {
  RBF rbf(x, b);
  ...
  OrderedFixedList<KDNode> node_list =
      kd_tree[action]->FindKNearestNeighbours(x, K);
 ...
 */
void KNNModel::UpdateValue(TrajectorySample& start_sample, real alpha, int K,
                           real b) {
  ...
  for (int a = 0; a < n_actions; ++a) {
    OrderedFixedList<KDNode> node_list =
        kd_tree[a]->FindKNearestNeighbours(start_sample.s, K);
