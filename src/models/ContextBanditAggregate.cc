...
ContextBanditAggregate::ContextBanditAggregate(int n_aggregated_states,
                                               int n_states, int n_actions,
                                               real tau, real mu_0, real tau_0)
    : ContextBanditGaussian(n_states, n_actions, tau, mu_0, tau_0)

{
  mdp_dbg(
      "Creating ContextBanditAggregate from %d states to %d sets and %d "
      "actions\n",
...
/// This constructor makes a random set
ContextBanditAggregate::ContextBanditAggregate(bool blah, int n_factors,
                                               int n_values,
...
void ContextBanditAggregate::BuildFactoredAggregate(int n_factors,
                                                    int n_values) {
  for (int i = 0; i < n_aggregated_states; i++) {
    int s = i % n_states;
    X[s].add(i);
...
real ContextBanditAggregate::getTransitionProbability(int s, int a,
                                                      int s2) const {
...
void ContextBanditAggregate::Reset() { ContextBanditGaussian::Reset(); }
