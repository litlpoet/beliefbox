...
/** Context tree non-parametric conditional density estimation on \f$R^n \times
   R^m\f$.
    ...
    directly.
 */
class ConditionalKDGaussianClassifier {
 public:
  // public classes
  struct Node {
    ConditionalKDGaussianClassifier& tree;  ///< the tree
    ...
    MultivariateGaussianClassifier*
        local_probability;    ///< local probability estimator
    Node* prev;               ///< previous node
    std::vector<Node*> next;  ///< pointers to next nodes
    real w;                   ///< backoff weight
    real log_w;               ///< log of w

    Node(ConditionalKDGaussianClassifier& tree_, Vector& lower_bound_x_,
         Vector& upper_bound_x_);
    Node(Node* prev_, Vector& lower_bound_x, Vector& upper_bound_x);
    ~Node();
  ...
  // public methods
  ConditionalKDGaussianClassifier(int n_branches_, int max_depth_,
                                  Vector& lower_bound_x, Vector& upper_bound_x,
                                  int n_classes_);
  ~ConditionalKDGaussianClassifier();
