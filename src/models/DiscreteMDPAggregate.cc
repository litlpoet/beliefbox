...
DiscreteMDPAggregate::DiscreteMDPAggregate(int n_aggregated_states,
                                           int n_states, int n_actions,
                                           int init_transition_count)
    : DiscreteMDPCounts(n_states, n_actions, init_transition_count)

{
  mdp_dbg(
      "Creating DiscreteMDPAggregate from %d states to %d sets and %d "
      "actions\n",
...
DiscreteMDPAggregate::DiscreteMDPAggregate(Gridworld& gridworld,
                                           int n_aggregated_states,
                                           int n_states, int n_actions,
                                           int init_transition_count)
    : DiscreteMDPCounts(n_states, n_actions, init_transition_count) {
  mdp_dbg(
...
void DiscreteMDPAggregate::BuildGridworldAggregate(Gridworld& gridworld) {
  ...
  printf("divx: %d, divy: %d, remaining: %d/%d\n", n_divisions_x, n_divisions_y,
         remaining_states, n_states);
  for (int i = 0; i < n_aggregated_states; i++) {
    state_map[i] = -1;
  }
  ...
  for (int y = 0; y < n_divisions_y; ++y) {
    for (int x = 0; x < n_divisions_x; ++x) {
      for (int y_orig = y * y_dist; y_orig < std::min(height, (y + 1) * y_dist);
           ++y_orig) {
        for (int x_orig = x * x_dist;
             x_orig < std::min(width, (x + 1) * x_dist); ++x_orig) {
...
real DiscreteMDPAggregate::getTransitionProbability(int s, int a,
                                                    int s2) const {
  int x = Aggregate(s);
  int x2 = Aggregate(s2);
  int n = X[x2].size();
...
void DiscreteMDPAggregate::Reset() { DiscreteMDPCounts::Reset(); }
