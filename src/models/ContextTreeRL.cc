...
ContextTreeRL::Node::Node(int n_branches_, int n_outcomes_)
 ...
 */
real ContextTreeRL::Node::Observe(Ring<int>& history, Ring<int>::iterator x,
                                  int z, real r, real probability,
...
// Standard
  ...
  // P(z | B_k) = P(z | B_k, h_k) P(h_k | B_k) + (1 - P(h_k | B_k)) P(z |
  // B_{k-1})
  ...
  if (std::isnan(log_w)) {
    fprintf(stderr,
            "Warning: log_w at depth %d is nan! log_w=%f, w = %f, p = %f, p(x) "
            "= %f, p(z) = %f, p(r)=%f, prior = %f, z = %d, r = %f \n",
            depth, log_w, w, p_observations, P[z], p_reward, total_probability,
            log_w_prior, z, r);
    P.print(stderr);
    log_w = -2;
  }
  ...
  if (x != history.end() && S > threshold) {
    ...
    total_probability =
        next[k]->Observe(history, x, z, r, total_probability, active_contexts);
...
real ContextTreeRL::Node::QValue(Ring<int>& history, Ring<int>::iterator x,
                                 real Q_prev) {
  w = exp(log_w_prior + log_w);
...
ContextTreeRL::ContextTreeRL(int n_branches_, int n_observations_,
                             int n_actions_, int n_symbols_, int max_depth_)
    : n_branches(n_branches_),
  ...
  std::cout << "# Making new CTRL with depth: " << max_depth
            << " branches:" << n_branches << " actions:" << n_actions
            << " observations:" << n_observations << " symbols:" << n_symbols
            << std::endl;
}

ContextTreeRL::~ContextTreeRL() { delete root; }
...
int ContextTreeRL::NChildren() { return root->NChildren(); }
/** Q Learning implementation.


 */
real ContextTreeRL::QLearning(real step_size, real gamma, int observation,
                              real reward) {
  real Q_prev =
      root->QValue(history, history.begin(), 0);  ///< previous prediction
  ...
  for (std::list<Node*>::iterator i = active_contexts.begin();
       i != active_contexts.end(); ++i) {
    real p_i = (*i)->context_probability;
    // real dQ_i = reward + gamma * max_Q - (*i)->Q; ///< This works even
    // better!
 ...
 */
real ContextTreeRL::Sarsa(real step_size, real gamma, int observation,
                          real reward) {
