...
/** Create a counting model of an MDP.
   ...
   \arg init_transition_count the prior for the Dirichlet. The higher this is,
   the more the model will expect to see unseen transitions.
   \arg init_reward_count the prior number of counts for the reward.  This
   should be used in conjuction with the prior reward average to bias the
   rewards.
   \arg The prior reward average. This can be used to bias the average to some
   particular value.
 */
DiscreteMDPCounts::DiscreteMDPCounts(int n_states, int n_actions,
                                     real init_transition_count,
                                     RewardFamily reward_family_)
...
void DiscreteMDPCounts::ShowModel() const {
  ...
  for (int a = 0; a < n_actions; a++) {
    for (int i = 0; i < n_states; i++) {
      ...
      std::cout << " [" << transitions.getParameters(i, a).Sum() << "]\n";
    }
  }
