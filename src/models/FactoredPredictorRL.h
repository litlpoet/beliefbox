...
class FactoredPredictorRL {
 public:
  ...
  /// Observe the current action \c act and the next observation \c obs and
  /// reward \c r.
  virtual real Observe(const A& act, const X& obs, real r) = 0;
  /// Return the probability (density) of observing \c x aftering taking action
  /// \c act.
  ...
  virtual void Show() { printf("FactoredPredictorRL: Nothing to show\n"); }
};
...
class TFactoredPredictorRL : public FactoredPredictorRL<int, int> {
 ...
 public:
  ...
  virtual void Reset() { current_obs = 0; }
};

/** A factored predictor RL wrapper for continuous state and action problems. */
template <class T>
class ContinuousTFactoredPredictorRL
    : public FactoredPredictorRL<Vector, Vector> {
 protected:
 ...
 public:
  ContinuousTFactoredPredictorRL(const Vector& lower_bound_state_action,
                                 ...
                                 int context_depth, int prediction_depth)
      : n_contexts(lower_bound_state_action.Size()),
        n_obs(lower_bound_state.Size()),
        n_actions(n_contexts - n_obs),
        tree(2, context_depth, prediction_depth, lower_bound_state_action,
             upper_bound_state_action, lower_bound_state, upper_bound_state),
        current_obs(n_obs) {
...
template <class T>
class ContinuousStateTFactoredPredictorRL
    : public FactoredPredictorRL<Vector, int> {
 protected:
  int n_contexts;       ///< the number of distinct contexts
  int n_obs;            ///< the number of distinct observations
  int n_actions;        ///< the number of actions
 ...
 public:
  ...
  ContinuousStateTFactoredPredictorRL(int n_actions_,
                                      ...
                                      const Vector& upper_bound_state,
                                      int context_depth, int prediction_depth,
                                      real depth_factor, real weight_factor)
      : n_obs(lower_bound_state.Size()),
        n_actions(n_actions_),
        tree(n_actions, context_depth, prediction_depth, lower_bound_state,
             upper_bound_state, depth_factor, weight_factor),
        current_obs(n_obs),
        current_action(-1) {
