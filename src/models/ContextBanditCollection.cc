...
ContextBanditCollection::ContextBanditCollection(int n_aggregates, int n_states,
                                                 int n_actions, real tau,
                                                 real mu_0, real tau_0)
    : MDPModel(n_states, n_actions) {
  mdp_dbg(
      "Creating ContextBanditCollection of %d aggregates, from %d states and "
      "%d actions\n",
  ...
  for (int i = 0; i < n_aggregates; i++) {
    ...
    if (i == 0) {
      ...
      A[i] = new ContextBanditAggregate(false, 3, 2, n_states, n_sets,
                                        n_actions, tau, mu_0, tau_0);
    }
  }
}
...
real ContextBanditCollection::getTransitionProbability(int s, int a,
                                                       int s2) const {
