...
class ContinuousContextTreeRL {
 public:
  ...
  struct Node {
    ...
    Node(ContinuousContextTreeRL& tree_, Vector& lower_bound_x_,
         Vector& upper_bound_x_);
    Node(Node* prev_, Vector& lower_bound_x, Vector& upper_bound_x);
    ~Node();
    real Observe(Vector& x, Vector& y, real reward, real probability,
  ...
  // public methods
  ContinuousContextTreeRL(int n_branches_, int max_depth_, int max_depth_cond_,
  ...
  void Reset() { active_contexts.clear(); }
  void Show();
  int NChildren();
