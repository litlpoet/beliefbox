...
ContinuousContextTreeRL::Node::Node(ContinuousContextTreeRL& tree_,
...
#else
  mid_point = (upper_bound_x[splitting_dimension] +
               lower_bound_x[splitting_dimension]) /
              2.0;
#endif

  local_density = new ContextTreeKDTree(tree.n_branches, tree.max_depth_cond,
                                        tree.lower_bound_y, tree.upper_bound_y);
  ...
  normal_density = new MultivariateNormalUnknownMeanPrecision(
      mean, 1.0, 1.0, Matrix::Unity(y_dim, y_dim));
...
// log_w_prior( - log(10))
{
  ...
  int y_dim = tree.upper_bound_y.Size();
  normal_density = new MultivariateNormalUnknownMeanPrecision(
      (tree.upper_bound_y + tree.lower_bound_y) * 0.5, 1.0, 1.0,
      Matrix::Unity(y_dim, y_dim));
  log_prior_normal = log(DEFAULT_PRIOR_NORMAL);
  prior_normal = DEFAULT_PRIOR_NORMAL;
}
...
*/
real ContinuousContextTreeRL::Node::Observe(Vector& x, Vector& y, real reward,
                                            real probability,
  ...
  if ((tree.max_depth == 0 || depth < tree.max_depth) && S > threshold) {
    ...
    total_probability =
        next[k]->Observe(x, y, reward, total_probability, active_contexts);
...
real ContinuousContextTreeRL::Node::QValue(Vector& x, real Q_prev) {
  w = exp(log_w_prior + log_w);
 ...
 */
real ContinuousContextTreeRL::QLearning(real step_size, real gamma, Vector& y,
                                        real reward) {
...
ContinuousContextTreeRL::ContinuousContextTreeRL(
    int n_branches_, int max_depth_, int max_depth_cond_, Vector& lower_bound_x,
    Vector& upper_bound_x, Vector& lower_bound_y_, Vector& upper_bound_y_)
...
ContinuousContextTreeRL::~ContinuousContextTreeRL() { delete root; }
...
int ContinuousContextTreeRL::NChildren() { return root->NChildren(); }
