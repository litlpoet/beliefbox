...
class TransitionDistribution {
 public:
  ...
  virtual real pdf(StateType state, ActionType action,
...
class Transition {
 public:
  ...
  Transition(StateType s, ActionType a, StateType s2)
...
class TransitionDistribution<int, int> {
 public:
  ...
  int GetNStates() const { return n_states; }

  int GetNActions() const { return n_actions; }

  /// Set a state transition
  virtual void SetTransition(int state, int action, int next_state,
                             real probability);
  ...
  /// upon reaching this state-action pair.
