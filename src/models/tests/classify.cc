...
int main(int argc, char** argv) {
  ...
  SparseLinearClassifier sparse_linear_classifier(n_inputs, n_classes,
                                                  projection_size);
  sparse_linear_classifier.setStepSize(step_size);
  // LinearClassifierMixture classifier(n_inputs, n_classes, n_classifiers);
  // HashedLinearClassifierMixture linear_classifier_mixture(n_inputs,
  // n_classes, n_classifiers, projection_size);
  std::vector<KNNClassifier*> experts(n_classifiers);
  for (int i = 0; i < n_classifiers; ++i) {
    experts[i] = new KNNClassifier(n_inputs, n_classes, n_neighbours);
  }
  HashedClassifierMixture<KNNClassifier> hashed_classifier_mixture(
      n_inputs, n_classes, experts);
  ...
  ConditionalKDGaussianClassifier tree_gaussian(2, tree_depth, lower_bound,
                                                upper_bound, n_classes);
  ConditionalKDNNClassifier tree_knn(2, tree_depth, lower_bound, upper_bound,
                                     n_classes);
