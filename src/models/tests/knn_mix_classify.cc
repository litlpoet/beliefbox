...
int main(int argc, char** argv) {
  ...
  if (argc < 4) {
    fprintf(stderr,
            "Argumens: training_data n_neighbours n_classifiers [test_data]\n");
    exit(-1);
  }
  ...
  int n_classifiers = atoi(argv[3]);

  bool test = false;
  if (argc == 5) {
    // fprintf(stderr, "Reading %s\n", argv[3]);
    ReadClassData(test_data, test_labels, argv[4]);
    test = true;
  }
  int T = data.Rows();
  ...
  std::vector<KNNClassifier*> classifiers(n_classifiers);
  for (int i = 0; i < n_classifiers; ++i) {
    classifiers[i] = new KNNClassifier(n_classes, n_inputs, n_neighbours);
  }
  HashedClassifierMixture<KNNClassifier> classifier(n_inputs, n_classes,
                                                    classifiers);

  printf("# K: %d, T: %d, d: %d inputs, n: %d classes, N: %d classifiers\n",
         n_neighbours, T, n_inputs, n_classes, n_classifiers);
