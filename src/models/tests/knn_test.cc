...
bool knn_test(int n_neighbours, real rbf_beta) {
  ...
  for (real t = 0; t <= T; t += dt) {
    ...
    model.GetExpectedTransition(alpha, s, 0, predicted_reward, predicted_state,
...
bool knn_environment_test(Environment<Vector, int>& environment, real alpha,
                          int n_neighbours, real rbf_beta, real gamma, int T,
                          int max_samples) {
  ...
  for (int t = 0; t < T; ++t) {
    ...
    // predict next state
    model.GetExpectedTransition(alpha, state, action, predicted_reward,
                                predicted_state, n_neighbours, rbf_beta);
    ...
    if (!action_ok) {
      for (int a = 0; a <= n_actions; ++a) {
        TrajectorySample sample(environment.getState(), action, reward,
                                environment.getState(), true);
      }
      model.AddSample(sample, n_neighbours, rbf_beta);
  ...
  if (n_dim == 1) {
    ...
    for (real x = x_min; x < x_max; x += dx) {
      ...
      for (int a = 0; a < n_actions; ++a) {
        // model.GetExpectedTransition(alpha, state, action, predicted_reward,
        // predicted_state, n_neighbours, rbf_beta);
        model.GetExpectedTransition(alpha, state, a, next_reward, next_state,
                                    n_neighbours, rbf_beta);
    ...
    for (real x = x_min; x < x_max; x += dx) {
      for (real y = y_min; y < y_max; y += dy) {
        ...
        for (int a = 0; a < n_actions; ++a) {
          real V =
              model.GetExpectedActionValue(state, a, n_neighbours, rbf_beta);
          printf("%f ", V);
        }
        printf("# Q\n");
      }
...
int main(int argc, char** argv) {
  ...
  if (argc != 8) {
    fprintf(stderr,
            "usage: knn_test task alpha n_neighbours rbf_beta gamma horizon "
            "n_samples\n");
    fprintf(stderr,
            "task in {chain, pendulum, car}\n alpha in [0,1]\n n_neighbours in "
            "{1, 2, ...}\n rbf_beta > 0\n");
    exit(-1);
  }
  ...
  if (!strcmp(task, "car")) {
    success = knn_environment_test(mountain_car, alpha, n_neighbours, rbf_beta,
                                   gamma, horizon, n_samples);
  } else if (!strcmp(task, "pendulum")) {
    success = knn_environment_test(pendulum, alpha, n_neighbours, rbf_beta,
                                   gamma, horizon, n_samples);
  } else if (!strcmp(task, "chain")) {
    success = knn_environment_test(continuous_chain, alpha, n_neighbours,
                                   rbf_beta, gamma, horizon, n_samples);
  }
