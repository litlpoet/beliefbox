...
bool EvaluateMaze(std::string maze, int n_obs, real maze_random,
                  real action_random, RandomNumberGenerator* environment_rng,
                  FactoredPredictorRL<int, int>* factored_predictor,
                  Statistics& statistics);

bool Evaluate1DWorld(Corridor& environment, real action_randomness,
                     FactoredPredictorRL* factored_predictor,
                     Statistics& statistics);

bool EvaluateGeneral(Environment<int, int>* environment, real action_randomness,
                     FactoredPredictorRL<int, int>* factored_predictor,
                     Statistics& statistics);

enum EnvironmentType { ONEDMAZE, GRIDWORLD, MOUNTAIN_CAR, PENDULUM };
...
int main(int argc, char** argv) {
  ...
  if (argc <= 8) {
    fprintf(
        stderr,
        "Usage: factored_model environment model_name n_iter T depth env_rand act_rand [environment parameters]\n\
  ...
  for (int iter = 0; iter < n_iter; ++iter) {
    ...
    if (environment_type == MOUNTAIN_CAR) {
      ...
      mountain_car = new DiscretisedEnvironment<MountainCar>(
          continuous_mountain_car, n_intervals);
    ...
    if (!model_name.compare("BVMM")) {
      factored_predictor = new TFactoredPredictorRL<ContextTreeRL>(
          n_actions, n_obs, max_depth + 1);
    } else {
    ...
    switch (environment_type) {
      case ONEDMAZE: {
        ...
        //                    factored_predictor = new
        //                    POMDPBeliefPredictor(environment.getPOMDP());
        //}
        success = Evaluate1DWorld(environment, action_random,
      ...
      case GRIDWORLD: {
        ...
        success =
            EvaluateMaze(maze, n_obs, maze_random, action_random,
                         &mersenne_twister, factored_predictor, statistics);
      } break;
      case MOUNTAIN_CAR: {
        success = EvaluateGeneral(mountain_car, action_random,
                                  factored_predictor, statistics);
      } break;
      case PENDULUM: {
        success = EvaluateGeneral(pendulum, action_random, factored_predictor,
                                  statistics);
      } break;
      default:
  ...
  for (int t = 0; t < T; ++t) {
    ...
    printf("%f %f %f\n", statistics.probability[t] * inv_iter,
           statistics.reward[t] * inv_iter, statistics.error[t] * inv_iter);
  }
  return 0;
}

bool EvaluateMaze(std::string maze, int n_obs, real world_randomness,
                  real action_randomness,
                  RandomNumberGenerator* environment_rng,
                  FactoredPredictorRL* factored_predictor,
                  Statistics& statistics) {
  POMDPGridworld environment(environment_rng, maze.c_str(), n_obs,
                             world_randomness, -1.0, 1, -0.1);
...
bool Evaluate1DWorld(Corridor& environment, real action_randomness,
  ...
  printf("# states: %d, actions: %d, obs: %d, T: %d, a_rand: %f\n", n_states,
         n_actions, n_obs, T, action_randomness);
