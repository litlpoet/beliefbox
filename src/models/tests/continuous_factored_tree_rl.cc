...
enum LearningAlgorithm { SARSA, QLEARNING };
...
enum EnvironmentType { MOUNTAIN_CAR, MOUNTAIN_CAR_3D, PENDULUM, LINEAR_BANDIT };
...
int main(int argc, char** argv) {
  ...
  if (argc <= 9) {
    fprintf(
        stderr,
        "Usage: factored_model environment model_name n_iter T depth env_rand act_rand weight_factor depth_factor[environment parameters]\n\
  ...
  for (int iter = 0; iter < n_iter; ++iter) {
    ...
    factored_predictor =
        new ContinuousStateTFactoredPredictorRL<ContinuousStateContextTreeRL>(
            n_actions, L_S, U_S, max_depth, max_depth, depth_factor,
            weight_factor);
    ...
    switch (environment_type) {
      case MOUNTAIN_CAR: {
        success = EvaluateGeneral(
            mountain_car, action_random, factored_predictor, discount_factor,
            episode_statistics[iter], statistics, learning_algorithm);
      } break;
      case MOUNTAIN_CAR_3D: {
        success = EvaluateGeneral(
            mountain_car_3d, action_random, factored_predictor, discount_factor,
            episode_statistics[iter], statistics, learning_algorithm);
      } break;
      case PENDULUM: {
        success = EvaluateGeneral(pendulum, action_random, factored_predictor,
                                  discount_factor, episode_statistics[iter],
                                  statistics, learning_algorithm);
      } break;
      case LINEAR_BANDIT: {
        success = EvaluateGeneral(bandit, action_random, factored_predictor,
                                  discount_factor, episode_statistics[iter],
                                  statistics, learning_algorithm);
      } break;
      default:
  ...
  for (int iter = 0; iter < n_iter; ++iter) {
    ...
    for (int k = 0; k < episode_statistics[iter].n_episodes; ++k) {
      printf("k: %d, iter:%d, ep:%d\n", k, iter,
             episode_statistics[iter].n_episodes);
  ...
  for (int k = 0; k < max_episodes; ++k) {
    ...
    printf("%f %f %f %d #EPISODE\n", total_reward * icnt, utility * icnt,
           steps * icnt, cnt);
  }
