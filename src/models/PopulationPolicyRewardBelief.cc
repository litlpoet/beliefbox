...
PopulationPolicyRewardBelief::PopulationPolicyRewardBelief(
    real eta_, real gamma_, const DiscreteMDP& mdp_)
    : mdp(mdp_),
...
real PopulationPolicyRewardBelief::logLikelihood(
...
FixedDiscretePolicy PopulationPolicyRewardBelief::samplePolicy(Matrix& R,
                                                               real beta) {
  ...
  value_iteration.ComputeStateActionValues(
      epsilon, (int)ceil(log(epsilon * (1 - gamma)) / log(gamma)));
...
/// Monte-Carlo sampler
void PopulationPolicyRewardBelief::MonteCarloSampler(
    Demonstrations<int, int>& D, int n_iterations) {
