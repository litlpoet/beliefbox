...
class FiniteDiscreteRewardBelief : public DiscreteRewardBelief {
 protected:
  std::vector<const DiscreteSpaceRewardDistribution*>&
      rewards;  ///< reward distributions
  Vector
      probabilities;  ///< vector of probabilites for each reward distribution
 public:
  FiniteDiscreteRewardBelief(
      int n_states_, int n_actions_,
  ...
  virtual real Update(int state, int action, real reward) { return 0; }
};
...
class DirichletRewardBelief : public DiscreteRewardBelief {
 ...
 public:
  ...
  DirichletRewardBelief(int n_states_, int n_actions_, const Vector& alpha)
      : DiscreteRewardBelief(n_states_, n_actions_), dirichlet(alpha) {
