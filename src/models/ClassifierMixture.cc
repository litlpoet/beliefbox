...
LinearClassifierMixture::LinearClassifierMixture(int n_inputs_, int n_classes_,
                                                 int n_classifiers,
                                                 int projection_size)
    : n_inputs(n_inputs_),
  ...
  for (int i = 0; i < n_classifiers; ++i) {
    classifiers[i] =
        new SparseLinearClassifier(n_inputs, n_classes, projection_size);
...
HashedLinearClassifierMixture::HashedLinearClassifierMixture(
    int n_inputs_, int n_classes_, int n_classifiers, int projection_size)
    : LinearClassifierMixture(n_inputs_, n_classes_, n_classifiers,
                              projection_size) {
