 ...
 public:
  ...
  struct Node {
    ...
    MultivariateNormalUnknownMeanPrecision*
        normal_density;                       ///< local density estimator
    ...
    Node(ContinuousStateContextTreeRL& tree_, const Vector& lower_bound_x_,
         const Vector& upper_bound_x_);
    Node(Node* prev_, const Vector& lower_bound_x, const Vector& upper_bound_x);
    virtual ~Node();
    real Observe(const Vector& x, const Vector& y, real reward,
                 real probability, ContextList& active_contexts);
  ...
  // public methods
  ContinuousStateContextTreeRL(int n_actions_, int max_depth_,
                               int max_depth_cond_, const Vector& lower_bound_x,
                               const Vector& upper_bound_x, real depth_factor_,
                               real weight_factor_);
  ...
  real ValueIteration();
  real Sarsa(real epsilon, real step_size, real gamma, const Vector& y,
             real reward);
  void Reset() {
    current_action = -1;
