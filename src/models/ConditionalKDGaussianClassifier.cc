...
ConditionalKDGaussianClassifier::Node::Node(
    ConditionalKDGaussianClassifier& tree_, Vector& lower_bound_x_,
  ...
  local_probability =
      new MultivariateGaussianClassifier(lower_bound_x.Size(), tree.n_classes);
}

/// Make a node for K symbols at nominal depth d
ConditionalKDGaussianClassifier::Node::Node(
    ConditionalKDGaussianClassifier::Node* prev_, Vector& lower_bound_x_,
    Vector& upper_bound_x_)
    : tree(prev_->tree),
      lower_bound_x(lower_bound_x_),
      upper_bound_x(upper_bound_x_),
...
*/
real ConditionalKDGaussianClassifier::Node::Observe(const Vector& x,
                                                    const int y,
                                                    real probability) {
  ...
  if ((tree.max_depth < 0 || depth < tree.max_depth) && S > threshold &&
    ...
    if (splitting_dimension < 0) {
      ...
      Vector other_mean =
          local_probability->getClassMean(second_most_observed_class);
      splitting_dimension = ArgMax(abs(mean - other_mean));
      mid_point =
          0.5 * (mean(splitting_dimension) + other_mean(splitting_dimension));
...
*/
real ConditionalKDGaussianClassifier::Node::pdf(const Vector& x, const int y,
                                                real probability) {
  real total_probability;
...
/** Recursively obtain \f$P(y | x)\f$.
*/
Vector ConditionalKDGaussianClassifier::Node::Output(const Vector& x,
                                                     const Vector& P_y) {
...
ConditionalKDGaussianClassifier::ConditionalKDGaussianClassifier(
    int n_branches_, int max_depth_, Vector& lower_bound_x,
    Vector& upper_bound_x, int n_classes_)
...
int ConditionalKDGaussianClassifier::NChildren() { return root->NChildren(); }
