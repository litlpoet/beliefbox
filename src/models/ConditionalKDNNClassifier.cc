...
#else
  mid_point = 0;  //(upper_bound_x[splitting_dimension] +
                  //lower_bound_x[splitting_dimension]) / 2.0;
#endif
  local_probability =
      new KNNClassifier(lower_bound_x.Size(), tree.n_classes, 2);
}
...
ConditionalKDNNClassifier::Node::~Node() {
...
*/
real ConditionalKDNNClassifier::Node::Observe(const Vector& x, const int y,
                                              real probability) {
  real total_probability;

  // the local distribution
  real P_local = FUDGE / (real)tree.n_classes +
                 (1 - FUDGE) * local_probability->Observe(x, y);
  ...
  for (int i = 0; i < tree.n_classes; ++i) {
    if (A(i) > 1) {
      ...
      if (i != most_observed_class && A(i) > n_second_most_observed_class) {
  ...
  if ((tree.max_depth < 0 || depth < tree.max_depth) && S > threshold &&
      number_of_observed_classes > 1) {
    if (splitting_dimension < 0) {
      splitting_dimension = ArgMax(upper_bound_x - lower_bound_x);
      mid_point = 0.5 * (upper_bound_x(splitting_dimension) +
                         lower_bound_x(splitting_dimension));
...
*/
real ConditionalKDNNClassifier::Node::pdf(const Vector& x, const int y,
                                          real probability) {
  real total_probability;

  // the local distribution
  real P_local = FUDGE / (real)tree.n_classes +
                 (1 - FUDGE) * local_probability->Output(x)(y);
...
/** Recursively obtain \f$P(y | x)\f$.
*/
Vector ConditionalKDNNClassifier::Node::Output(const Vector& x,
                                               const Vector& P_y) {
  Vector total_probability(tree.n_classes);

  // the local distribution
  Vector P_local =
      local_probability->Output(x) * (1 - FUDGE) + FUDGE / (real)tree.n_classes;
...
void ConditionalKDNNClassifier::Node::Show() {
#if 1
  printf("%d %f #w\n", depth, w);
...
int ConditionalKDNNClassifier::Node::NChildren() {
  int my_children = 0;
  for (int k = 0; k < tree.n_branches; ++k) {
    if (next[k]) {
      my_children++;
...
ConditionalKDNNClassifier::ConditionalKDNNClassifier(
    int n_branches_, int max_depth_, const Vector& lower_bound_x,
    const Vector& upper_bound_x, const int n_classes_)
...
ConditionalKDNNClassifier::~ConditionalKDNNClassifier() { delete root; }

/** Obtain \f$\xi_t(y \mid x)\f$ and calculate \f$\xi_{t+1}(w) = \xi_t(w \mid x,
 * y)\f$.
 */
real ConditionalKDNNClassifier::Observe(const Vector& x, const int y) {
  return root->Observe(x, y, 1);
}

/** Obtain \f$\xi_t(y \mid x)\f$ */
real ConditionalKDNNClassifier::pdf(const Vector& x, const int y) {
  return root->pdf(x, y, 1);
}

/** Obtain \f$\xi_t(y \mid x)\f$ */
Vector& ConditionalKDNNClassifier::Output(const Vector& x) {
  Vector tmp(n_classes);
  output = root->Output(x, tmp);
  return output;
}

void ConditionalKDNNClassifier::Show() {
  root->Show();
  std::cout << "Total contexts: " << NChildren() << std::endl;
}

int ConditionalKDNNClassifier::NChildren() { return root->NChildren(); }
