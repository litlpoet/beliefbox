...
real GoalStateRewardDistribution::pdf(int state, int action,
                                      real reward) const {
...
/// Constructor. Set up the default reward to be received
DiscreteSpaceRewardDistribution::DiscreteSpaceRewardDistribution(int n_states_,
                                                                 int n_actions_)
    : n_states(n_states_),
...
DiscreteSpaceRewardDistribution::DiscreteSpaceRewardDistribution(
    const DiscreteSpaceRewardDistribution& rhs) {
...
DiscreteSpaceRewardDistribution& DiscreteSpaceRewardDistribution::operator=(
    const DiscreteSpaceRewardDistribution& rhs) {
...
/// Generate pdf.
real DiscreteSpaceRewardDistribution::pdf(int state, int action,
                                          real reward) const {
  int ID = getID(state, action);
  if (R[ID]) {
...
void DiscreteSpaceRewardDistribution::setRewardDistribution(
    int s, int a, Distribution* reward) {
  int ID = getID(s, a);
  R[ID] = reward;
...
void DiscreteSpaceRewardDistribution::addRewardDistribution(
    int s, int a, Distribution* reward) {
  int ID = getID(s, a);
  assert(reward);
...
// only use this function once per state-action pair
void DiscreteSpaceRewardDistribution::addFixedReward(int s, int a,
                                                     real reward) {
...
void DiscreteSpaceRewardDistribution::setFixedReward(int s, int a,
                                                     real reward) {
  int ID = getID(s, a);
