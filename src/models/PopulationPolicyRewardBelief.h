...
/** A Multi-task (population) model for inverse reinforcement learning using a
   arior on policy given reward belief.
...
class PopulationPolicyRewardBelief {
 protected:
  ...
  std::vector<std::vector<Matrix> >
      rewards;  ///< matrix of reward function samples ordered [task][sample]
  std::vector<std::vector<FixedDiscretePolicy> >
 ...
 public:
  PopulationPolicyRewardBelief(real eta_, real gamma_, const DiscreteMDP& mdp_);
