...
ContinuousStateContextTreeRL::Node::Node(ContinuousStateContextTreeRL& tree_,
  ...
  local_density = new ContextTreeKDTree(tree.n_branches, tree.max_depth_cond,
                                        tree.lower_bound_x, tree.upper_bound_x);
  ...
  normal_density = new MultivariateNormalUnknownMeanPrecision(
      mean, 1.0, (real)y_dim, Matrix::Unity(y_dim, y_dim));
...
ContinuousStateContextTreeRL::Node::Node(
    ContinuousStateContextTreeRL::Node* prev_, const Vector& lower_bound_x_,
    const Vector& upper_bound_x_)
    : tree(prev_->tree),
      lower_bound_x(lower_bound_x_),
      upper_bound_x(upper_bound_x_),
  ...
  int y_dim = tree.upper_bound_x.Size();
  normal_density = new MultivariateNormalUnknownMeanPrecision(
      (tree.upper_bound_x + tree.lower_bound_x) * 0.5, 1.0, (real)y_dim,
...
// real mean_reward = reward_prior.getMean();
  ...
  if ((tree.max_depth == -1 || depth < tree.max_depth) && S > threshold) {
    ...
    total_probability =
        next[k]->Observe(x_t, y_t, reward, total_probability, active_contexts);
 ...
 */
real ContinuousStateContextTreeRL::Sarsa(real epsilon, real step_size,
                                         real gamma, const Vector& y,
                                         real reward) {
  if (current_action == -1) {
    return 0;
  }
...
/** Perform value iteration.
    ...
    NOT IMPLEMENTED
 */
real ContinuousStateContextTreeRL::ValueIteration() { return 0; }
...
ContinuousStateContextTreeRL::ContinuousStateContextTreeRL(
    int n_actions_, int max_depth_, int max_depth_cond_,
    const Vector& lower_bound_x_, const Vector& upper_bound_x_,
    real depth_factor_, real weight_factor_)
...
/** Observe a transition and update the model and active contexts.
        ...
        \f]
 */
real ContinuousStateContextTreeRL::Observe(const Vector& x, const int a,
                                           const Vector& y, real r) {
