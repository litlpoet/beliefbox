...
class MDP : public AbstractMDP {
 ...
 public:
  MDP(TransitionDistribution<StateType, ActionType>& transition_distribution_,
      RewardDistribution<StateType, ActionType>& reward_distribution_)
      : transition_distribution(transition_distribution_),
        reward_distribution(reward_distribution_) {}
  virtual ~MDP() {}
  StateType getState() const { return state; }
  virtual real getTransitionProbability(const StateType& s, const ActionType& a,
                                        const StateType& s2) const {
    return transition_distribution.pdf(s, a, s2);
  }
  virtual real getRewardProbability(const StateType& s, const ActionType& a,
                                    real r) const {
  ...
  virtual real generateReward(const StateType& s, const ActionType& a) const {
