...
// $Id: test_value_iteration.c,v 1.8 2006/11/12 23:16:03 olethros Exp
// cdimitrakakis $
/***************************************************************************
 *                                                                         *
...
int test_belief_mdp(int T, real gamma, real prior) {
  ...
  for (int t = 0; t < T; t++) {
    for (int f = 0; f <= t; f++) {
      int s = t - f;
      int n = get_bionomial_index(f, s);
      printf("%+.4f %+.4f", value_iteration.getValue(n, PLAY),
...
real evaluate_belief_mdp(int T, real gamma, int n_trials, int horizon) {
  ...
  for (int trial = 0; trial < n_trials; trial++) {
    ...
    SimpleBelief initial_belief(low_value, high_value, prior_belief,
                                1.0 - prior_belief);
    ...
    if (trial % 10 == 0) {
      ...
      fprintf(stderr, "T:%.1fs %.1fs/trial ETA:%.1fm\n", total_time,
              s_per_trial, ((double)n_trials - trial) * s_per_trial / 60.0);
    }
  }
