...
int main(int argc, char** argv) {
  fprintf(stderr,
          "test_simple_bandit $Id: test_simple_bandit.c,v 1.1 2006/10/31 "
          "16:59:39 cdimitrakakis Exp cdimitrakakis $\n");
  if (argc != 4) {
  ...
  VPIPolicy bayes_vpi_policy99(n_actions, &bernoulli_estimate, 0.99,
                               n_samples2);
  VPIPolicy bayes_vpi_policy999(n_actions, &bernoulli_estimate, 0.999,
                                n_samples2);

  PFVPIPolicy pf_vpi_policy(n_actions, &pf_estimate, gamma);

  PopulationOptimalPolicy bayes_policy5(n_actions, &bernoulli_estimate, 0.5,
                                        n_samples2);
  PopulationOptimalPolicy bayes_policy9(n_actions, &bernoulli_estimate, 0.9,
                                        n_samples2);
  PopulationOptimalPolicy bayes_policy99(n_actions, &bernoulli_estimate, 0.99,
                                         n_samples2);
  PopulationOptimalPolicy bayes_policy999(n_actions, &bernoulli_estimate, 0.999,
                                          n_samples2);
  PopulationOptimalPolicy bayes_policy9999(n_actions, &bernoulli_estimate,
                                           0.9999, n_samples2);
  OptimalInfinitePolicy optimal_infinite(n_actions);

  UCB1Policy ucb1_policy(n_actions);

  // Evaluate ("out_opt_inf", optimal_infinite, n_actions);
  Evaluate("out_ucb1", ucb1_policy, n_actions);
  // Evaluate ("out_ucb2", ucb2_policy, n_actions);
  ...
  // Evaluate ("out_bay_vpi999", bayes_vpi_policy, n_actions);
...
void Evaluate(char* fname, DiscreteBanditPolicy& policy, int n_actions) {
  ...
  printf("Completion %d%%. Time: %d elapsed, %d total, %d remaining.\n",
         (int)percent_complete, (int)elapsed_time, (int)total_time,
         (int)remaining_time);
}
