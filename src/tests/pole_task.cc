...
int main(int argc, char** argv) {
      ...
      case 0:
        ...
        switch (option_index) {
          case 0:
            ...
            break;
          case 1:
            max_episodes = atoi(optarg);
            break;
          case 2:
            stat_filename = optarg;
            break;
          case 3:
            max_stats = atoi(optarg);
            break;
          case 4:
          ...
          case 5:
            ...
            break;
          case 6:
            decision_time = atoi(optarg);
            break;
          case 7:
            init_eval = atof(optarg);
            break;
          case 8:
            theta_factor = atof(optarg);
            break;
          case 9:
            gamma = atof(optarg);
            break;
          ...
          case 14:
            ...
            break;
          case 15:
            show_final_policy = true;
            break;
          case 16:
            max_episode_time = atof(optarg);
            break;
          case 17:
            estimate = Sarsa;
            break;
  ...
  fprintf(stat_file, "# %s %s g:%f z:%f l:%f ep:%d st:%d dt:%f(x%d) init:%f\n",
          argv[1], argv[6], gamma, zeta, lambda, max_episodes, max_stats, dt,
          decision_time, init_eval);
  ...
  policy = new DiscretePolicy(n_states, n_actions, alpha, gamma, lambda, false,
                              0.1, init_eval);
  // policy
...
void Evaluate(char* stat_file_name, Policy& policy, int n_actions) {
  ...
  if (stat_filename) {
    ...
    if (!small_stat_file) {
      std::cerr << "# Warning: Could not open file: " << small_stat_filename
                << std::endl;
    }
  }
  real max_t = 0.0f;
  ...
  for (int episode = 0; episode < max_episodes; episode++) {
    ...
    do {
      ...
      if (show_final_policy && episode > max_episodes - 10) {
        printf("%f %f %f %f %f %f\n", pole_cart.x, pole_cart.theta,
               pole_cart.x_ball, pole_cart.y_ball, pole_cart.u, pole_cart.w);
      }
      if (termination) {
        pos = MakeState(pole_cart.w, pole_cart.u, pole_cart.x, pole_cart.theta);
    ...
    if (episode % 100 == 0) {
      std::cerr << "# Ep: " << episode << " T_av: " << local_average_t / 100.0f
