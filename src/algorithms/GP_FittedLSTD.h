...
class GP_FittedLSTD {
 ...
 public:
  GP_FittedLSTD(const real& gamma_, const int& N_,
                Environment<S, A>* environment_,
                std::vector<std::vector<SparseGaussianProcess*> > regression_t_,
                RBFBasisSet* RBFs_, bool update_samples_ = false)
      : gamma(gamma_),
        N(N_),
  ...
  const void Update(real threshold = 0.000001, int max_iter = 10) {
    ...
    do {
      ...
      printf(
          "#ValueIteration (Threshold = %f) :: ComputeStateValues Exiting at "
          "d:%f, n:%d\n",
  ...
  real getValue(const S& state, const A& action) {
    ...
    if (final) temp_v = getValue(next_state);
