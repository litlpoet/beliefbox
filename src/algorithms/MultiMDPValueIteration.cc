...
// $Id: ValueIteration.c,v 1.5 2006/11/08 17:20:17 cdimitrakakis Exp
// cdimitrakakis $
/***************************************************************************
 *                                                                         *
...
/// Setup a value iteration procedure for a list of MDPs with weights w and
/// discount factor gamma
MultiMDPValueIteration::MultiMDPValueIteration(
    const Vector& w_, const std::vector<const DiscreteMDP*>& mdp_list_,
    real gamma_)
    : w(w_), mdp_list(mdp_list_), gamma(gamma_), n_mdps(mdp_list.size()) {
...
/** Return the state-action value for the next stage.
    ...
    \f[
    Q_{\mu,t}(s,a) = r_\mu(s,a) + \gamma \sum_{s'} V_{\mu, t+1}(s') {\cal
   T}_\mu(s' | s, a)
    \f]
 */
real MultiMDPValueIteration::ComputeStateActionValueForSingleMDP(int mu, int s,
                                                                 int a) {
  Vector& V_i = V[mu];
...
void MultiMDPValueIteration::ComputeStateValues(real threshold, int max_iter) {
  ...
  // logmsg ("Runnign ComputeStateValues with epsilon: %f, iter: %d, gamma: %f",
  // threshold, max_iter, gamma);
  do {
...
void MultiMDPValueIteration::ComputeStateActionValues(real threshold,
                                                      int max_iter) {
#if 1
