...
OnlineLSPI::OnlineLSPI(real gamma_, real Delta_, int n_dimension_,
                       int n_actions_, int max_iteration_, RBFBasisSet* bfs_)
    : gamma(gamma_),
      Delta(Delta_),
...
OnlineLSPI::OnlineLSPI(real gamma_, real Delta_, int n_dimension_,
                       int n_actions_, int max_iteration_, int algorithm_,
                       RBFBasisSet* bfs_)
    : gamma(gamma_),
      Delta(Delta_),
      n_dimension(n_dimension_),
...
void OnlineLSPI::LSTD(const Vector& state, const int& action,
                      const real& reward, const Vector& state_,
                      const int& action_, const bool& endsim,
                      const bool& update) {
  if (algorithm == 1)
...
void OnlineLSPI::LSTDQ(const Vector& state, const int& action,
                       const real& reward, const Vector& state_,
                       const int& action_, const bool& endsim,
                       const bool& update) {
  Vector Phi_;
...
void OnlineLSPI::LSTDQ_OPT(const Vector& state, const int& action,
                           const real& reward, const Vector& state_,
                           const int& action_, const bool& endsim,
                           const bool& update) {
  Vector Phi_;
