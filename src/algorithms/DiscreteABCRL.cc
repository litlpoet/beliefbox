...
DiscreteABCRL::DiscreteABCRL(int n_states_, int n_actions_, real gamma_,
                             real epsilon_,
                             EnvironmentGenerator<int, int>* generator_,
                             RandomNumberGenerator* rng_, int max_samples_,
                             int n_iterations_, bool use_upper_bound_)
    : n_states(n_states_),
...
/// Full observation
real DiscreteABCRL::Observe(int state, int action, real reward, int next_state,
                            int next_action) {
  if (state >= 0) {
...
DiscreteMDP* DiscreteABCRL::GenerateMDP() const {
  ...
  while (iter < n_iterations) {
    ...
    for (uint i = 0; i < policies.size(); ++i) {
      test_demos.Simulate(*environment, *policies[i], gamma,
                          demonstrations.length(i));
    }
    real mean = 0;
    for (uint i = 0; i < policies.size(); ++i) {
...
int DiscreteABCRL::Act(real reward, int next_state) {
