...
FixedContinuousPolicy::FixedContinuousPolicy(int n_dimension, int n_actions_,
                                             RBFBasisSet* bfs_)
...
FixedContinuousPolicy::FixedContinuousPolicy(int n_dimension_, int n_actions_,
                                             RBFBasisSet* bfs_,
...
void FixedContinuousPolicy::Observe(const Vector& previous_state,
                                    const int& action, real r,
...
real FixedContinuousPolicy::getActionProbability(const Vector& start_state,
                                                 const int& action) {
  assert(action >= 0 && action < n_actions);
  state = start_state;
  StatePolicy();
