...
class DiscreteABCRL : public OnlineAlgorithm<int, int> {
 protected:
  ...
  std::vector<ValueIteration*>
      value_iteration;  ///< value iteration on each separate model
 ...
 public:
  ...
  DiscreteABCRL(int n_states_, int n_actions_, real gamma_, real epsilon_,
                EnvironmentGenerator<int, int>* generator_,
                RandomNumberGenerator* rng_, int max_samples_ = 1,
                int n_iterations_ = 25, bool use_upper_bound_ = false);
  ...
  inline real UpperBound(int state, int action) { return QU(state, action); }

  inline real LowerBound(int state, int action) { return QL(state, action); }

  /** Set the rewards to Singular distributions.

      Since this is a Bayesian approach, we can simply set the belief about the
     reward in each state to be a singular distribution.
  */
