...
BayesianMultivariate::BayesianMultivariate(
    int n_actions_, int n_input_dim_, int n_output_dim_, real gamma_,
    real epsilon_, RBFBasisSet* rbf_,
    ...
    FittedValueIteration<Vector, int>* FVI_, FittedLSTD<Vector, int>* FLSTD_,
    FittedQValueIteration<Vector, int>* FQVI_, real baseline_)
...
void BayesianMultivariate::Observe(Vector state, int action, real reward,
                                   Vector next_state) {
  Vector phi;
  Vector r(reward);
  if (rbf != NULL) {
