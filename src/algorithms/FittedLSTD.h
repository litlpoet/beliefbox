...
class FittedLSTD {
 ...
 public:
  FittedLSTD(const real& gamma_, const int& N_, const int& M_,
             const int& grids_, Environment<S, A>* environment_,
             std::vector<BayesianMultivariateRegression*> regression_t_,
             RBFBasisSet* RBFs_, real scale_ = 1.0,
  ...
  const void Update(real threshold = 0.0001, int max_iter = -1) {
    ...
    do {
      ...
      for (int i = 0; i < N; ++i) {
        ...
        environment->Reset();
        environment->setState(states[i]);
        /// In this point we calculate the basis function for the collected
        /// state i
      ...
      weights = ((1.0 / N) * AA + (lambda)*Matrix::Unity(dim, dim)).Inverse() *
                (b * (1.0 / N));
  ...
  // BasisModelCreation returns the basis function for state s that used for the
  ...
  // BasisAPICreation returns the basis function for state s that used for the
  // API algorithm
