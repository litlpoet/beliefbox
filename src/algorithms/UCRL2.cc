...
/** Constructor.
    ...
    \rng
*/
UCRL2::UCRL2(int n_states_, int n_actions_, real gamma_,
             DiscreteMDPCounts* model_, RandomNumberGenerator* rng_, real delta)
    : n_states(n_states_),
...
UCRL2::~UCRL2() { delete value_iteration; }
void UCRL2::Reset() {
  state = -1;
  n_resets++;
...
/// Full observation
real UCRL2::Observe(int state, int action, real reward, int next_state,
                    int next_action) {
  if (state >= 0) {
...
int UCRL2::Act(real reward, int next_state) {
  ...
  action = ArgMax(tmpQ);
  // printf("%f %d %d %f# rsaV'\n", reward, state, action,
  // value_iteration->getValue(state, action));
  return action;
}
