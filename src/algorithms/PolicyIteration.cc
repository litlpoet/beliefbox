...
PolicyIteration::PolicyIteration(const DiscreteMDP* mdp_, real gamma_,
                                 real baseline_)
...
/** ComputeStateValues
    ...
    threshold - exit policy estimation when difference in Q is smaller than the
   threshold
    max_iter - exit policy estimation when the number of iterations reaches
   max_iter
    The policy iteration itself stops whenever the policy has been the same for
   a bit.
