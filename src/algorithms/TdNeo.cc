...
TdNeo::TdNeo(const int n_states_, const int n_actions_, const real gamma_,
             const real lambda_, const real alpha_,
             VFExplorationPolicy* const exploration_policy_,
             const real initial_value_, const real baseline_)
    : N_STATES(n_states_),
...
real TdNeo::Observe(const int state, const int action, const real reward,
  ...
  // LAMBDA(state, action) = ((double) (max - min)) / ((double) (TOTAL * (1 +
  // U)));
  LAMBDA(state, action) = (mean == 0)
                              ? DEFAULT_LAMBDA
                              : ((deviation > mean) ? 0 : 1 - deviation / mean);

  // - Delta
  // const real DELTA = (reward - BASELINE) + GAMMA * Q(next_state, next_action)
  // - Q(state, action);
  const real DELTA =
      (reward - BASELINE) + GAMMA * MEANS(next_state, next_action) - mean;
...
real TdNeo::Observe(const real reward, const int next_state,
...
int TdNeo::Act(const real reward, const int next_state) {
