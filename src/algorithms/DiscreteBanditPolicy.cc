 ...
 * $*/
// copyright (c) 2004-2007 by Christos Dimitrakakis
...
/// Make e-greedy
EpsilonGreedyPolicy::EpsilonGreedyPolicy(int n_actions, real epsilon,
...
/// Reset
void EpsilonGreedyPolicy::Reset() { estimator->Reset(); }

/// observe
void EpsilonGreedyPolicy::Observe(int a, real r) { estimator->Observe(a, r); }
...
PopulationOptimalPolicy::PopulationOptimalPolicy(int n_actions,
                                                 ActionValueEstimate* estimator,
...
/// Reset
void PopulationOptimalPolicy::Reset() { estimator->Reset(); }
...
PopulationSamplePolicy::PopulationSamplePolicy(int n_actions,
                                               ActionValueEstimate* estimator,
                                               real gamma) {
  this->n_actions = n_actions;
  this->estimator = estimator;
...
/// Reset
void PopulationSamplePolicy::Reset() { estimator->Reset(); }
void PopulationSamplePolicy::Observe(int a, real r) {
  estimator->Observe(a, r);
}
PopulationSamplePolicy::~PopulationSamplePolicy() {}
int PopulationSamplePolicy::SelectAction() { return estimator->Sample(); }
...
/// Reset
void NaiveE3Policy::Reset() { estimator->Reset(); }
void NaiveE3Policy::Observe(int a, real r) { estimator->Observe(a, r); }
/// Destroy
NaiveE3Policy::~NaiveE3Policy() { delete estimator; }

/// Select action: randomly with probability epsilon, otherwise greedily
int NaiveE3Policy::SelectAction() { return estimator->Sample(); }

/// Make new population VPI policy
PopulationVPIPolicy::PopulationVPIPolicy(int n_actions,
...
/// Reset
void PopulationVPIPolicy::Reset() { estimator->Reset(); }
void PopulationVPIPolicy::Observe(int a, real r) { estimator->Observe(a, r); }
...
PFVPIPolicy::PFVPIPolicy(int n_actions, PFActionValueEstimate* estimator,
                         real gamma, int n_samples) {
  this->n_actions = n_actions;
  this->estimator = estimator;
  this->gamma = gamma;
...
/// Reset
void PFVPIPolicy::Reset() { estimator->Reset(); }

void PFVPIPolicy::Observe(int a, real r) { estimator->Observe(a, r); }
...
void OptimalInfinitePolicy::Reset() { estimate->Reset(); }
void OptimalInfinitePolicy::Observe(int a, real r) { estimate->Observe(a, r); }
OptimalInfinitePolicy::~OptimalInfinitePolicy() { delete estimate; }
int OptimalInfinitePolicy::SelectAction() { return estimate->Sample(); }
