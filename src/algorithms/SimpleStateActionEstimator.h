/* -*- Mode: C++; -*- */
/* VER: $Id: SimpleStateActionEstimator.h,v 1.1 2006/10/23 08:33:32 olethros Exp
 * cdimitrakakis $*/
...
class SimpleStateActionEstimator : public StateActionEstimator {
 public:
  ...
  real gamma;     ///< gamma
  SimpleStateActionEstimator(int n_states, int actions, real gamma,
                             real init_val = 0.0f);
...
class MaxStateActionEstimator : public SimpleStateActionEstimator {
 public:
  MaxStateActionEstimator(int n_states, int actions, real alpha, real gamma,
                          real init_val = 0.0f);
  virtual ~MaxStateActionEstimator();
  virtual void Observe(int s_p, int a_p, real r, int s = -1, int a = -1);
  real MaxActionValue(int state) { return Max(n_actions, Q[state]); }
};

class SarsaStateActionEstimator : public SimpleStateActionEstimator {
 public:
  SarsaStateActionEstimator(int n_actions, int n_states, real alpha, real gamma,
                            real init_val = 0.0f);
