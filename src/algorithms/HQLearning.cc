...
HQLearning::HQLearning(int n_agents_, int n_states_, int n_actions_,
                       real gamma_, real lambda_, real alpha_, real epsilon_,
                       real beta_, real initial_value_, real baseline_)
    : n_agents(n_agents_),
      n_states(n_states_),
  ...
  for (int i = 0; i < n_agents; ++i) {
    sub_agent[i] =
        new SubAgent(n_states, n_actions, gamma, lambda, alpha, epsilon, beta);
  }
  Reset();
}
...
real HQLearning::Observe(int action, int next_state, real reward) { return 0; }
