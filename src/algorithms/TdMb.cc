...
TdMb::TdMb(const int n_states_, const int n_actions_, const real gamma_,
...
real TdMb::Observe(const int state, const int action, const real reward,
  ...
  LAMBDA(state, action) =
      (max + U) / (TOTAL * (1 + U)) - (min + U) / (TOTAL * (1 + U));
...
real TdMb::Observe(const real reward, const int next_state,
