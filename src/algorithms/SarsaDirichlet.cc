...
SarsaDirichlet::SarsaDirichlet(int n_states_, int n_actions_, real gamma_,
                               real lambda_, real alpha_,
                               VFExplorationPolicy* exploration_policy_,
                               real initial_value_, real baseline_)
    : Sarsa(n_states_, n_actions_, gamma_, lambda_, alpha_, exploration_policy_,
            initial_value_, baseline_),
...
real SarsaDirichlet::Observe(real reward, int next_state, int next_action) {
  real n_R = (reward - baseline) +
             gamma * Q(next_state, next_action);  // partially observed return
  real p_R = 0.0;
  real TD = 0.0;
  if (state >= 0 && action >= 0) {
