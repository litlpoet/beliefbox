...
AverageValueIteration::AverageValueIteration(const DiscreteMDP* mdp,
                                             bool relative, bool synchronous)
    : RELATIVE(relative), SYNCHRONOUS(synchronous), baseline(0) {
  assert(mdp);
  this->mdp = mdp;
...
void AverageValueIteration::ComputeStateValues(real threshold, int max_iter) {
  ...
  do {
    ...
    max_iter--;
  } while (stuck_count < 10 && Delta >= threshold &&
           (max_iter > 0 || dDelta < 0));
