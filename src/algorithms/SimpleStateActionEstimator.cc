/* -*- Mode: C++; -*- */
/* VER: $Id: SimpleStateActionEstimator.c,v 1.1 2006/10/23 08:33:32 olethros Exp
 * cdimitrakakis $*/
...
SimpleStateActionEstimator::SimpleStateActionEstimator(int n_states,
                                                       int n_actions,
                                                       real gamma,
                                                       real init_val)
    : StateActionEstimator(n_states, n_actions) {
...
real SimpleStateActionEstimator::TransProbability(int s_p, int a, int s) {
  fprintf(
      stderr,
      "SimpleStateActionEstimator cannot estimate transition probability\n");
  exit(-1);
}
...
real SimpleStateActionEstimator::GetTransitionPrior(int s_p, int a, int s,
                                                    real x) {
  fprintf(
      stderr,
      "SimpleStateActionEstimator cannot estimate transition probability\n");
 ...
 */
MaxStateActionEstimator::MaxStateActionEstimator(int n_states, int n_actions,
                                                 real alpha, real gamma,
                                                 real init_val)
    : SimpleStateActionEstimator(n_states, n_actions, gamma, init_val) {
  this->alpha = alpha;
}
 ...
 */
SarsaStateActionEstimator::SarsaStateActionEstimator(int n_states,
                                                     int n_actions, real alpha,
                                                     real gamma, real init_val)
    : SimpleStateActionEstimator(n_states, n_actions, gamma, init_val) {
  this->alpha = alpha;
}
 ...
 */
void SarsaStateActionEstimator::Observe(int s_p, int a_p, real r, int s,
                                        int a) {
  // DISABLED_ASSERT(s>=0);
  // DISABLED_ASSERT(a>=0);
  real Er = Q[s][a];
