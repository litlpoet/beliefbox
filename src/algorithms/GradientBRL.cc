...
GradientBRL::GradientBRL(int n_states_, int n_actions_, real gamma_,
                         real epsilon_, real step_size_, MDPModel* model_,
                         RandomNumberGenerator* rng_, bool use_upper_bound_)
    : n_states(n_states_),
...
/// Full observation
real GradientBRL::Observe(int state, int action, real reward, int next_state,
                          int next_action) {
  if (state >= 0) {
