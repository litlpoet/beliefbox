...
void FixedDiscretePolicy::Observe(const int& previous_state, const int& action,
                                  real r, const int& next_state) {
  state = next_state;
}
...
void FixedDiscretePolicy::Reset(const int& start_state) { state = start_state; }
...
real FixedDiscretePolicy::getActionProbability(const int& state,
...
// How to calculate this? When gamma -> 1, matrix inversion may be hard.
// However, the same is true for the infinite expansion.
#if 0
