...
class FittedQValueIteration {
 ...
 public:
  ...
  real getValue(const S& state) {
    Vector Q(n_actions);
    for (int i = 0; i < n_actions; i++) Q[n_actions] = getValue(state, i);
    return Max(Q);
  }
  ...
  void sampleSelection() {
    ...
    AbstractPolicy<Vector, int>* policy =
        new RandomPolicy(environment->getNActions(), rng);
    ...
    for (int i = 0; i < rollout->getNRollouts(); ++i) {
      for (int j = 0; j < rollout->getNSamples(i); ++j) {
        Vector phi =
            BasisFunction(rollout->getState(i, j), rollout->getAction(i, j));
