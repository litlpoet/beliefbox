...
class UCTMC {
 ...
 public:
  ...
  real UCT_Search(S state, int depth, bool terminal) {
    ...
    real bestValue =
        Q(index, bestAction) +
        2 * c_uct * sqrt(log(C.RowSum(index)) / (C(index, bestAction) + 1));

    for (int a = 1; a < nActions; ++a) {
      real value = Q(index, a) +
                   2 * c_uct * sqrt(log(C.RowSum(index)) / (C(index, a) + 1));
      if (value > bestValue) {
        bestValue = value;
        bestAction = a;
      }
    ...
    Q(index, bestAction) +=
        (SampleReturn - Q(index, bestAction)) / C(index, bestAction);
    ...
    real LambdaReturn =
        lambda * SampleReturn + (1 - lambda) * Q(index, bestAction);
    return LambdaReturn;
  };
