...
class Rollout {
 public:
  ...
  Rollout(const S& start_state_, const A& start_action_, P* policy_,
          Environment<S, A>* environment_, real gamma_)
  ...
  Rollout(const S& start_state_, P* policy_, Environment<S, A>* environment_,
          real gamma_, bool sampling_)
      : start_state(start_state_),
        policy(policy_),
        environment(environment_),
        gamma(gamma_),
  ...
  void Act(const A& a, real discount_factor) {
    ...
    if (sampling) {
      esamples.push_back(
          Observations(previous_state, a, reward, end_state, endsim));
    }
  ...
  void Sample(const int period, bool use_start_state = true) {
    ...
    // logmsg("Length: %d, R: %f, U: %f, State: ", T, total_reward,
    // discounted_reward); end_state.print(stdout);
  }
  ...
  void UniformSampling(const int episodes, const int period) {
    for (int i = 0; i < episodes; ++i) {
      SetState(urandom(environment->StateUpperBound(),
  ...
  void SetState(const S& start_state_) { start_state = start_state_; }
  S getState(const int& r, const int& s) { return Samples[r][s].state; }
  A getAction(const int& r, const int& s) { return Samples[r][s].action; }
  real getReward(const int& r, const int& s) { return Samples[r][s].reward; }
  S getNextState(const int& r, const int& s) {
    return Samples[r][s].next_state;
  }
  bool getEndsim(const int& r, const int& s) { return Samples[r][s].endsim; }
  int getNRollouts() { return Samples.size(); }
  int getNSamples() {
    int n_samples = 0;
  ...
  int getNSamples(const int r) { return Samples[r].size(); }
};
