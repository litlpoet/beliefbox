/* -*- Mode: C++; -*- */
/* VER: $Id: ActionValueEstimate.c,v 1.6 2006/10/31 16:59:39 cdimitrakakis Exp
 * cdimitrakakis $*/
...
void PointEstimate::Observe(int a, real r) { q[a] += alpha * (r - q[a]); }

/// Get the mean reward of action a
real PointEstimate::GetMean(int a) { return q[a]; }

/// Sample an action i with probability p(q[i] > q[j])
int PointEstimate::Sample() { return GetMax(); }
...
real PointEstimate::Sample(int a) { return q[a]; }

PopulationEstimate::PopulationEstimate(int n_actions, int n_members,
                                       real alpha) {
  this->n_actions = n_actions;
  q.resize(n_actions);
  s.resize(n_actions);
...
/// Update estimates when observing reward r after action a is taken
void PopulationEstimate::Observe(int a, real r) { q[a].Observe(r); }

/// Get the mean reward of action a
real PopulationEstimate::GetMean(int a) { return q[a].GetMean(); }

/// Get the mean reward of action a
real PopulationEstimate::GetVar(int a) { return q[a].GetVar(); }
...
real PopulationEstimate::Sample(int a) { return q[a].Sample(); }

real PopulationEstimate::GetMemberValue(int a, int i) { return q[a].x[i]; }
...
real AverageEstimate::GetMean() { return Ex; }

int AverageEstimate::GetN() { return N; }

/// This is a fully Bayesian estimate, yay
BernoulliEstimate::BernoulliEstimate(int n_actions, int n_samples, real alpha,
                                     real beta) {
  this->n_actions = n_actions;
...
/// Get the mean reward of action a
real BernoulliEstimate::GetMean(int a) { return prior[a].getMean(); }
...
real BernoulliEstimate::Sample(int a) { return prior[a].generate(); }
/// Reset estimates
...
ActionValueE3Estimate::ActionValueE3Estimate(int n_actions, real epsilon,
                                             real T) {
...
/// Get the mean reward of action a
real ActionValueE3Estimate::GetMean(int a) { return q[a].GetMean(); }
...
real ActionValueE3Estimate::Sample(int a) { return q[a].GetMean(); }
/// Reset estimates
...
CountingBernoulliEstimate::~CountingBernoulliEstimate() { delete[] obs; }
...
/// Sample the action with maximum mean
int CountingBernoulliEstimate::GetMax() { return Sample(); }
/// Sample the second action
int CountingBernoulliEstimate::GetSecondMax() { return Sample(); }

/// Sample an action i with probability p(q[i] > q[j])
real CountingBernoulliEstimate::Sample(int a) { return GetMean(a); }
/// Reset estimates
