...
class FittedValueIteration {
 protected:
  ...
  RBFBasisSet*
      RBFs_model;  ///< The Radial basis functions that used in the model.
  RBFBasisSet* RBFs;
 ...
 public:
  FittedValueIteration(
      const real& gamma_, const int& N_, const int& M_, const int& grids_,
      Environment<S, A>* environment_,
      std::vector<BayesianMultivariateRegression*> regression_t_,
      RBFBasisSet* RBFs_, real scale_ = 1.0, bool update_samples_ = false)
      : gamma(gamma_),
        N(N_),
        M(M_),
        grids(grids_),
  ...
  //    printf("#ValueIteration::ComputeStateValues Exiting at d:%f,
  //n:%d\n", distance, n_iter);
  //  }
  //
  const void Update(real threshold = 0.0001, int max_iter = -1) {
    ...
    do {
      ...
      weights = ((1.0 / steps) * AA + (0.001 * steps) * Matrix::Unity(dim, dim))
                    .Inverse_LU() *
                (b * (1 / steps));
  ...
  //            //if(final) {
  //            Vector next_state =
  //regression_t[a]->generate(phi_model);
  //            //Vector next_state =
  //environment->getState();
  //            V[a] = getValue(next_state) + r;
  //            //    }
  ...
  //        }
  //      }
  //      weights = ((1.0/steps)*AA +
  //(0.01*steps)*Matrix::Unity(dim,dim)).Inverse_LU()*(b*(1/steps));
  //
  //      //        Vector v_old =
  //Transpose(PHI)*weights;
  //      //        weights = pseudo_inv*T;
  //      printf("weights\n");
  //      weights.print(stdout);
  //      //        printf("PREDICTION\n");
  ...
  //      printf("adfadfadf\n");
  //      weights = (((1.0/steps)*AA) +
  //1*Matrix::Unity(dim,dim)).Inverse_LU()*(b*(1.0/steps));
  //
  ////        Vector v_old =
  //Transpose(PHI)*weights;
  //      //        weights = pseudo_inv*T;
  ...
  void sampleSelection() {
    ...
    pseudo_inv =
        (PHI * Transpose(PHI) + lambda * Matrix::Unity(dim, dim)).Inverse_LU() *
        PHI;
  }
  ...
  //    AbstractPolicy<Vector, int>* policy = new
  //RandomPolicy(environment->getNActions(), rng);
  //    rollout = new Rollout<Vector, int, AbstractPolicy<Vector, int>
  //>(environment->getState(), policy, environment, gamma, true);
