...
class CoverFittedLSTDQ {
 ...
 public:
  CoverFittedLSTDQ(const real& gamma_, const int& N_, const int& M_,
                   Environment<S, A>* environment_,
                   std::vector<CoverTree*> cover_, const int& grids_ = 0,
                   real scale_ = 1.0, bool update_samples_ = false)
      : gamma(gamma_),
        N(N_),
        M(M_),
  ...
  const void Update(real threshold = 0.0001, int max_iter = -1) {
    ...
    do {
      ...
      weights =
          ((1.0 / steps) * AA + (lambda * steps) * Matrix::Unity(dim, dim))
              .Inverse_LU() *
          (b * (1 / steps));
  ...
  real getValue(const S& state) {
    Vector V(n_actions);
    for (int i = 0; i < n_actions; ++i) V[i] = getValue(state, i);
    weights.print(stdout);
    V.print(stdout);
    return Max(V);
  }
  ...
  void sampleSelection() {
    ...
    //    pseudo_inv = (PHI*Transpose(PHI) +
    // lambda*Matrix::Unity(dim,dim)).Inverse()*PHI;
  }
  ...
  Vector BasisConstruction(const S& state, const A& action) {
    ...
    if (RBFs == NULL) {
      ...
      std::vector<std::pair<int, real> > path =
          cover[action]->ExternalBasisCreation(state);
