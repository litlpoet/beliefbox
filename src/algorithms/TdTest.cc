...
TdTest::TdTest(const int n_states_, const int n_actions_, const real gamma_,
...
real TdTest::Observe(const int state, const int action, const real reward,
  ...
  LAMBDA(state, action) = (mean == 0)
                              ? DEFAULT_LAMBDA
                              : ((deviation > mean) ? k : 1 - deviation / mean);
...
real TdTest::Observe(const real reward, const int next_state,
