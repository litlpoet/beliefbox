...
class RolloutState {
 public:
  ...
  std::vector<Rollout<Vector, int, AbstractPolicy<Vector, int> >*>
      rollouts;  ///< The set of rollouts
  ...
  RolloutState(Environment<Vector, int>* environment_,
               AbstractPolicy<Vector, int>* policy_, Vector& start_state_,
               real gamma_);
  ~RolloutState();
  Vector SampleFromPolicy();
  Rollout<Vector, int, AbstractPolicy<Vector, int> >* NewRollout(
  ...
  void Bootstrap(KDTree<RolloutState>& tree, real L);
  real Gap();
};
...
/** Rollout sampling approximate policy iteration.
    ...
    Iteration",
    C. Dimitrakakis, M. Lagoudakis, European Workshop on Reinforcement Learning
   (EWRL), 2008.
