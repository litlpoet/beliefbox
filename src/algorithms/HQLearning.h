...
/** Hierarchical Q-learning.
        ...
        Marco Wiering, Juergen Schmidhuber, 1997, "HQ-Learning", Adaptive
   Behaviour 6:2.
 */
class HQLearning : public OnlineAlgorithm<int, int> {
 public:
  class SubAgent {
   public:
    ...
    SubAgent(int n_states_, int n_actions_, real gamma_, real lambda_,
             real alpha_, real epsilon_, real beta_)
        : n_states(n_states_),
      ...
      q_learning = new QLearning(n_states, n_actions, gamma, lambda, alpha,
                                 exploration_policy);
    ...
    void Reset() { q_learning->Reset(); }
    int Act(real reward, int state) { return q_learning->Act(reward, state); }
 ...
 protected:
  ...
  std::vector<SubAgent*> sub_agent;  ///< the sub-agent that will perform Q
                                     ///learning on just the observations.
 ...
 public:
  HQLearning(int n_agents, int n_states_, int n_actions_, real gamma_,
             real lambda_, real alpha_, real epsilon_, real beta_,
  ...
  virtual real getValue(int s, int a) { return Q(state, action); }
};
