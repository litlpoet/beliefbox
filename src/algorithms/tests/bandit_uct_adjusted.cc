...
int MakeDecision(ExpansionMethod expansion_method, int n_states, int n_actions,
  ...
  for (int iter = 0; iter < n_iter; iter++) {
    ...
    if (expansion_method == SerialExpansion) {
      ...
      upper_mdp.Check();
      ValueIteration value_iteration_upper(&upper_mdp, gamma);
      value_iteration_upper.ComputeStateActionValues(0.00001,
    ...
    if (verbose >= 100) {
      std::cout << n_leaf_nodes << " leaf nodes, " << n_edge_nodes
                ...
                << "expanding node " << node_index << " & action node "
                << action_node_index << std::endl;
    }
      ...
      for (uint e = 0; e < action_node->outs.size(); ++e) {
        ...
        for (uint d = 0; d < leaf_nodes.size(); ++d) {
          if (node_set[leaf_nodes[d]] == dst) {
            ...
            if (verbose >= 100) {
              std::cout << "-> descendant: " << leaf_nodes[d] << std::endl;
            }
  ...
  for (uint s = 0; s < node_set.size(); s++) {
    node_set[s]->L =
        node_set[s]->belief.getGreedyReturn(node_set[s]->state, gamma);
  }
