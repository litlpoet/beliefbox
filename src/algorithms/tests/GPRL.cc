...
void OfflineGPRL(
    real gamma, int n_steps, int n_episodes,
...
void EvaluatePrediction(
    int N, std::vector<std::vector<SparseGaussianProcess*> > algorithm,
    ContinuousStateEnvironment* environment);
...
int main(int argc, char* argv[]) {
  ...
  int m = RBFs->size() + 1;  // redefinition of the input dimensions (size of
                             // the created basis functions plus a dummy state)
  std::cout << "# Number of basis functions: " << m << std::endl;

  std::cout << "# Creating " << n_actions * n_states
            << " Gaussian Prediction environement models..." << std::endl;

  // bayesian multivariate regression model for the system transition model (one
  // for each action & dimension)
  std::vector<std::vector<SparseGaussianProcess*> > GaussianPrediction(
      n_actions, std::vector<SparseGaussianProcess*>(n_states));
  ...
  for (int i = 0; i < n_actions; ++i) {
    for (int j = 0; j < n_states; ++j) {
      GaussianPrediction[i][j] = new SparseGaussianProcess(
          noise_variance, scale_length, sig_val, threshold, scale_length_dic);
    }
  }
  std::cout
      << "Creation of the Gaussian Prediction environement models completed..."
      << std::endl;

  std::cout << "Fitted LSTD Initialization..." << std::endl;
  GP_FittedLSTD<Vector, int>* algorithm = new GP_FittedLSTD<Vector, int>(
      options.gamma, options.n_samples, environment, GaussianPrediction, RBFs);
  ...
  if (online_test) {
    ...
    run_statistics = OnlineGPRL(options.gamma, options.n_testing_e_steps,
                                options.n_testing_episodes, algorithm,
                                GaussianPrediction, environment);
  } else {
    episodes = options.n_testing_episodes;
    OfflineGPRL(options.gamma, options.n_training_e_steps,
                options.n_training_episodes, algorithm, GaussianPrediction,
                environment);
  ...
  char buffer[100];
  sprintf(buffer, "%s_GPRL_RESULTS_STEPS_%s", test_mode,
          options.environment_name);
  ...
  fclose(output);
  sprintf(buffer, "%s_GPRL_RESULTS_REWARDS_%s", test_mode,
          options.environment_name);
...
void OfflineGPRL(
  ...
  //						  EnvPrediction,
  //						  environment);
  printf("The offline training was completed\n");
}
...
Statistics OnlineGPRL(
  ...
  for (int episode = 0; episode < n_episodes; ++episode) {
    ...
    for (int a = 0; a < n_actions; ++a) {
      if (Input[a].size() > 0) {
        for (int dim = 0; dim < n_states; ++dim) {
          EnvPrediction[a][dim]->AddObservation(Input[a],
                                                Output[a * n_states + dim]);
...
Statistics EvaluateAlgorithm(real gamma, int episode_steps, int n_episodes,
  ...
  while (1) {
    ...
    if (!action_ok) {
      ...
      //                statistics.ep_stats[episode].steps++;
      //                statistics.ep_stats[episode].total_reward += reward;
      //				statistics.ep_stats[episode].discounted_reward
      //+= discount * reward;
      //            }
      total_reward += reward;
