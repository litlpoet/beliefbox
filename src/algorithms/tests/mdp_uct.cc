...
int main(int argc, char** argv) {
  ...
  if (argc <= 1) {
    fprintf(stderr,
            "Usage: mdp_uct method gamma n_iter n_actions verbose "
  ...
  for (int experiment = 0; experiment < n_experiments; experiment++) {
    ...
    int state = 0;
    EasyMDPBelief belief(n_actions, alpha, beta);
    ...
    for (int t = 0; t < horizon; t++) {
      ...
      int action =
          MakeDecision(method, n_states, n_actions, belief, state, gamma,
                       n_iter, verbose, max_value_iterations, fout);
...
int ChooseGreedyBranch(BeliefTree<EasyMDPBelief>& tree, real gamma) {
...
int MakeDecision(ExpansionMethod expansion_method, int n_states, int n_actions,
                 EasyMDPBelief prior, int state, real gamma, int n_iter,
                 int verbose, int max_value_iterations, FILE* fout) {
  BeliefTree<EasyMDPBelief> tree(prior, state, n_states, n_actions);
  std::vector<BeliefTree<EasyMDPBelief>::Node*> node_set = tree.getNodes();
  ...
  for (int iter = 0; iter < n_iter; iter++) {
    ...
    if (expansion_method == SerialExpansion) {
      ...
      for (int i = 0; i < n_leaf_nodes; i++) {
        BeliefTree<EasyMDPBelief>::Node* node = node_set[leaf_nodes[i]];
      ...
      for (int i = 0; i < n_leaf_nodes; i++) {
        int n = leaf_nodes[i];
        BeliefTree<EasyMDPBelief>::Node* node = node_set[n];
      ...
      for (int i = 0; i < n_leaf_nodes; i++) {
        ...
        real discount = pow(gamma, (real)node->depth);
    ...
    if (verbose >= 100) {
      std::cout << n_leaf_nodes << " leaf nodes, " << n_edge_nodes
                << " edge nodes, "
                << "expanding node " << node_index << std::endl;
    }
    ...
    if (verbose >= 60) {
      for (int s = 0; s < mean_mdp.getNStates(); s++) {
        std::cout << "V[" << s << "]"
                  ...
                  << value_iteration_upper.getValue(s) << std::endl;
      }
    }
