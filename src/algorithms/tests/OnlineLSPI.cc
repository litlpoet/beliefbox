...
struct PerformanceStatistics {
  ...
  PerformanceStatistics()
      : total_reward(0), discounted_reward(0), run_time(0) {}
};
...
int main(int argc, char* argv[]) {
  ...
  //	printf("Total number of collected samples ->
  //%d\n",rollout->getNSamples());

  RBFBasisSet* RBFs = new RBFBasisSet(Discretisation, scale);
  OnlineLSPI* lspi = new OnlineLSPI(gamma, delta, state_dimension, n_actions,
                                    max_iteration, RBFs);
  ...
  for (int run = 0; run < n_runs; ++run) {
    ...
    for (int ep = 0; ep < episodes; ++ep) {
      ...
      for (uint t = 0; t < episode_steps; ++t) {
        ...
        lspi->LSTDQ(state, action, reward, next_state, next_action,
                    environment->getEndsim(), false);
        steps++;
        total_reward += reward;
        epsilon *= 0.9968;
