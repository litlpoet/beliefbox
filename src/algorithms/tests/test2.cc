...
class TotalRewardStatistic {
 ...
 public:
  ...
  real distance(Demonstrations<X, A>& data, Demonstrations<X, A>& sample) {
    ...
    real bound =
        sqrt(C * (1.0 / (real)data.size() + 1.0 / (real)sample.size()));
...
class ABCRL {
 public:
  ...
  F statistic;
  ABCRL() {}
  void GetSample(Demonstrations<X, A>& data, P& policy, real discounting,
                 real epsilon, real delta, real R_max, int n_trajectories,
                 int n_samples, std::vector<M>& samples,
...
void RunTest(Options& options) {
  ...
  for (int i = 0; i < V_LSPI.Size(); ++i) {
    ...
    AbstractPolicy<Vector, int>* lspi_policy = getLSPIPolicy(
        &samples[i], policy, &training_rollouts, RBFs, estimation_options);
  ...
  printf("%f %f %f # V V_LSPI V_LSPI_oracle\n", V_initial,
         V_LSPI.Sum() / (real)V_LSPI.Size(), V_lspi_oracle);
}
