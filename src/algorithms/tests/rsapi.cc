...
struct PerformanceStatistics {
  ...
  PerformanceStatistics()
      : total_reward(0), discounted_reward(0), n_of_steps(0), run_time(0) {}
};

struct AveragePerformanceStatistics : PerformanceStatistics {
  int N;
  AveragePerformanceStatistics() : N(0) {}
...
int main(int argc, char* argv[]) {
  ...
  enum SamplingMethod { UNIFORM, UPPER_BOUND, ERROR_BOUND };
  int n_states = 100;
  real gamma = 0.99;
  int n_iter = 10;
  int n_rollouts = 100;
  ...
  for (int iter = 0; iter < n_iter; ++iter) {
    ...
    for (int i = 0; i < 100; ++i) {
      PerformanceStatistics run_statistics =
          Evaluate(environment, policy, gamma, 1000);
    ...
    // experts[i] = new KNNClassifier(state_dimension,
    // environment->getNActions(), n_neighbours);
    //}
    // HashedClassifierMixture<KNNClassifier> hcm_classifier(state_dimension,
    // environment->getNActions(), experts);
    // Classifier<Vector, int, Vector> new_classifier = hcm_classifier;
    // Classifier<Vector, int, Vector> *new_classifier = new
    // HashedClassifierMixture<KNNClassifier>(state_dimension,
    // environment->getNActions(), experts);

    Classifier<Vector, int, Vector>* new_classifier = new KNNClassifier(
        state_dimension, environment->getNActions(), n_neighbours);
