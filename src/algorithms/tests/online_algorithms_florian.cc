...
int main(int argc, char** argv) {
  ...
  for (uint run = 0; run < n_runs; ++run) {
    ...
    if (!strcmp(algorithm_name, "Sarsa")) {
      ...
      discrete_mdp =
          new DiscreteMDPCounts(n_states, n_actions, 1.0 / (real)n_states);
    ...
    if (model) {
      ...
      if (discrete_mdp) {
        ...
        for (int i = 0; i < n_samples; ++i) {
          ...
          // mean policy
          PolicyEvaluation sample_mean_PE(sample_mean_policy, sample_mdp,
                                          gamma);
...
#ifdef USE_FILES
  // Open the file
  const string SUFFIX = algorithm_name;
  const string FILENAME = "results/result_" + SUFFIX + ".dat";
  ofstream resultsFile;
  resultsFile.open(FILENAME.data());
#endif
...
#if 0
		// Write in the file
		resultsFile << statistics.ep_stats[i].steps << " "
...
#ifdef USE_FILES
    // Write in the file
    resultsFile << statistics.reward[i] << endl;
#endif
  }
