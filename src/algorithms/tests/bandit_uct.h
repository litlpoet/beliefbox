...
class BanditPrior {
 public:
  ...
  BanditPrior() {}
  BanditPrior(std::vector<BetaDistribution> prior_) : prior(prior_) {}
...
class BanditBelief {
 ...
 public:
  ...
  real ModelProbability(std::vector<real>& x) const { return prior.pdf(x); }
  /// Gives the Thompson return
...
class BeliefTree {
 ...
 public:
  ...
  int n_actions;
  real gamma;
  BeliefTree(BanditBelief prior, int state, int n_states_, int n_actions_,
             real gamma_)
