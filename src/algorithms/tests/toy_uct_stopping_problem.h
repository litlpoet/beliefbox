...
class SimpleBelief {
 ...
 public:
  /// Create a belief
  SimpleBelief(real alpha = 1.0, real beta = 1.0, real r1_ = 0.0,
               real r2_ = 1.0)
      : prior(alpha, beta), r1(r1_), r2(r2_) {}
  BetaDistribution getPrior() { return prior; }
  void update(int state, int action, real reward, int next_state) {
    if (state == 0 && action == 0) {
      if (reward == r1) {
        ...
        fprintf(stderr,
                "Reward of %f should not have been observed in this state\n",
                reward);
      }
    }
  }
...
class BeliefTree {
 ...
 public:
  ...
  int n_states;
  int n_actions;
  BeliefTree(SimpleBelief prior, int state, int n_states_, int n_actions_)
...
int MakeDecision(ExpansionMethod expansion_method, int n_states, int n_actions,
                 SimpleBelief prior, int state, real gamma, int n_iter,
