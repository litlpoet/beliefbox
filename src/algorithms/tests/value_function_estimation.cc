...
std::vector<Statistics> EvaluateAlgorithm(int n_steps, int n_episodes,
...
int main(int argc, char** argv) {
  ...
  if (argc != 6) {
    std::cerr << "Usage: value_function_estimation n_states n_actions gamma "
                 "lambda randomness\n";
    return -1;
  }
  ...
  if (1) {
    ...
    if (0) {
      ...
      environment =
          new Gridworld("/home/olethros/projects/beliefbox/dat/maze2",
                        randomness, pit_value, goal_value, step_value);
    }
    ...
    if (0) {
      ...
      mdp_model =
          new DiscreteMDPCounts(n_states, n_actions, 1. / ((real)n_states));

      algorithm = new ModelBasedRL(n_states, n_actions, gamma, epsilon,
                                   mdp_model, &rng);
    }

    std::vector<Statistics> run_statistics =
        EvaluateAlgorithm(n_steps, n_episodes, algorithm, environment, gamma);

    for (uint i = 0; i != run_statistics.size(); ++i) {
      std::cout << "R:" << run_statistics[i].total_reward
                << " V:" << run_statistics[i].discounted_reward << std::endl;
    }
    delete environment;
    delete algorithm;
    delete mdp_model;
...
std::vector<Statistics> EvaluateAlgorithm(int n_steps, int n_episodes,
...

  {
    ...
    printf("# Value iteration complete\n");
    for (uint i = 0; i < environment->getNStates(); ++i) {
      printf("%d %f # true_value\n", i, value_iteration.getValue(i));
    }
    delete mdp;
  }

  // return statistics;
  DiscreteMDPCounts model(environment->getNStates(), environment->getNActions(),
  ...
  for (int episode = 0; episode < n_episodes; ++episode) {
    ...
    for (int sample = 0; sample < n_samples; ++sample) {
      ...
      fflush(stdout);
      {
        PolicyEvaluation pe_mean(empirical_policy, mdp_samples[sample], gamma);
      ...
      {
        PolicyEvaluation pe_pessimistic(pessimistic_policy, mdp_samples[sample],
                                        gamma);
