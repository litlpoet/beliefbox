...
class Adversary {
 ...
 public:
  ...
  real getReward(int state, int action) const { return reward(state, action); }
  real getReward() const { return last_reward; }
  const Matrix& getRewardMatrix() const { return reward; }
};
...
/** This adversary randomly chooses a fixed reward function in the beginning and
 * never changes it. */
