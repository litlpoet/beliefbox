...
class ContextBanditPrior {
 ...
 public:
  ...
  ContextBanditPrior(int n_states, int n_actions, real tau_, real mu_0,
                     real tau_0) {
    tau = tau_;
...
class ContextBanditBelief {
 ...
 public:
  ...
  /// Create a belief
  ContextBanditBelief(int n_states_, int n_actions_, real tau, real mu_0,
                      real tau_0)
      : n_states(n_states_),
...
class BeliefTree {
 protected:
  std::vector<Distribution*>
      densities;  ///< hold densities for the tree-derived MDPs.
 public:
  class Edge;
  ...
  class Node {
   public:
    ...
    real GetPathProbability() { return p; }
  ...
  class Edge {
   public:
    ...
    real GetEdgeProbability() { return p; }
  ...
  int n_actions;
  real gamma;
  BeliefTree(ContextBanditBelief prior, int state, int n_states_,
             int n_actions_, real gamma_)
      : n_states(n_states_), n_actions(n_actions_), gamma(gamma_) {
    root = new Node;
    root->belief = prior;
    root->state = state;
    root->index = 0;
  ...
  Node* ExpandAction(Node* selected_node, int a, real r, int s,
                     int verbose = 0) {
    Node* next = new Node;
    ...
    real p =
        selected_node->belief.getProbability(selected_node->state, a, r, s);
    ...
    if (verbose >= 100) {
      printf("Added edge  %d --(%d %f %f)-> %d\n", next_edge->src->index,
             next_edge->a, next_edge->r, next_edge->p, next_edge->dst->index);
    }
  ...
  Node* FindObservation(Node* src, int a, real r, int s, int verbose = 0) {
    for (typename std::list<Edge*>::iterator j = src->outs.begin();
         j != src->outs.end(); ++j) {
      Edge* edge = *j;
      if (edge->a == a && edge->r == r && edge->dst->state == s) {
        return edge->dst;
      }
    }
  ...
  DiscreteMDP CreateMeanMDP(real gamma, int verbose = 0) {
    ...
    {
      Distribution* reward_density = new SingularDistribution(0.0);
      densities.push_back(reward_density);
      for (int a = 0; a < n_actions; a++) {
        mdp.setRewardDistribution(0, a, reward_density);
      }
    }
    ...
    for (typename std::list<Node*>::iterator i = nodes.begin();
      ...
      for (typename std::list<Edge*>::iterator j = node->outs.begin();
           j != node->outs.end(); ++j) {
        Edge* edge = *j;
        Distribution* reward_density = new SingularDistribution(edge->r);

        densities.push_back(reward_density);
        mdp.setTransitionProbability(node->index, edge->a, edge->dst->index,
                                     edge->p);
        if (verbose >= 90) {
          printf("%d: - a=%d - r=%f - p=%f ->%d\n", node->index, edge->a,
                 edge->r, edge->p, edge->dst->index);
        }
        for (int a = 0; a < n_actions; a++) {
          mdp.setRewardDistribution(edge->dst->index, a, reward_density);
        }
      }
...
int MakeDecision(BeliefTree<ContextBanditBelief>& new_tree,
                 ...
                 int verbose, int max_value_iterations, FILE* fout = NULL);
