...
int main(int argc, char** argv) {
  ...
  DiscreteMDPCounts::RewardFamily reward_prior = DiscreteMDPCounts::NORMAL;
  real transition_prior = 0.5;
  ...
  for (uint run = 0; run < n_runs; ++run) {
    ...
    if (!strcmp(environment_name, "RandomMDP")) {
      ...
      environment = new ContextBandit(n_states, n_actions, rng, false);
      ...
      if (discrete_mdp) {
        ...
        for (int i = 0; i < n_samples; ++i) {
          ...
          for (int s = 0; s < n_states; ++s) {
            V_k(i, s) = mean_PE.getValue(s);
            L_k(i, s) = mmvi_PE.getValue(s);
            U_k(i, s) = upper_VI.getValue(s);
            hV[s] += V_k(i, s);
            hL[s] += L_k(i, s);
            hU[s] += U_k(i, s);
          }
        ...
        {
          printf("Upper bound samples\n");
          FILE* f = fopen("upper_bound.dat", "w");
          U_k.print(f);
          fclose(f);
        }

        {
          printf("Lower bound samples\n");
          FILE* f = fopen("lower_bound.dat", "w");
          L_k.print(f);
          fclose(f);
        }

        {
          printf("Empirical bound samples\n");
          FILE* f = fopen("empirical_bound.dat", "w");
          V_k.print(f);
          fclose(f);
        }

        {
          ...
          FILE* f = fopen("mean_value_function.dat", "w");
          MVI.getStateValues().print(f);
          fclose(f);
        }
...
Statistics EvaluateAlgorithm(int episode_steps, int n_episodes, uint n_steps,
  ...
  for (int i = 0; i < n_states; i++) {
    for (int a = 0; a < n_actions; a++) {
      ...
      printf(" %d %d %f %f # s, a, Q, ~Q VALUE_FUNCTION\n", i, a, V, hV);
      real err = V - hV;
      sse += err * err;
    }
  }
// statistics.ep_stats[episode].mse += sse /((real) (n_states*n_actions));
  ...
  printf("# Exiting after %d episodes, %d steps (%d %d)\n", episode, n_steps,
         (int)statistics.ep_stats.size(), (int)statistics.reward.size());
