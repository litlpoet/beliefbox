...
class Agent {
 public:
  ...
  Agent(int n_states, int n_actions, real gamma)
      : last_observation(0), terminal_state(n_states) {
    double dirichlet_mass = 0.1;
    double epsilon = 0.0;
    int max_samples = 1;

    std::cout << "# Creating new MDP model" << std::endl;
    model = new DiscreteMDPCounts(n_states + 1, n_actions, dirichlet_mass);

    std::cout << "# Creating new planner" << std::endl;
    sampling = new SampleBasedRL(n_states + 1, n_actions, gamma, epsilon, model,
  ...
  void EpisodeEnd(double reward) { sampling->Act(reward, terminal_state); }
};
...
const action_t *agent_step(double reward,
                           const observation_t *this_observation) {
  return agent->Step(reward, this_observation);
}

/** Called when the episode ends */
void agent_end(double reward) { return agent->EpisodeEnd(reward); }
