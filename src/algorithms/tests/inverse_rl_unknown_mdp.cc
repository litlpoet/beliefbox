/* -*- Mode: C++; -*- */
// copyright (c) 2010-2011 by Christos Dimitrakakis
...
enum SamplerType { INVALID = 0x0, METROPOLIS, MONTE_CARLO };
...
Statistics EvaluateAlgorithm(int episode_steps, int n_episodes, uint n_steps,
                             OnlineAlgorithm<int, int>* algorithm, real epsilon,
                             DiscreteEnvironment* environment, real gamma,
                             int iterations, Sampler sampler, real accuracy,
                             RandomNumberGenerator* rng);
static const char* const environment_names_list =
    "{MountainCar, ContextBandit, RandomMDP, Gridworld, Chain, Optimistic, "
    "Inventory}";
...
int main(int argc, char** argv) {
        ...
        case 0:
          ...
          switch (option_index) {
            ...
            case 7:
              ...
              break;
            case 8:
              printf("multi-sample not implented; ignored\n");
              break;
            case 9:
              maze_name = optarg;
              break;
            ...
            case 15:
              ...
              break;
            case 16:
              episode_steps = atoi(optarg);
              break;
            case 17:
              iterations = atoi(optarg);
              break;
            case 18:
              sampler.n_chain_samples = atoi(optarg);
              break;
            case 19:
              sampler.n_chains = atoi(optarg);
              break;
            case 20:
              sampler.type = METROPOLIS;
              break;
            case 21:
              sampler.type = MONTE_CARLO;
              break;
            case 22:
  ...
  if (!environment_name) {
    std::cerr << "Please choose an environment from " << environment_names_list
              << std::endl;
    exit(-1);
  }
  ...
  for (uint run = 0; run < n_runs; ++run) {
    ...
    if (!strcmp(environment_name, "RandomMDP")) {
      environment = new RandomMDP(n_actions, n_states, randomness, step_value,
                                  pit_value, goal_value, rng, false);
      ...
      environment = new DiscretisedEnvironment<MountainCar>(
          continuous_mountain_car, grid_size);
      ...
      environment =
          new InventoryManagement(n_states, n_actions, randomness, 0.1);
    } else {
    ...
    if (!strcmp(algorithm_name, "Oracle")) {
      ...
      algorithm = new QLearning(n_states, n_actions, gamma, lambda, alpha,
                                exploration_policy, 1.0);
    } else if (!strcmp(algorithm_name, "HQLearning")) {
      algorithm = new HQLearning(4, n_states, n_actions, gamma, lambda, alpha,
                                 0.01, 1.0);
      ...
      discrete_mdp = new DiscreteMDPCounts(n_states, n_actions,
                                           1.0 / (real)n_states, reward_prior);
      ...
      if (environment == gridworld) {
        collection =
            new DiscreteMDPCollection(*gridworld, 4, n_states, n_actions);
      } else {
    ...
    Statistics run_statistics = EvaluateAlgorithm(
        episode_steps, n_episodes, n_steps, algorithm, epsilon, environment,
        gamma, iterations, sampler, accuracy, rng);
...
Statistics EvaluateAlgorithm(int episode_steps, int n_episodes, uint n_steps,
  ...
  const DiscreteMDP* true_mdp = environment->getMDP();
  if (!true_mdp) {
    Serror("The environment requires a true MDP\n");
    exit(-1);
  }
  ValueIteration value_iteration(true_mdp, gamma);
  value_iteration.ComputeStateValues(accuracy);
  FixedSoftmaxPolicy softmax_policy(value_iteration.Q, 1.0 / epsilon);

  int n_states = true_mdp->getNStates();
  int n_actions = true_mdp->getNActions();
  ...
  MDPModel* mdp_model = new DiscreteMDPCounts(
      n_states, n_actions, 1.0 / (real)n_states, reward_prior);
  SampleBasedRL sample_based_rl(n_states, n_actions, gamma, accuracy, mdp_model,
                                rng, 1, true);
  for (uint step = 0; step < n_steps; ++step) {
    ...
    // std::cout << "t:" << current_time << " s:" << state << " r:" << reward <<
    // " a:" << action << std::endl;
    action_ok = environment->Act(action);
    current_time++;
    sample_based_rl.Observe(0, state, action);
    // perhaps do not observe all steps in the demonstration while the algorithm
    // is learning...
  ...
  sample_based_rl.Resample();
  {
    double start_time;
    double end_time;
    // fprintf (stderr, "Trying to guess policy!\n");

    DiscreteMDP* sampled_mdp = mdp_model->generate();
    if (!sampled_mdp->Check()) {
      Serror("MDP model is nonsense\n");
      sampled_mdp->ShowModel();
    }

    // -------- MWAL -------- //
    start_time = GetCPU();
    DiscreteMDP computation_mdp(*sampled_mdp);
    ...
    PolicyEvaluation mwal_evaluator(&mwal.mean_policy, true_mdp, gamma);
    ...
    PolicyEvaluation mwal_greedy_evaluator(&mwal_greedy, true_mdp, gamma);
    ...
    // -------- optimal -------- //
    ValueIteration VI(true_mdp, gamma);
    ...
    softmax_policy.Show();
    PolicyEvaluation smax_evaluator(&softmax_policy, true_mdp, gamma);
    ...
    start_time = GetCPU();
    DiscreteMDP mdp_copy(*sampled_mdp);
    RewardPolicyBelief reward_policy_belief(expected_optimality, gamma,
                                            mdp_copy, dirichlet,
                                            sampler.n_chain_samples);
    reward_policy_belief.setAccuracy(accuracy);

    FixedDiscretePolicy* rpb_policy =
    ...
    PolicyEvaluation rpb_evaluator(rpb_policy, true_mdp, gamma);
    ...
    start_time = GetCPU();
    PolicyRewardBelief prb(1.0, gamma, mdp_copy);

    if (sampler.type == METROPOLIS) {
      logmsg("Metropolis sampler: %d %d\n", sampler.n_chain_samples,
             sampler.n_chains);
      prb.MHSampler(demonstrations, sampler.n_chain_samples, sampler.n_chains);
    } else if (sampler.type == MONTE_CARLO) {
    ...
    PolicyEvaluation prb_evaluator(prb_policy, true_mdp, gamma);
    ...
    PolicyEvaluation imitating_evaluator(&imitating_policy, true_mdp, gamma);
