...
int main(int argc, char** argv) {
        ...
        case 0:
          ...
          switch (option_index) {
            ...
            case 18:
              ...
              break;
            case 19:
              goal_value = atof(optarg);
              break;
            case 20:
              step_value = atof(optarg);
              break;
            case 21:
              pit_value = atof(optarg);
              break;
            case 22:
  ...
  for (uint run = 0; run < n_runs; ++run) {
    ...
    EnvironmentGenerator<int, int>* environment_generator = NULL;
    if (!strcmp(environment_name, "RandomMDP")) {
      ...
      environment = new Gridworld(maze_name, randomness, pit_value, goal_value,
                                  step_value);
      ...
      real margin = 1.1;
      environment = new InventoryManagement(period, max_items, demand, margin);
      environment_generator =
          new InventoryManagementGenerator(period, max_items);
      ...
      environment =
          new DiscretisedEnvironment<Bike>(continuous_bicycle, grid_size);
    } else if (!strcmp(environment_name, "Pendulum")) {
      environment =
          new DiscretisedEnvironment<Pendulum>(continuous_pendulum, grid_size);
    } else if (!strcmp(environment_name, "CartPole")) {
      environment =
          new DiscretisedEnvironment<CartPole>(continuous_cart_pole, grid_size);
    } else if (!strcmp(environment_name, "Puddle")) {
      environment = new DiscretisedEnvironment<PuddleWorld>(
          continuous_puddle_world, grid_size);
    } else if (!strcmp(environment_name, "Acrobot")) {
      environment =
          new DiscretisedEnvironment<Acrobot>(continuous_acrobot, grid_size);
    } else {
    ...
    if (!strcmp(algorithm_name, "Oracle")) {
      ...
      algorithm = new Sarsa(n_states, n_actions, gamma, lambda, alpha,
                            exploration_policy, initial_reward);
      ...
      algorithm = new SarsaDirichlet(n_states, n_actions, gamma, lambda, alpha,
                                     exploration_policy);
      ...
      model = (MDPModel*)discrete_mdp;
      algorithm =
          new UCRL2(n_states, n_actions, gamma, discrete_mdp, rng, epsilon);
    } else if (!strcmp(algorithm_name, "LGBRL")) {
      discrete_mdp = new DiscreteMDPCounts(n_states, n_actions, dirichlet_mass,
                                           reward_prior);
      model = (MDPModel*)discrete_mdp;
      GradientBRL* gbrl = new GradientBRL(n_states, n_actions, gamma, epsilon,
                                          alpha, model, rng, false);
      algorithm = gbrl;
      ...
      DiscreteABCRL* abc = new DiscreteABCRL(
          n_states, n_actions, gamma, epsilon, environment_generator, rng,
  ...
  for (uint i = 0; i < statistics.reward.size(); ++i) {
    ...
    std::cout << statistics.n_runs[i] << " " << statistics.reward[i]
