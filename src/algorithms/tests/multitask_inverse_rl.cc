...
*/
Vector AddDemonstration(uint n_steps, Demonstrations<int, int>& demonstrations,
                        real epsilon, DiscreteEnvironment* environment,
                        real discount) {
  real accuracy = 1e-6;
...
Statistics EvaluateAlgorithm(Demonstrations<int, int>& demonstrations,
                             ...
                             real gamma, real epsilon, Sampler sampler,
                             real accuracy, int iterations);
  ...
  for (uint run = 0; run < n_runs; ++run) {
    ...
    for (uint i = 0; i < n_tasks; ++i) {
      ...
      if (!strcmp(environment_name, "RandomMDP")) {
        environment =
            new RandomMDP(n_actions, n_states, transition_randomness,
                          step_value, pit_value, goal_value, rng, false);
      } else if (!strcmp(environment_name, "Gridworld")) {
        real pit_value = -1.0 * (1.0 - randomness) + urandom() * randomness;
        real step_value =
            -0.1 * (1.0 - randomness) + (urandom() - 0.5) * randomness;
        real goal_value = 1.0 * (1.0 - randomness) + urandom() * randomness;
        environment = new Gridworld(maze_name, transition_randomness, pit_value,
                                    goal_value, step_value);
        ...
        environment = new DiscreteChain(
            n_states, 0.2, 0.2 * (1.0 - randomness) + urandom() * randomness,
            1.0 * (1.0 - randomness) + urandom() * randomness);
      } else if (!strcmp(environment_name, "Optimistic")) {
        environment = new OptimisticTask(
            0.1 * (1.0 - randomness) + urandom() * randomness,
            0.01 * (1.0 - randomness) + urandom() * randomness);
    ...
    for (uint demonstration = 0; demonstration < n_demonstrations;
         ++demonstration) {
    ...
    for (uint d = 0; d < n_demonstrations; ++d) {
      ...
      Vector V = AddDemonstration(episode_steps, demonstrations, epsilon,
                                  environment, gamma);
    ...
    Statistics run_statistics =
        EvaluateAlgorithm(demonstrations, environments, gamma, epsilon, sampler,
                          accuracy, iterations);
    // }}}
    for (uint i = 0; i < n_tasks; ++i) {
      delete tasks[i];
...
Statistics EvaluateAlgorithm(Demonstrations<int, int>& demonstrations,
  ...
  if (sampler.type == METROPOLIS) {
    logmsg("Metropolis sampler: %d %d\n", sampler.n_chain_samples,
           sampler.n_chains);
    sprb.MHSampler(demonstrations, sampler.n_chain_samples, sampler.n_chains);
...
#if 1
  printf("%f %f %f %f # DV run\n", statistics.DV(0), statistics.DV(1),
         statistics.DV(2), statistics.DV(3));
#endif
  fflush(stdout);
  fflush(stderr);
