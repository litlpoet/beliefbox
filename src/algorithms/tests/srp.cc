...
Statistics EvaluateAlgorithm(int n_episodes, uint n_steps,
...
int main(int argc, char** argv) {
  ...
  for (uint run = 0; run < n_runs; ++run) {
    ...
    Statistics run_statistics = EvaluateAlgorithm(
        n_episodes, n_steps, algorithm, environment, gamma, adversary);
    // delete actual_mdp;
