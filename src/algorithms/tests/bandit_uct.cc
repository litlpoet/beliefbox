...
int main(int argc, char** argv) {
  ...
  if (argc <= 1) {
    fprintf(stderr,
            "Usage: bandit_uct method gamma n_iter n_actions verbose "
            "n_experiments horizon\n");
    exit(-1);
  }
  ...
  for (int experiment = 0; experiment < n_experiments; experiment++) {
    BeliefTree<BanditBelief>* new_tree = new BeliefTree<BanditBelief>(
        prior_belief, 0, n_states, n_actions, gamma);
    ...
    for (int t = 0; t < horizon; t++) {
      ...
      int action =
          MakeDecision(*new_tree, method, n_states, n_actions, belief, state,
                       gamma, n_iter, verbose, max_value_iterations, fout);
      ...
      BeliefTreeNode* new_root =
          new_tree->FindObservation(new_tree->root, action, reward, next_state);
      ...
      if (new_root) {
        ...
        delete new_tree;
        new_tree =
            new BeliefTree<BanditBelief>(belief, 0, n_states, n_actions, gamma);
      }
    ...
    oracle_return =
        max_reward * (1.0 - pow(gamma, (real)(horizon + 1))) / (1.0 - gamma);
  ...
  // show statistics
  printf("%d %d %f %d %f %f %f %f %f %d\n", method, n_iter, gamma, n_actions,
         total_reward, discounted_reward, average_oracle_reward,
         average_oracle_return, end_time - start_time, n_experiments);
  return 0;
}
...
int MakeDecision(BeliefTree<BanditBelief>& new_tree,
                 ExpansionMethod expansion_method, int n_states, int n_actions,
                 BanditBelief prior, int state, real gamma, int n_iter,
                 int verbose, int max_value_iterations, FILE* fout) {
  ...
  int starting_iter =
      (tree.GetNumberOfLeafNodes() - 1) / (branching_factor - 1);
  ...
  for (int iter = starting_iter; iter < n_iter; iter++) {
    ...
    if (expansion_method == SerialExpansion) {
      ...
      for (int i = 0; i < n_leaf_nodes; i++) {
        ...
        real discount =
            node->GetPathProbability() * pow(gamma, (real)node->depth);
      ...
      upper_mdp.Check();
      ValueIteration value_iteration_upper(&upper_mdp, gamma);
      value_iteration_upper.ComputeStateActionValues(vi_threshold,
      ...
      while (1) {
        ...
        for (BTNodeSet::iterator i = node_set.begin(); i != node_set.end();
             ++i) {
        ...
        real pr = 0.0;
        real X = urandom();
        for (BTEdgeSet::iterator j = node->outs.begin(); j != node->outs.end();
             ++j) {
      ...
      real sampled_return = sampled_node->belief.estimateMDPReturn(
          sampled_mdp, sampled_node->state, gamma);
      ...
      for (int i = 0; i < n_leaf_nodes; i++) {
        ...
        // Ub.push_back(node->belief.estimateMDPreturn(sampled_mdp, node->state,
        // gamma) * w / sampled_weight);
    ...
    if (verbose >= 100) {
      std::cout << n_leaf_nodes << " leaf nodes, " << n_edge_nodes
                << " edge nodes, " << node_set.size() << " total nodes, "
                << "expanding node " << selected_node->index << std::endl;
    }
    ...
    if (verbose >= 10) {
      ...
      value_iteration_upper.ComputeStateActionValues(vi_threshold,
                                                     max_value_iterations);
      value_iteration_lower.ComputeStateActionValues(vi_threshold,
                                                     max_value_iterations);

      for (int a = 0; a < mean_mdp.getNActions(); a++) {
        std::cout << state << " " << a << " "
                  << value_iteration_lower.getValue(state, a) << " "
                  << value_iteration_upper.getValue(state, a) << " " << iter
                  << "# ITER Q" << std::endl;
      }
    }
  ...
  if (verbose >= 90) {
    std::cout << "Final expansion : " << tree.GetNumberOfLeafNodes()
              << " leaf nodes, " << tree.getNodes().size() << " nodes"
              << std::endl;
  }
  BTNodeSet& node_set = tree.getNodes();
  for (BTNodeSet::iterator i = node_set.begin(); i != node_set.end(); ++i) {
...
#if 1
  if (fout) {
    value_iteration_lower.ComputeStateValues(vi_threshold,
                                             max_value_iterations);
    value_iteration_upper.ComputeStateValues(vi_threshold,
                                             max_value_iterations);

    if (verbose >= 60) {
      std::cout << "mdp states " << mean_mdp.getNStates() << " nodes "
                << node_set.size() << std::endl;
      for (int s = 0; s < mean_mdp.getNStates() - 1; s++) {
        if (s >= (int)node_set.size()) {
          std::cerr << "ERROR in mdp size!" << s << " >= " << node_set.size()
                    << "mdp size = " << mean_mdp.getNStates() << std::endl;
          exit(-1);
        }
        real L_bound = -INF;
        real U_bound = INF;
        ...
        std::cout << "V(" << s << ")"
                  ...
                  << value_iteration_upper.getValue(s) << "] C [ " << L_bound
                  << ", " << U_bound << "] " << std::endl;
      }
    }

    for (int s = 0; s < mean_mdp.getNStates(); s++) {
      fprintf(fout, "s%d [label = \"%.2f - %.2f\"];\n", s,
  ...
  if (verbose >= 10) {
    ...
    for (int a = 0; a < mean_mdp.getNActions(); a++) {
      std::cout << "Q[" << s << ", " << a << "]"
                ...
                << value_iteration_upper.getValue(s, a) << std::endl;
    }
  }
  if (verbose >= 20) {
    for (int s = 0; s < mean_mdp.getNStates(); s++) {
