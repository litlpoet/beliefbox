...
struct Options {
  ...
  int n_iterations;          ///< number of value or policy iterations
  int n_evaluations;         ///< number of evaluations
  bool reuse_training_data;  ///< reuse training data!
...
*/
DiscretePolicy* getVIPolicy(DiscreteMDP* training_environment,
                            AbstractPolicy<Vector, int>& policy,
                            RBFBasisSet& RBFs, Options& options) {
...
void RunOnlineTest(Options& options) {
  ...
  std::vector<DiscretePolicy*> policy_list;
  std::vector<Demonstrations<int, int> > training_data_list;

  // Add the random policy to the list.
  policy_list.push_back(
      new DiscreteRandomPolicy(environment.getNActions(), &options.rng));

  Rollout<int, int, DiscretePolicy> training_rollouts(
      urandom(state_dimension), policy_list.back(), &environment, options.gamma,
      true);
...
int main(int argc, char* argv[]) {
  ...
  if (online_test) {
    for (int i = 0; i < options.n_evaluations; ++i) {
      ...
      if (!strcmp(options.environment_name, "InventoryManagement")) {
        logmsg("Testing inventory management\n");
        RunOnlineTest<InventoryManagementGenerator, InventoryManagement>(
            options);
      } else {
