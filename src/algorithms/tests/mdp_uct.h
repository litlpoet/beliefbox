...
class EasyMDPBelief {
 ...
 public:
  EasyMDPBelief() { n_actions = 0; }
  /// Create a belief
  EasyMDPBelief(int n_actions_, real alpha, real beta)
      : n_actions(n_actions_), prior(n_actions, alpha, beta) {}
  ~EasyMDPBelief() {
    for (int i = 0; i < n_actions; i++) {
      //    delete prior[i];
    }
...
class BeliefTree {
 ...
 public:
  ...
  int n_states;
  int n_actions;
  BeliefTree(EasyMDPBelief prior, int state, int n_states_, int n_actions_)
  ...
  Node* ExpandAction(int i, int a, real r, int s, int verbose = 0) {
    ...
    if (verbose >= 100) {
      printf("Added edge %d : %d --(%d %f %f)-> %d\n", k, edges[k]->src->index,
             edges[k]->a, edges[k]->r, edges[k]->p, edges[k]->dst->index);
    }
  ...
  DiscreteMDP CreateMeanMDP(real gamma, int verbose = 0) {
    ...
    for (int i = 0; i < n_nodes; i++) {
      ...
      for (int j = 0; j < n_edges; j++) {
        ...
        densities.push_back(reward_density);
        mdp.setTransitionProbability(i, edge->a, edge->dst->index, edge->p);
        if (verbose >= 90) {
          printf("%d: - a=%d - r=%f - p=%f ->%d\n", i, edge->a, edge->r,
                 edge->p, edge->dst->index);
        }
        for (int a = 0; a < n_actions; a++) {
      ...
      if (!n_edges) {
        real mean_reward =
            nodes[i]->belief.getGreedyReturn(nodes[i]->state, gamma);
        for (int a = 0; a < n_actions; a++) {
          mdp.setTransitionProbability(i, a, terminal, 1.0);

          Distribution* reward_density = new SingularDistribution(mean_reward);
          densities.push_back(reward_density);
    ...
    for (int i = 0; i < n_nodes; i++) {
      ...
      if (!n_edges) {
        ...
        while (nodes[i]->U.size() <= 0) {
          nodes[i]->U.push_back(
              nodes[i]->belief.sampleReturn(nodes[i]->state, gamma));
        }
        ...
        for (int a = 0; a < n_actions; a++) {
          ...
          Distribution* reward_density = new SingularDistribution(Ub);
          densities.push_back(reward_density);
          mdp.setRewardDistribution(i, a, reward_density);
        }
      }
