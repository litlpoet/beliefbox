...
void TrainingAlgorithm1(
    real gamma, int n_episodes, BayesianMultivariate* algorithm,
    ContinuousStateEnvironment* environment,
    RepresentativeStateModel<LinearModel<Vector, int>, Vector, int>* R);

void TrainingAlgorithm2(real gamma, int N, int n_episodes,
                        BayesianMultivariate* algorithm,
                        ContinuousStateEnvironment* environment);

Statistics EvaluateAlgorithm(int episode_steps, int n_episodes,
                             BayesianMultivariate* algorithm,
                             ContinuousStateEnvironment* environment,
                             real gamma);

static const char* const help_text =
    "BMR - Linear Bayesian reinforcement learning using Bayesian Multivariate Regression\n\
...
int main(int argc, char* argv[]) {
        ...
        case 0:
          ...
          switch (option_index) {
            ...
            case 3:
              ...
              break;
            case 4:
              n_train_episodes = atoi(optarg);
              break;
            case 5:
              n_test_episodes = atoi(optarg);
              break;
            case 6:
              n_steps = atoi(optarg);
              break;
  ...
  std::cout << environment_name << " environment creation completed"
            << std::endl;
  ...
  //	LinearModel<Vector,int>* lm = new LinearModel<Vector,int>( m, d_s, RBFs,
  // environment,regression_t);
  LinearModel<Vector, int>* lm = NULL;

  std::cout << "Fitted value iteration initialization..." << std::endl;
  FittedValueIteration<Vector, int>* FVI =
      new FittedValueIteration<Vector, int>(gamma, 3000, 1, grids, environment,
                                            regression_t, RBFs);

  std::cout << "Representative model creation..." << std::endl;
  //	RepresentativeStateModel<LinearModel<Vector,int>, Vector, int> *RSM =
  // new RepresentativeStateModel<LinearModel<Vector,int>,Vector,int>(gamma,
  // threshold, *lm, n_samples, n_actions,FVI);
  ...
  algorithm =
      new BayesianMultivariate(n_actions, m, d_s, gamma, epsilon, RBFs,
                               regression_t, regression_r, lm, RSM, FVI);
  ...
  for (uint run = 0; run < n_runs; ++run) {
    ...
    //		TrainingAlgorithm1(gamma, n_train_episodes, algorithm,
    //environment,
    // RSM);

    TrainingAlgorithm2(gamma, n_samples, n_train_episodes, algorithm,
                       environment);

    epsilon = 0.0;
    Statistics run_statistics = EvaluateAlgorithm(
        episode_steps, n_test_episodes, algorithm, environment, gamma);
    ...
    //			statistics.ep_stats[i].steps +=
    // run_statistics.ep_stats[i].steps;
    //			statistics.ep_stats[i].mse +=
    // run_statistics.ep_stats[i].mse;
  ...
  char buffer[100];
  sprintf(buffer, "BRL_RESULTS_STEPS_%s->(Rollouts = %d)", environment_name,
          n_train_episodes);
  ...
  fclose(output);
  sprintf(buffer, "BRL_RESULTS_REWARDS_%s->(Rollouts = %d)", environment_name,
          n_train_episodes);
...
void TrainingAlgorithm1(
  ...
  for (int episode = 1; episode < n_episodes; ++episode) {
    ...
    for (int r_sample = 0; r_sample < nsamples; ++r_sample) {
      for (int action_rr = 0; action_rr < (int)environment->getNActions();
           ++action_rr) {
        environment->Reset();
        current_time = 0;
...
void TrainingAlgorithm2(real gamma, int N, int n_episodes,
  ...
  for (int episode = 0; episode < n_episodes; ++episode) {
    ...
    //		printf("Number of representative states = %d\n",nsamples);
    //		for( int r_sample = 0; r_sample < nsamples; ++r_sample ) {
    //			for( int action_rr = 0; action_rr <
    //(int)environment->getNActions();
    //++action_rr) {
    environment->Reset();
    state = environment->getState();
    //				int action_rr =  (int) floor(urandom(0.0, (real)
