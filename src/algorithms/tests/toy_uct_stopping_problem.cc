...
int main(int argc, char** argv) {
  ...
  if (argc > 1) {
    ...
    printf(
        "Usage: toy_uct_stopping_problem method gamma n_iter verbose "
        "experiments horizon max_value_iterations\n");
  }
  ...
  for (int experiment = 0; experiment < n_experiments; experiment++) {
    real actual_probability =
        rng.uniform();  // urandom(0,1); //true_random(false); //urandom(0, 1);
    ...
    for (int t = 0; t < horizon; t++) {
      ...
      int action =
          MakeDecision(expansion_method, n_states, n_actions, belief, state,
  ...
  printf("%d %f %d %f %f %f\n", expansion_method, gamma, n_iter, total_reward,
         discounted_reward, average_oracle_return);
  return 0;
}
...
int MakeDecision(ExpansionMethod expansion_method, int n_states, int n_actions,
  ...
  if (verbose >= 60) {
    for (int s = 0; s < mdp.getNStates(); s++) {
      std::cout << "V[" << s << "]"
                << " = " << value_iteration.getValue(s) << std::endl;
    }
  }
  ...
  if (fout) {
    for (int s = 0; s < mdp.getNStates(); s++) {
      ...
      fprintf(fout, "s%d [label = \"%.2f %d %d\"];\n", s,
              value_iteration.getValue(s), (int)alpha, (int)beta);
      // belief.alpha,
      //       belief.beta);
    }
  }
  ...
  if (verbose >= 10) {
    for (int s = 0; s < mdp.getNStates(); s++) {
      for (int a = 0; a < mdp.getNActions(); a++) {
        std::cout << "Q[" << s << ", " << a << "]"
                  << " = " << value_iteration.getValue(s, a) << std::endl;
      }
    }
  }
