...
int main(int argc, char** argv) {
  ...
  if (argc > 1) {
    ...
    fprintf(
        stderr,
        "Usage: bandit_ucb method gamma actions verbose experiments horizon\n");
  }
  ...
  for (int experiment = 0; experiment < n_experiments; experiment++) {
    ...
    switch (method) {
      ...
      case 2:
        ...
        break;
      case 3:
        policy = new EpsilonGreedyPolicy(n_actions, 0.0,
                                         &biased_bernoulli_estimator);
        break;
      case 4:
  ...
  printf("%d %f %d %f %f %f %f\n", method, gamma, n_actions, total_reward,
         discounted_reward, average_oracle_reward, average_oracle_return);
  return 0;
}
