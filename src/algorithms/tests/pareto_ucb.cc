...
enum Method { UNDEFINED = 0x0, EPSILON_GREEDY, HOEFFDING_UCB };
...
int main(int argc, char** argv) {
  ...
  if (argc <= 6) {
    fprintf(stderr,
            "arguments: n_actions n_outcomes horizon n_runs method epsilon \n "
  ...
  for (int experiment = 0; experiment < n_experiments; experiment++) {
    ...
    switch (method) {
      case EPSILON_GREEDY:
        policy = new EpsilonGreedyObjectiveGeneralBandit(n_actions, n_outcomes,
                                                         rng, epsilon);
        break;
      case HOEFFDING_UCB:
        policy =
            new HoeffdingObjectiveGeneralBandit(n_actions, n_outcomes, rng);
        break;
      default:
