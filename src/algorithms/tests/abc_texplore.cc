...
class TotalRewardStatistic {
 ...
 public:
  ...
  real distance(Demonstrations<X, A>& data, Demonstrations<X, A>& sample) {
...
class CumulativeFeaturesStatistic {
 ...
 public:
  ...
  void setBound(real delta_, real gamma_, real R_max) { gamma = gamma_; }
  real getAverageTotalReward(Demonstrations<X, A>& data) {
    real r_d = 0;
    for (uint i = 0; i < data.total_rewards.size(); ++i) {
      r_d += data.total_rewards[i];
...
*/
AbstractPolicy<Vector, int>* getLSPIPolicy(
    Environment<Vector, int>* training_environment,
    AbstractPolicy<Vector, int>& policy,
    Rollout<Vector, int, AbstractPolicy<Vector, int> >* data, RBFBasisSet& RBFs,
    Options& options) {
  ...
  if (data && (!training_environment || options.reuse_training_data)) {
    ...
    rollout = new Rollout<Vector, int, AbstractPolicy<Vector, int> >(
        urandom(S_L, S_U), &policy, environment, options.gamma, true);
  }
  if (training_environment) {
  ...
  LSPI lspi(options.gamma, options.accuracy, state_dimension, n_actions,
            options.lspi_iterations, &RBFs, rollout);
  lspi.PolicyIteration();
  AbstractPolicy<Vector, int>* lspi_policy =
      new FixedContinuousPolicy(lspi.ReturnPolicy());
  delete rollout;
  return lspi_policy;
}
...
class ABCRL {
 public:
  ...
  void GetSample(std::vector<Demonstrations<X, A> >& data_list,
                 std::vector<P*>& policy_list, real discounting, real epsilon,
                 real delta, real R_max, int n_trajectories, int n_samples,
                 std::vector<M>& samples, std::vector<real>& values) {
...
void RunTest(Options& options) {
  ...
  Rollout<Vector, int, AbstractPolicy<Vector, int> > training_rollouts(
      urandom(S_L, S_U), &policy, &environment, options.gamma, true);
  training_rollouts.StartingDistributionSampling(options.n_training, -1);

  // start ABC-RL
  // ABCRL<G, TotalRewardStatistic<Vector, int>, M, AbstractPolicy<Vector, int>,
  // Vector, int> abcrl;
  ABCRL<G, CumulativeFeaturesStatistic<Vector, int>, M,
        AbstractPolicy<Vector, int>, Vector, int> abcrl;

  // start UCT
  EvenGrid discretize(environment.StateLowerBound(),
                      environment.StateUpperBound(), options.grid_uct);
  UCTMC<Vector, int> mcts(options.gamma, options.c_uct, &environment,
                          &options.rng, discretize, options.stepsize_uct,
                          options.lambda_uct, options.depth_uct,
                          options.n_rollouts_uct);
  ...
  abcrl.GetSample(training_data, policy, options.gamma, options.epsilon,
                  options.delta, options.R_max, options.n_trajectories,
                  options.n_samples, samples, values);
  abc_time += GetCPU() - abc_start;

  logmsg("Evaluating initial policy on %d samples\n", (int)samples.size());
  real V_initial = EvaluatePolicy<Vector, int, M, AbstractPolicy<Vector, int> >(
      environment, policy, options.gamma, options.n_testing);
  ...
  for (int i = 0; i < V_LSPI.Size(); ++i) {
    ...
    double abc_start = GetCPU();
    AbstractPolicy<Vector, int>* lspi_policy =
        getLSPIPolicy(&samples[i], policy, NULL, RBFs, estimation_options);
    abc_time += GetCPU() - abc_start;
    logmsg("Evaluating sampled policy\n");

    V_LSPI(i) = EvaluatePolicy<Vector, int, M, AbstractPolicy<Vector, int> >(
        environment, *lspi_policy, options.gamma, options.n_testing);
  ...
  if (samples.size() > 0) {
    ...
    for (int episode = 0; episode < options.n_testing; ++episode) {
      ...
      // logmsg("Sampled Episode = %d: Steps = %d, Total Reward = %f, Discounted
  ...
  AbstractPolicy<Vector, int>* oracle_lspi_policy =
      getLSPIPolicy(NULL, policy, &training_rollouts, RBFs, options);
  lspi_time += GetCPU() - lspi_start;
  logmsg("Evaluating LSPI policy\n");

  real V_lspi_oracle =
      EvaluatePolicy<Vector, int, M, AbstractPolicy<Vector, int> >(
          environment, *oracle_lspi_policy, options.gamma, options.n_testing);

  printf(
      "%f %f %f %f %f %f %f # V V_ABC_LSPI V_LSPI V_ABC_UCT T_ABC T_LSPI "
      "T_ABC_UCT\n",
      V_initial, V_LSPI.Sum() / (real)V_LSPI.Size(), V_lspi_oracle, V_ABC_UCT,
      abc_time, lspi_time, abc_uct_time);
}
...
void RunOnlineTest(Options& options) {
  ...
  policy_list.push_back(
      new RandomPolicy(environment.getNActions(), &options.rng));

  Rollout<Vector, int, AbstractPolicy<Vector, int> > training_rollouts(
      urandom(S_L, S_U), policy_list.back(), &environment, options.gamma, true);
  ...
  for (int episode = 0; episode < options.n_training; ++episode) {
    ...
    if (options.sampling) {
      ...
      for (int i = 0; i < n_episodes_per_iteration; ++i) {
        training_data.Simulate(environment, policy, options.gamma,
                               rollout_horizon);
      }
      ...
      // ABCRL<G, TotalRewardStatistic<Vector, int>, M, AbstractPolicy<Vector,
      // int>, Vector, int> abcrl;

      ABCRL<G, CumulativeFeaturesStatistic<Vector, int>, M,
            AbstractPolicy<Vector, int>, Vector, int> abcrl;
      std::vector<M> samples;
      std::vector<real> values;
      abcrl.GetSample(training_data_list, policy_list, options.gamma,
                      options.epsilon, options.delta, options.R_max,
                      options.n_trajectories, options.n_samples, samples,
                      values);
      Options estimation_options = options;
      estimation_options.n_training = options.n_model_rollouts;
      AbstractPolicy<Vector, int>* sampled_policy = getLSPIPolicy(
          &samples[0], *policy_list[0], NULL, RBFs, estimation_options);
      policy_list.push_back(sampled_policy);
    } else {
      training_rollouts.StartingDistributionSampling(n_episodes_per_iteration,
                                                     rollout_horizon);
...
real EvaluateLSTD(Environment<Vector, int>& environment,
                  AbstractPolicy<Vector, int>& policy,
                  Demonstrations<Vector, int>& data, Options& options) {
  int state_dimension = environment.getNStates();
  int n_actions = environment.getNActions();
  Vector S_L = environment.StateLowerBound();
  Vector S_U = environment.StateUpperBound();
  ...
  EvenGrid Discretisation(S_L, S_U, options.grid);
  RBFBasisSet RBFs(Discretisation, options.scale);
  LSTDQ lstdq(options.gamma, state_dimension, n_actions, RBFs, data);
        ...
        case 0:
          ...
          switch (option_index) {
            ...
            case 2:
              ...
              break;
            case 3:
              options.n_trajectories = atoi(optarg);
              break;
            case 4:
              options.n_samples = atoi(optarg);
              break;
            case 5:
              options.n_training = atoi(optarg);
              break;
            case 6:
              options.n_testing = atoi(optarg);
              break;
            case 7:
              options.grid = atoi(optarg);
              break;
            case 8:
            ...
            case 9:
              ...
              break;
            case 10:
              options.accuracy = atof(optarg);
              break;
            case 11:
              options.lspi_iterations = atoi(optarg);
              break;
            case 12:
              options.n_evaluations = atoi(optarg);
              break;
            case 13:
              options.reuse_training_data = true;
              break;
            case 14:
              online_test = true;
              break;
            ...
            case 17:
              ...
              break;
            case 18:
              options.R_max = atof(optarg);
              break;
            case 19:
              options.c_uct = atof(optarg);
              break;
            case 20:
              options.n_rollouts_uct = atoi(optarg);
              break;
            case 21:
              options.lambda_uct = atof(optarg);
              break;
            case 22:
              options.stepsize_uct = atof(optarg);
              break;
            case 23:
              options.depth_uct = atoi(optarg);
              break;
            case 24:
              options.grid_uct = atoi(optarg);
              break;
            default:
