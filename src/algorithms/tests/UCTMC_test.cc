...
std::vector<EpisodeStatistics> RunTest(ContinuousStateEnvironment* environment,
  ...
  EvenGrid discretize(environment->StateLowerBound(),
                      environment->StateUpperBound(), options.grids);

  // UCTMCL<S,A> mcts(options.gamma, options.c_uct,  environment, &options.rng,
  // discretize, options.learning_rate, options.depth, options.n_rollouts);
  UCTMC<S, A> mcts(options.gamma, options.c_uct, environment, &options.rng,
                   discretize, options.learning_rate, options.lambda,
                   options.depth, options.n_rollouts);
        ...
        case 0:
          ...
          switch (option_index) {
            ...
            case 8:
              ...
              break;
            case 9:
              options.n_rollouts = atoi(optarg);
              break;
            case 10:
              seed_filename = optarg;
              break;
            case 11:
              options.n_episodes = atoi(optarg);
              break;
            case 12:
