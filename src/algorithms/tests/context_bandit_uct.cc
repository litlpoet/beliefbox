...
int main(int argc, char** argv) {
  ...
  for (int experiment = 0; experiment < n_experiments; experiment++) {
    BeliefTree<ContextBanditBelief>* new_tree =
        new BeliefTree<ContextBanditBelief>(prior_belief, 0, n_states,
                                            n_actions, gamma);
    ...
    for (int t = 0; t < horizon; t++) {
      ...
      if (new_root) {
        ...
        delete new_tree;
        new_tree = new BeliefTree<ContextBanditBelief>(
            belief, next_state, n_states, n_actions, gamma);
      }
  ...
  printf("%d %d %f %d %f %f %f %f %f %d %d\n", method, n_iter, gamma, n_actions,
         ...
         average_oracle_return, end_time - start_time, n_experiments, horizon);
...
int MakeDecision(BeliefTree<ContextBanditBelief>& new_tree,
                 ...
                 ContextBanditBelief prior, int state, real gamma, int n_iter,
                 int verbose, int max_value_iterations, FILE* fout) {
  double start_time = GetCPU();

  double vi_threshold = 0.0001;
  // BeliefTree<ContextBanditBelief> tree(prior, state, n_states, n_actions,
  // gamma);
