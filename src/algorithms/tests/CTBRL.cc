...
void OfflineCTBRL(real gamma, int n_steps, int n_episodes,
                  CoverBayesianMultivariate* algorithm,
                  ContinuousStateEnvironment* environment);

Statistics OnlineCTBRL(real gamma, int n_steps, int n_episodes,
                       CoverBayesianMultivariate* algorithm,
                       ContinuousStateEnvironment* environment);

void EvaluatePrediction(int N, CoverBayesianMultivariate* algorithm,
                        ContinuousStateEnvironment* environment);
...
int main(int argc, char* argv[]) {
  ...
  if (model_rbf) {
    ...
    int m = RBFs->size() + 1;  // redefinition of the input dimensions (size of
                               // the created basis functions plus a dummy
                               // state)
  ...
  std::cout
      << "--Creating environment models (one CoverTree for each action)..."
      << std::endl;
  ...
  for (int i = 0; i < n_actions; ++i) {
    if (!strcmp(options.sampling, "Thompson")) {
      cover[i] = new CoverTree(options.tree_c, options.a_model,
                               options.N0_model, RBFs, true);
    } else if (!strcmp(options.sampling, "Marginal")) {
      cover[i] = new CoverTree(options.tree_c, options.a_model,
                               options.N0_model, RBFs, false);
    } else {
  ...
  if (!strcmp(options.method, "FVI")) {
    ...
    FVI = new CoverFittedValueIteration<Vector, int>(
        options.gamma, options.n_samples, 1, options.grid, environment, cover,
        RBFs, options.scale);
  } else if (!strcmp(options.method, "LSTD")) {
    std::cout << "Least Square Temporal Difference (LSTD) Initialization"
              << std::endl;
    FLSTD = new CoverFittedLSTD<Vector, int>(options.gamma, options.n_samples,
                                             environment, cover, options.grid,
                                             options.scale);
  } else {
    fprintf(stderr, "Unknown environment %s \n", options.environment_name);
  }
  ...
  CoverBayesianMultivariate* algorithm = new CoverBayesianMultivariate(
      n_actions, options.gamma, options.epsilon, cover, FVI, FLSTD, FLSTDQ);
  ...
  for (int n = 0; n < options.n_runs; ++n) {
    ...
    if (online_test) {
      ...
      run_statistics =
          OnlineCTBRL(options.gamma, options.n_testing_e_steps,
                      options.n_testing_episodes, algorithm, environment);
    } else {
      episodes = options.n_testing_episodes;
      OfflineCTBRL(options.gamma, options.n_training_e_steps,
                   options.n_training_episodes, algorithm, environment);

      //			run_statistics =
      //EvaluateAlgorithm(options.gamma,
  ...
  char buffer[100];
  sprintf(buffer, "%s_CTBRL_RESULTS_STEPS_%s", test_mode,
          options.environment_name);
  ...
  fclose(output);
  sprintf(buffer, "%s_CTBRL_RESULTS_REWARDS_%s", test_mode,
          options.environment_name);
...
Statistics OnlineCTBRL(real gamma, int n_steps, int n_episodes,
  ...
  for (int episode = 0; episode < n_episodes; ++episode) {
    ...
    if (!update_flag) {
      action = (int)floor(
          urandom(0.0, (real)n_actions));  // Randomly selected action
    } else {
    ...
    if (total_steps > thres) update_flag = true;
    if (update_flag) {
      algorithm->Update();
    }
  }
...
Statistics EvaluateAlgorithm(real gamma, int episode_steps, int n_episodes,
  ...
  while (1) {
    ...
    if (!action_ok) {
      ...
      //				printf ("# episode %d complete Step =
      //%d\n", episode,current_time);
