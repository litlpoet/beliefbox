...
void OfflineLBRL(real gamma, int n_steps, int n_episodes,
                 BayesianMultivariate* algorithm,
                 ContinuousStateEnvironment* environment);

Statistics OnlineLBRL(real gamma, int n_steps, int n_episodes,
                      BayesianMultivariate* algorithm,
                      ContinuousStateEnvironment* environment);

void EvaluatePrediction(int N, BayesianMultivariate* algorithm,
                        ContinuousStateEnvironment* environment);

Statistics EvaluateAlgorithm(real gamma, int episode_steps, int n_episodes,
                             BayesianMultivariate* algorithm,
                             ContinuousStateEnvironment* environment);
...
int main(int argc, char* argv[]) {
        ...
        case 0:
          ...
          switch (option_index) {
            ...
            case 2:
              ...
              break;
            case 3:
              options.sampling = optarg;
              break;
            case 4:
              options.a_model = atof(optarg);
              break;
            case 5:
              options.N0_model = atof(optarg);
              break;
            case 6:
              options.n_samples = atoi(optarg);
              break;
            case 7:
              options.n_training_e_steps = atoi(optarg);
              break;
            case 8:
              options.n_training_episodes = atoi(optarg);
              break;
            case 9:
              options.n_testing_e_steps = atoi(optarg);
              break;
            case 10:
              options.n_testing_episodes = atoi(optarg);
              break;
            case 11:
            ...
            case 15:
              ...
              break;
            case 16:
              options.randomness = atof(optarg);
              break;
            case 17:
  ...
  int n_actions =
      environment->getNActions();  /// Number of environment's actions
  ...
  std::cout << "Creating environment: " << options.environment_name << " with "
            << n_states << " states and , " << n_actions << " actions.\n";

  int m = n_states +
          1;  // Input dimensions (input state dimension plus a dummy state)
  ...
  if (model_rbf) {
    ...
    m = RBFs->size() + 1;  // redefinition of the input dimensions (size of the
                           // created basis functions plus a dummy state)
    std::cout << "# Number of basis functions: " << m << std::endl;
  }

  std::cout << "# Creating " << n_actions << " bayesian multivariate models..."
            << std::endl;

  // Bayesian Multivariate Regression model for the system transition model, one
  // for each action
  std::vector<BayesianMultivariateRegression*> regression_t(n_actions);
  // Bayesian Multivariate Regression model for the system reward model, one for
  // each action
  ...
  for (int i = 0; i < n_actions; ++i) {
    if (!strcmp(options.sampling, "Thompson")) {
      regression_t[i] = new BayesianMultivariateRegression(
          m, d_s, S0s, options.N0_model, options.a_model, true);
      regression_r[i] = new BayesianMultivariateRegression(
          m, d_r, S0r, options.N0_model, options.a_model, true);
    } else if (!strcmp(options.sampling, "Marginal")) {
      regression_t[i] = new BayesianMultivariateRegression(
          m, d_s, S0s, options.N0_model, options.a_model, false);
      regression_r[i] = new BayesianMultivariateRegression(
          m, d_r, S0r, options.N0_model, options.a_model, false);
    } else {
      fprintf(stderr, "Unknown Sampling approach %s\n", options.sampling);
    }
  }

  std::cout << "Creation of the bayesian multivariate models completed..."
            << std::endl;
  ...
  if (!strcmp(options.method, "FVI")) {
    ...
    FVI = new FittedValueIteration<Vector, int>(
        options.gamma, options.n_samples, 1, options.grid, environment,
        regression_t, RBFs, options.scale);
  } else if (!strcmp(options.method, "LSTD")) {
    std::cout << "Least Square Temporal Difference (LSTD) Initialization"
              << std::endl;
    FLSTD = new FittedLSTD<Vector, int>(options.gamma, options.n_samples, 1,
                                        options.grid, environment, regression_t,
                                        RBFs, options.scale);
  } else {
    fprintf(stderr, "Unknown environment %s \n", options.environment_name);
  }

  BayesianMultivariate* algorithm = new BayesianMultivariate(
      n_actions, m, d_s, options.gamma, options.epsilon, RBFs, regression_t,
      regression_r, lm, RSM, FVI, FLSTD);
  ...
  for (int n = 0; n < options.n_runs; ++n) {
    ...
    if (online_test) {
      ...
      run_statistics =
          OnlineLBRL(options.gamma, options.n_testing_e_steps,
                     options.n_testing_episodes, algorithm, environment);
    } else {
      episodes = options.n_testing_episodes;
      OfflineLBRL(options.gamma, options.n_training_e_steps,
                  options.n_training_episodes, algorithm, environment);

      run_statistics =
          EvaluateAlgorithm(options.gamma, options.n_testing_e_steps,
                            options.n_testing_episodes, algorithm, environment);
    }
  ...
  char buffer[100];
  sprintf(buffer, "%s_LBRL_RESULTS_STEPS_%s", test_mode,
          options.environment_name);
  ...
  fclose(output);
  sprintf(buffer, "%s_LBRL_RESULTS_REWARDS_%s", test_mode,
          options.environment_name);
