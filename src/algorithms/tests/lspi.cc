    ...
    for (int i = 0; i < runs; ++i) {
      ...
      Rollout<Vector, int, AbstractPolicy<Vector, int> >* rollout =
          new Rollout<Vector, int, AbstractPolicy<Vector, int> >(
              state_init, policy, environment, gamma, true);
      ...
      LSPI* lspi = new LSPI(gamma, delta, state_dimension, n_actions,
                            max_iteration, RBFs, rollout);
      ...
      for (int j = 0; j < eval_iter; ++j) {
        PerformanceStatistics run_statistics =
            Evaluate(environment, lspi->ReturnPolicy(), gamma, 1000);
