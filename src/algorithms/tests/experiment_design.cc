...
real TreatmentValue(real p) { return p; }
...
class Hypothesis {
 public:
  ...
  Hypothesis(std::vector<BetaDistribution>& prior, int n_actions_, int t_,
             int T_, std::vector<real>& outcomes)
  ...
  Hypothesis(const std::vector<BetaDistribution>& prior, const int n_actions_,
             const int t_, const int T_, const std::vector<real>& outcomes,
             const int action, const real outcome)
      : belief(prior),
        n_actions(n_actions_),
        t(t_),
        T(T_),
        ...
        V(n_actions) {
    // printf ("prior: %f %f -> %f\n", belief[action].alpha,
    // belief[action].beta, belief[action].getMean());
    ...
    if (t < T) {
      ...
      // prior[action].alpha, prior[action].beta);
    }
  }
  ...
  void MakeChildren(const std::vector<real>& outcomes) {
    for (int a = 0; a < n_actions; ++a) {
      ...
      for (uint i = 0; i != outcomes.size(); ++i) {
        ...
        edge->child =
            new Hypothesis(belief, n_actions, t + 1, T, outcomes, a, x);
  ...
  void ShowValues() {
    for (uint i = 0; i < edges.size(); ++i) {
      ...
      printf("[%d (%d, %.1f): %f w.p. %f]\n", i, edges[i]->action,
             edges[i]->outcome, edges[i]->value, edges[i]->probability);
...
HypothesisEdge::~HypothesisEdge() { delete child; }
...
class ExperimentDesign {
 public:
  ...
  int T;
  ExperimentDesign(std::vector<real>& outcomes_, int n_actions_,
                   std::vector<BetaDistribution>& prior_, int T_)
      : outcomes(outcomes_), n_actions(n_actions_), prior(prior_), T(T_) {
    root = new Hypothesis(prior, n_actions, 0, T, outcomes);
  }
  ~ExperimentDesign() { delete root; }
  real Value() { return root->Value(); }

  void ShowValues() { root->ShowValues(); }
  int SelectAction() { return root->SelectAction(); }
};
...
void Experiment(int n_actions, int T, int n_iter) {
  ...
  printf("%f %f %f %f #VAL\n", lookahead_results / (real)n_iter,
         randomised_results / (real)n_iter, bayesopt_results / (real)n_iter,
