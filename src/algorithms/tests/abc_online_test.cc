...
class ABCRL {
 public:
  ...
  void GetSample(Demonstrations<X, A>& data, P& policy, real discounting,
                 real epsilon, int n_trajectories, int n_samples,
...
void RunTest(Options& options) {
  ...
  abcrl.GetSample(training_data, policy, options.gamma, options.epsilon,
                  options.n_trajectories, options.n_samples, samples, values);
  ...
  for (int i = 0; i < V.Size(); ++i) {
    V(i) = EvaluatePolicy<Vector, int, M, AbstractPolicy<Vector, int> >(
        samples[i], policy, options.gamma, options.n_testing);
    hV(i) = values[i];

    AbstractPolicy<Vector, int>* lspi_policy =
        getLSPIPolicy(samples[i], policy, training_data, RBFs, options);

    hV_LSPI(i) = EvaluatePolicy<Vector, int, M, AbstractPolicy<Vector, int> >(
        samples[i], *lspi_policy, options.gamma, options.n_testing);
  ...
  AbstractPolicy<Vector, int>* oracle_lspi_policy =
      getLSPIPolicy(environment, policy, training_data, RBFs, options);
  real V_lspi_oracle =
  ...
  printf(
      "%f %f %f %f %f %f %f # hV V V_oracle hV_LSPI V_LSPI V_LSPI_oracle "
      "V_LSTD\n",
      hV.Sum() / (real)hV.Size(), V.Sum() / (real)V.Size(), V_initial,
      hV_LSPI.Sum() / (real)hV_LSPI.Size(), V_LSPI.Sum() / (real)V_LSPI.Size(),
      V_lspi_oracle, V_LSTD);
}

AbstractPolicy<Vector, int>* getLSPIPolicy(
    Environment<Vector, int>& environment, AbstractPolicy<Vector, int>& policy,
    Demonstrations<Vector, int>& data, RBFBasisSet& RBFs, Options& options) {
  int state_dimension = environment.getNStates();
  int n_actions = environment.getNActions();
  Vector S_L = environment.StateLowerBound();
  Vector S_U = environment.StateUpperBound();
