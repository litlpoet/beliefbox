...
class CoverFittedLSTD {
 ...
 public:
  CoverFittedLSTD(const real& gamma_, const int& N_,
                  Environment<S, A>* environment_,
                  std::vector<CoverTree*> cover_, const int& grids_ = 0,
                  real scale_ = 1.0, const int& tilings_ = 0,
    ...
    if (!strcmp(environment->Name(), "Bike")) {
      ...
      if (tilings == 0) {
        ...
        printf(
            "# %d Tilings (%d x %d) was created (Total basis functions =%d)\n",
  ...
  const void Update(real threshold = 0.00001, int max_iter = -1) {
    ...
    do {
      ...
      b = (b * (1.0 / N));
      //			weights = ((1.0/N)*AA +
      //(lambda)*Matrix::Unity(dim,dim)).Inverse_LU()*(b*(1.0/N));
    ...
    if (!strcmp(environment->Name(), "Bike")) {
      ...
      for (int i = 0; i < n_actions; ++i) {
        std::vector<std::pair<int, real> > path =
            cover[i]->ExternalBasisCreation(state);
